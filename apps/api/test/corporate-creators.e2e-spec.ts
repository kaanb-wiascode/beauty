...
describe('Corporate creators (e2e)',()=>{
 ...
 it('governs creator profiles, collaborations and finance handoff by branch and permissions',async()=>{
  ...
  const financeRows=await prisma.$queryRawUnsafe<Array<{amount:unknown;accountCode:string;status:string}>>(`SELECT amount,expense_account_code AS "accountCode",status FROM corporate_marketing_expenses WHERE tenant_id=$1::text AND company_id=$2::text AND source_type='CREATOR' AND source_id=$3::text`,tenant.id,company.id,collaboration.body.id);expect(financeRows).toHaveLength(1);expect(Number(financeRows[0].amount)).toBe(50000);expect(financeRows[0].accountCode).toBe('760.05');expect(financeRows[0].status).toBe('PENDING_FINANCE');
