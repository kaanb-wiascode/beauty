  ...
  .superRefine((value, ctx) => {
    if (value.frequency === 'DAILY' && (value.dayOfWeek || value.dayOfMonth)) {
      ctx.addIssue({
        ...
        message: 'Daily schedules cannot define a weekday or month day',
      });
    }
    if (value.frequency === 'WEEKLY' && (!value.dayOfWeek || value.dayOfMonth)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Weekly schedules require only dayOfWeek',
      });
    }
    if (value.frequency === 'MONTHLY' && (!value.dayOfMonth || value.dayOfWeek)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Monthly schedules require only dayOfMonth',
      });
    }
  });
