...
import {
  ...
  getInstallmentRuntimeStatus,
} from './installment-planner';
...
describe('core commerce domain', () => {
  ...
  describe('session lifecycle', () => {
    ...
    it('does not allow consumed sessions to return to the pool', () => {
      expect(() => assertSessionTransition('CONSUMED', 'AVAILABLE')).toThrow(
        'Invalid session transition: CONSUMED -> AVAILABLE',
      );
    });
  });
  ...
  describe('installment planner', () => {
    it('splits money exactly and puts rounding remainder into the last installment', () => {
      ...
      expect(schedule.map((item) => item.amount)).toEqual([
