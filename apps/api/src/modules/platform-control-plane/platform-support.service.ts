...
export class PlatformSupportService {
  ...
  async createTicket(
    ...
    if (!subject || subject.length > 300) {
      throw new BadRequestException('subject must contain between 1 and 300 characters.');
    }
    const priority = this.requiredEnum(input.priority ?? 'MEDIUM', PRIORITIES, 'priority');
    const source = this.requiredEnum(input.source ?? 'PLATFORM', SOURCES, 'source');
    const description = this.cleanText(input.description, 10000);
    const requesterEmail = this.cleanText(input.requesterEmail, 320)?.toLowerCase() ?? null;
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const rows = await tx.$queryRaw<TicketRow[]>`
        INSERT INTO platform_support_tickets (
          ...
          assigned_platform_user_id, sla_policy_id, response_due_at, resolution_due_at,
          created_by_platform_user_id
        ) VALUES (
          ${tenantId}, ${subject}, ${description}, ${priority}, ${source}, ${requesterEmail},
          ${assignee}, ${policy.id},
        ...
        RETURNING
          ...
          assigned_platform_user_id AS "assignedPlatformUserId",
          sla_policy_id AS "slaPolicyId",
          response_due_at AS "responseDueAt", resolution_due_at AS "resolutionDueAt",
          first_response_at AS "firstResponseAt", resolved_at AS "resolvedAt", closed_at AS "closedAt",
          created_by_platform_user_id AS "createdByPlatformUserId",
          created_at AS "createdAt", updated_at AS "updatedAt"
      `;
      const ticket = rows[0];
      await this.addEventTx(tx, ticket.id, actorUserId, 'CREATED', null, 'OPEN', null);
  ...
  async getTicket(ticketId: string) {
    const tickets = await this.prisma.$queryRaw<TicketRow[]>`
      SELECT
        id, tenant_id AS "tenantId", subject, description, priority, status, source,
        requester_email AS "requesterEmail", assigned_platform_user_id AS "assignedPlatformUserId",
    ...
    const events = await this.prisma.$queryRaw`
      SELECT id, event_type AS "eventType", from_status AS "fromStatus", to_status AS "toStatus",
             note, created_by_platform_user_id AS "createdByPlatformUserId", created_at AS "createdAt"
  ...
  async updateTicket(
    ...
    if (!normalizedReason) throw new BadRequestException('Support ticket changes require a reason.');
    const nextStatus = input.status === undefined ? undefined : this.requiredEnum(input.status, STATUSES, 'status');
    const nextPriority = input.priority === undefined ? undefined : this.requiredEnum(input.priority, PRIORITIES, 'priority');
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (nextStatus && nextStatus !== before.status) this.assertTransition(before.status, nextStatus);
      const assignee = input.assignedPlatformUserId === undefined
      ...
      const policy = nextPriority && nextPriority !== before.priority
      ...
      await tx.$executeRaw`
        ...
        SET priority = ${priority},
            ...
            sla_policy_id = CASE WHEN ${policy?.id ?? null}::text IS NULL THEN sla_policy_id ELSE ${policy?.id ?? null} END,
      ...
      const after = { ...before, priority, status, assignedPlatformUserId: assignee };
      await this.audit.record(
        {
          actorUserId,
          resource: 'support',
  ...
  async respond(
    ...
    if (!normalized || normalized.length > 5000) {
      throw new BadRequestException('Response note must contain between 1 and 5000 characters.');
    }
    return this.prisma.$transaction(async (tx) => {
      const rows = await tx.$queryRaw<TicketRow[]>`
        SELECT id, tenant_id AS "tenantId", status, first_response_at AS "firstResponseAt"
        FROM platform_support_tickets WHERE id = ${ticketId} FOR UPDATE
      `;
      const ticket = rows[0];
      if (!ticket) throw new NotFoundException('Support ticket not found.');
      if (ticket.status === 'CLOSED') throw new ConflictException('Closed tickets cannot receive a response.');
      await tx.$executeRaw`
        UPDATE platform_support_tickets
        ...
        WHERE id = ${ticketId}
      `;
      await this.addEventTx(tx, ticketId, actorUserId, 'RESPONSE', ticket.status, ticket.status === 'OPEN' ? 'IN_PROGRESS' : ticket.status, normalized);
      await this.audit.record(
        {
          actorUserId,
          resource: 'support',
          ...
          targetEntityType: 'platform_support_ticket',
          targetEntityId: ticketId,
          afterState: { firstResponseRecorded: ticket.firstResponseAt == null },
          correlationId: correlationId ?? null,
        },
        tx,
      );
  ...
  async summary() {
    ...
    >`
      SELECT
        ...
        COUNT(*) FILTER (WHERE first_response_at IS NULL AND response_due_at < CURRENT_TIMESTAMP AND status NOT IN ('RESOLVED','CLOSED'))::bigint AS "responseBreachedCount",
        COUNT(*) FILTER (WHERE resolved_at IS NULL AND resolution_due_at < CURRENT_TIMESTAMP AND status <> 'CLOSED')::bigint AS "resolutionBreachedCount",
        COUNT(*) FILTER (WHERE priority = 'URGENT' AND status NOT IN ('RESOLVED','CLOSED'))::bigint AS "urgentOpenCount"
      FROM platform_support_tickets
    `;
    const row = rows[0];
    return {
  ...
  async listPolicies() {
    return this.prisma.$queryRaw<PolicyRow[]>`
      SELECT id, name, priority, clock_mode AS "clockMode",
  ...
  async replacePolicy(
    ...
    const normalizedReason = reason.trim();
    if (!normalizedReason) throw new BadRequestException('SLA policy changes require a reason.');
    const priority = this.requiredEnum(input.priority ?? '', PRIORITIES, 'priority');
    const name = input.name?.trim();
    const responseMinutes = input.initialResponseMinutes;
    const resolutionMinutes = input.resolutionMinutes;
    if (!name || name.length > 200) throw new BadRequestException('SLA policy name is required and must not exceed 200 characters.');
    if (!Number.isInteger(responseMinutes) || !Number.isInteger(resolutionMinutes) || (responseMinutes ?? 0) <= 0 || (resolutionMinutes ?? 0) < (responseMinutes ?? 0)) {
      throw new BadRequestException('SLA minutes must be positive and resolution must be at least the response target.');
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.$executeRaw`
        ...
        SET status = 'INACTIVE', updated_by_platform_user_id = ${actorUserId}, updated_at = CURRENT_TIMESTAMP
      ...
      const rows = await tx.$queryRaw<PolicyRow[]>`
        ...
        ) VALUES (${name}, ${priority}, ${responseMinutes}, ${resolutionMinutes}, ${actorUserId}, ${actorUserId})
        RETURNING id, name, priority, clock_mode AS "clockMode",
                  initial_response_minutes AS "initialResponseMinutes",
                  resolution_minutes AS "resolutionMinutes", status,
                  created_at AS "createdAt", updated_at AS "updatedAt"
      `;
  ...
  private async activePolicy(priority: Priority, db: any): Promise<PolicyRow> {
    ...
    `;
    const policy = rows[0];
    if (!policy) throw new ConflictException(`No active SLA policy exists for ${priority}.`);
    return policy;
  }

  private async assertActivePlatformAdmin(userId: string, db: any) {
    const rows = await db.$queryRaw<Array<{ userId: string }>>`
      ...
      LIMIT 1
    `;
    if (!rows[0]) throw new BadRequestException('Support assignee must be an active platform administrator.');
  }
  ...
  private async addEventTx(
    ...
    await tx.$executeRaw`
      INSERT INTO platform_support_ticket_events (
        ticket_id, event_type, from_status, to_status, note, created_by_platform_user_id
      ) VALUES (${ticketId}, ${eventType}, ${fromStatus}, ${toStatus}, ${note}, ${actorUserId})
    `;
  }
  ...
  private assertTransition(from: TicketStatus, to: TicketStatus) {
    ...
    if (!allowed[from].includes(to)) {
      throw new ConflictException(`Invalid support ticket transition: ${from} -> ${to}.`);
    }
  }

  private optionalEnum<const T extends readonly string[]>(value: string | undefined, allowed: T, field: string): T[number] | null {
    if (value == null || value.trim() === '') return null;
    return this.requiredEnum(value, allowed, field);
  }

  private requiredEnum<const T extends readonly string[]>(value: string, allowed: T, field: string): T[number] {
    const normalized = value.trim().toUpperCase();
    if (!allowed.includes(normalized as T[number])) throw new BadRequestException(`Invalid ${field}.`);
  ...
  private cleanText(value: string | null | undefined, maxLength: number) {
    ...
    if (normalized.length > maxLength) throw new BadRequestException(`Value must not exceed ${maxLength} characters.`);
    return normalized;
  }
}
