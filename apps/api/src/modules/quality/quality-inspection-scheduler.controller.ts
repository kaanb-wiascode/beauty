...
export class QualityInspectionSchedulerController {
  constructor(private readonly scheduler: QualityInspectionSchedulerService) {}
  ...
  processDue(@Body() body:{limit?:number;workerId?:string},@Req() req:{user?:{sub?:string}}){
    return this.scheduler.processDue(this.userId(req),{limit:body?.limit,workerId:body?.workerId});
  }
}
