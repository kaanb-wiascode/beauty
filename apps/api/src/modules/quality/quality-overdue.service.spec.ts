...
describe('QualityOverdueService',()=>{
  it('marks overdue findings and CAPAs with skip-locked processing',async()=>{
    let call=0;
    const client={$queryRawUnsafe:jest.fn(async()=>call++===0?[{id:'f1'}]:[{id:'c1'}])};
    ...
    await expect(service.process('u1',25)).resolves.toMatchObject({processedFindings:1,processedCapas:1});
    expect(client.$queryRawUnsafe.mock.calls[0][0]).toContain('FOR UPDATE SKIP LOCKED');
    expect(client.$queryRawUnsafe.mock.calls[1][0]).toContain("'OVERDUE'");
  });
});
