...
export class PosRefundService {
  ...
  private async completeAccounting(row: PosRefundRow) {
    const result = await this.financialEvents.recordInScope(
      ...
      row.id === row.financialEventId ? '' : await this.resolvePosTransactionId(row.id),
      {
        eventType: 'REFUND',
  ...
  private async resolvePosTransactionId(refundRequestId: string) {
    const rows = await this.prisma.$queryRawUnsafe<Array<{ posTransactionId: string }>>(
      `SELECT pos_transaction_id AS "posTransactionId"
       FROM pos_refund_requests WHERE id=$1::text LIMIT 1`,
      refundRequestId,
    );
    if (!rows.length) throw new NotFoundException('POS refund request not found.');
    return rows[0].posTransactionId;
  }
  ...
  async refund(posTransactionId: string, input: RefundInput) {
    ...
    const transactionRows = await this.prisma.$queryRawUnsafe<Array<{
      id: string;
      providerTransactionId: string;
      status: string;
      amount: number;
    ...
    if (amount - Number(transaction.amount) > 0.01) {
      throw new BadRequestException('Refund amount cannot exceed POS transaction amount.');
    }
    ...
    const existingRows = await this.prisma.$queryRawUnsafe<PosRefundRow[]>(
      `SELECT id,
              tenant_id AS "tenantId",
              company_id AS "companyId",
              branch_id AS "branchId",
    ...
    if (existingRows.length) {
      ...
      if (existing.providerTransactionId !== transaction.providerTransactionId || Math.abs(existing.amount - amount) > 0.01) {
    ...
    try {
      ...
      const raced = await this.prisma.$queryRawUnsafe<PosRefundRow[]>(
        `SELECT id,
                tenant_id AS "tenantId",
                company_id AS "companyId",
                branch_id AS "branchId",
      ...
      if (raced.length) throw new ConflictException('Refund request is already processing.');
      throw new ServiceUnavailableException('Refund request could not be initialized.');
    }
    ...
    if (Math.abs(providerResult.amount - amount) > 0.01 || providerResult.currency.toUpperCase() !== transaction.currency.toUpperCase()) {
      await this.prisma.$executeRawUnsafe(
