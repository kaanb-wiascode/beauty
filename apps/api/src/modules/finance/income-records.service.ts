...
export class IncomeRecordsService {
  ...
  private async validateDimensions(tx: Prisma.TransactionClient, categoryId: string, costCenterId?: string) {
  ...
  private async audit(tx: Prisma.TransactionClient, row: IncomeRow, actorId: string, eventType: string, reason?: string) {
    await tx.$executeRawUnsafe(
      `INSERT INTO income_audit_events(id,tenant_id,company_id,branch_id,income_record_id,actor_id,event_type,reason,after_state)
       VALUES($1::text,$2::text,$3::text,$4::text,$5::text,$6::text,$7,$8,$9::jsonb)`,
      randomUUID(), row.tenantId, row.companyId, row.branchId, row.id, actorId, eventType, reason ?? null, JSON.stringify(this.map(row)),
    );
  }
      ...
      if (input.sourceType && input.sourceId) {
        ...
        const existing = await tx.$queryRawUnsafe<IncomeRow[]>(
          `SELECT id,tenant_id AS "tenantId",company_id AS "companyId",branch_id AS "branchId",cost_center_id AS "costCenterId",
           ...
           FROM income_records WHERE tenant_id=$1::text AND company_id=$2::text AND source_type=$3 AND source_id=$4 LIMIT 1`,
          tenantId, companyId, input.sourceType, input.sourceId,
        );
      ...
      const rows = await tx.$queryRawUnsafe<IncomeRow[]>(
        `INSERT INTO income_records(
         ...
         RETURNING id,tenant_id AS "tenantId",company_id AS "companyId",branch_id AS "branchId",cost_center_id AS "costCenterId",
           ...
           reconciliation_status AS "reconciliationStatus",accounting_status AS "accountingStatus",source_type AS "sourceType",source_id AS "sourceId",
           version,created_by AS "createdBy",created_at AS "createdAt",updated_at AS "updatedAt"`,
        id, tenantId, companyId, branchId, input.costCenterId ?? null, input.categoryId, input.counterpartyName ?? null,
        input.counterpartyTaxNumber ?? null, input.documentType ?? null, input.documentNumber ?? null, input.documentDate ?? null,
        input.documentUrl ?? null, input.transactionDate, input.dueDate ?? null, input.grossAmount, input.netAmount, input.taxAmount,
        input.currency, input.exchangeRate, input.description ?? null, input.sourceType ?? null, input.sourceId ?? null, actorId,
      );
  ...
  async list(limit = 50) {
    ...
    const rows = await this.prisma.$queryRawUnsafe<IncomeRow[]>(
      `SELECT id,tenant_id AS "tenantId",company_id AS "companyId",branch_id AS "branchId",cost_center_id AS "costCenterId",
              ...
              reconciliation_status AS "reconciliationStatus",accounting_status AS "accountingStatus",source_type AS "sourceType",source_id AS "sourceId",
              version,created_by AS "createdBy",created_at AS "createdAt",updated_at AS "updatedAt"
       FROM income_records WHERE tenant_id=$1::text AND company_id=$2::text AND ($3::text IS NULL OR branch_id=$3::text)
  ...
  async get(id: string) {
    ...
    const rows = await this.prisma.$queryRawUnsafe<IncomeRow[]>(
      `SELECT id,tenant_id AS "tenantId",company_id AS "companyId",branch_id AS "branchId",cost_center_id AS "costCenterId",
              ...
              reconciliation_status AS "reconciliationStatus",accounting_status AS "accountingStatus",source_type AS "sourceType",source_id AS "sourceId",
              version,created_by AS "createdBy",created_at AS "createdAt",updated_at AS "updatedAt"
       FROM income_records WHERE id=$1::text AND tenant_id=$2::text AND company_id=$3::text AND ($4::text IS NULL OR branch_id=$4::text) LIMIT 1`,
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const updated = await tx.$queryRawUnsafe<IncomeRow[]>(
        `UPDATE income_records SET approval_status=$1::"FinanceApprovalStatus",version=version+1,updated_at=CURRENT_TIMESTAMP
         WHERE id=$2::text RETURNING id,tenant_id AS "tenantId",company_id AS "companyId",branch_id AS "branchId",cost_center_id AS "costCenterId",
         category_id AS "categoryId",counterparty_name AS "counterpartyName",counterparty_tax_number AS "counterpartyTaxNumber",document_type AS "documentType",
         document_number AS "documentNumber",document_date AS "documentDate",document_url AS "documentUrl",transaction_date AS "transactionDate",due_date AS "dueDate",
         gross_amount AS "grossAmount",net_amount AS "netAmount",tax_amount AS "taxAmount",currency,exchange_rate AS "exchangeRate",description,
         approval_status AS "approvalStatus",collection_status AS "collectionStatus",reconciliation_status AS "reconciliationStatus",accounting_status AS "accountingStatus",
         source_type AS "sourceType",source_id AS "sourceId",version,created_by AS "createdBy",created_at AS "createdAt",updated_at AS "updatedAt"`,
        next, id,
      );
      await this.audit(tx, updated[0], actorId, `INCOME_${next}`, reason);
  ...
  async update(id: string, input: UpdateIncomeInput, actorId: string) {
    ...
    if (current.version !== input.version) throw new ConflictException('Income record was modified by another request. Refresh and retry.');
    throw new BadRequestException('Income update field patching will be added in the next finance slice.');
  }
}
