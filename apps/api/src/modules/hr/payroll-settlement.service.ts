    ...
    return this.prisma.$transaction(async tx=>{
      ...
      const totals=await tx.$queryRawUnsafe<any[]>(
        `SELECT
         ...
         FROM payroll_items WHERE period_id=$1::text AND tenant_id=$2::text AND company_id=$3::text
           AND ($4::text[] IS NULL OR branch_id=ANY($4::text[]))`,periodId,tenantId,companyId,branchIds);
      ...
      await tx.$executeRawUnsafe(
        `INSERT INTO payroll_liability_payments(id,tenant_id,company_id,branch_id,period_id,type,amount,method,payment_account_code,note,journal_entry_id,created_by_user_id,status)
         VALUES($1::text,$2::text,$3::text,$4::text,$5::text,$6,$7,$8,$9,$10,$11::text,$12::text,'PAID')`,settlementId,tenantId,companyId,periodBranchId,periodId,type,amount,method,paymentCode,note??null,entry.id,userId);
