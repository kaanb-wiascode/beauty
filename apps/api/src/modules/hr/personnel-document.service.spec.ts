...
describe('PersonnelDocumentService',()=>{
  ...
  it('archives an already archived document idempotently',async()=>{const db=prisma();db.tx.$queryRawUnsafe.mockResolvedValueOnce([{id:'doc-1',status:'ARCHIVED'}]);const service=new PersonnelDocumentService(db as never,ctx as never,organizationScope as never);const result=await service.archive('staff-1','doc-1');expect(result).toEqual({id:'doc-1',status:'ARCHIVED'});expect(db.tx.$queryRawUnsafe).toHaveBeenCalledTimes(1)});

  it('casts bigint file size for JSON-safe list output',async()=>{const db=prisma();db.$queryRawUnsafe.mockResolvedValue([]);const service=new PersonnelDocumentService(db as never,ctx as never,organizationScope as never);await service.list('staff-1',true);expect(String(db.$queryRawUnsafe.mock.calls[0][0])).toContain('d.file_size::text AS "fileSize"')});
});
