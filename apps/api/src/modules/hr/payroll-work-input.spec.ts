...
describe('PayrollWorkInputService',()=>{
  ...
  const organizationScope={getBranchScopedWhere:jest.fn().mockResolvedValue({branchId:'branch-a'})} as never;

  it('rejects invalid payroll periods before querying',async()=>{
    const prisma={$queryRawUnsafe:jest.fn()} as never;
    const service=new PayrollWorkInputService(prisma,tenant,organizationScope);
