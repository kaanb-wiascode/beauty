...
export class PayrollWorkInputService {
  ...
  async preview(year:number,month:number){
    const {tenantId,companyId,branchIds}=await this.context(); const {start,end}=this.bounds(year,month);
  ...
  async attachToDraft(periodId:string,staffId:string){
    ...
    return this.prisma.$transaction(async tx=>{
      const periods=await tx.$queryRawUnsafe<any[]>(`SELECT id,year,month,status,branch_id AS "branchId" FROM payroll_periods WHERE id=$1::text AND tenant_id=$2::text AND company_id=$3::text AND status='DRAFT' FOR UPDATE`,periodId,tenantId,companyId);
      if(!periods.length) throw new BadRequestException('Draft payroll period is required.'); const period=periods[0];
      if(branchIds!==null&&period.branchId!==null&&!branchIds.includes(period.branchId)) throw new NotFoundException('Draft payroll period not found.');
      const items=await tx.$queryRawUnsafe<any[]>(`SELECT id,branch_id AS "branchId",calculation_snapshot AS "calculationSnapshot" FROM payroll_items WHERE period_id=$1::text AND staff_id=$2::text AND tenant_id=$3::text AND company_id=$4::text AND ($5::text[] IS NULL OR branch_id=ANY($5::text[])) FOR UPDATE`,periodId,staffId,tenantId,companyId,branchIds);
      ...
      const stats=await tx.$queryRawUnsafe<any[]>(
        `SELECT COALESCE(SUM(ar.worked_minutes),0)::int AS "workedMinutes",COALESCE(SUM(ar.overtime_minutes),0)::int AS "overtimeMinutes",
                ...
                (SELECT COUNT(*)::int FROM leave_requests lr WHERE lr.staff_id=$1::text AND lr.tenant_id=$2::text AND lr.status='APPROVED' AND lr.start_date <= $4::date AND lr.end_date >= $3::date AND ($5::text[] IS NULL OR lr.branch_id=ANY($5::text[]))) AS "approvedLeaveRecords",
                (SELECT COALESCE(SUM(lr.days),0)::numeric FROM leave_requests lr WHERE lr.staff_id=$1::text AND lr.tenant_id=$2::text AND lr.status='APPROVED' AND lr.start_date <= $4::date AND lr.end_date >= $3::date AND ($5::text[] IS NULL OR lr.branch_id=ANY($5::text[]))) AS "declaredLeaveDays"
         FROM attendance_records ar WHERE ar.staff_id=$1::text AND ar.tenant_id=$2::text AND ar.work_date BETWEEN $3::date AND $4::date AND ($5::text[] IS NULL OR ar.branch_id=ANY($5::text[]))`,staffId,tenantId,start,end,branchIds);
