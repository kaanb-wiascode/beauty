...
export class Employee360Service {
  ...
  private scope() {
    ...
    if (roleScope !== 'CENTRAL' && !branchId) {
      throw new BadRequestException('A branch must be selected for this operation.');
    }
    return { tenantId, branchId: roleScope === 'CENTRAL' ? null : branchId };
  }
  ...
  async get(staffId: string, includeSensitive = false) {
    ...
    const staff = await this.prisma.staff.findFirst({
      where: { id: staffId, tenantId, ...(branchId ? { branchId } : {}) },
      select: {
        id: true,
        firstName: true,
        lastName: true,
    ...
    const [masterRows, assignments, employment, attendance, leaves, payroll, payments, appointmentStats, recentAppointments] = await Promise.all([
      ...
      this.prisma.$queryRawUnsafe<any[]>(`SELECT a.id,a.branch_id AS "branchId",b.name AS "branchName",a.department_id AS "departmentId",d.name AS "departmentName",a.team_id AS "teamId",t.name AS "teamName",a.position_id AS "positionId",p.name AS "positionName",a.manager_staff_id AS "managerStaffId",m."firstName" AS "managerFirstName",m."lastName" AS "managerLastName",a.effective_from AS "effectiveFrom",a.effective_to AS "effectiveTo",a.reason FROM hr_employee_assignments a JOIN branches b ON b.id=a.branch_id LEFT JOIN hr_departments d ON d.id=a.department_id LEFT JOIN hr_teams t ON t.id=a.team_id LEFT JOIN hr_positions p ON p.id=a.position_id LEFT JOIN staff m ON m.id=a.manager_staff_id WHERE a.tenant_id=$1 AND a.staff_id=$2 ORDER BY a.effective_from DESC,a.created_at DESC`, tenantId, staffId),
      this.prisma.$queryRawUnsafe<any[]>(`SELECT h.id,h.event_type AS "eventType",h.branch_id AS "branchId",b.name AS "branchName",h.effective_from AS "effectiveFrom",h.effective_to AS "effectiveTo",h.employment_type AS "employmentType",h.gross_salary AS "grossSalary",h.salary_type AS "salaryType",h.cost_center_id AS "costCenterId",h.reason,h.metadata,h.created_by AS "createdBy",h.created_at AS "createdAt" FROM hr_employment_history h JOIN branches b ON b.id=h.branch_id WHERE h.tenant_id=$1 AND h.staff_id=$2 ORDER BY h.effective_from DESC,h.created_at DESC`, tenantId, staffId),
      ...
      includeSensitive ? this.prisma.$queryRawUnsafe<any[]>(`SELECT pp.year,pp.month,pp.status AS "periodStatus",pi.gross_amount AS "grossAmount",pi.net_amount AS "netAmount",pi.employer_cost AS "employerCost" FROM payroll_items pi JOIN payroll_periods pp ON pp.id=pi.period_id WHERE pi.tenant_id=$1 AND pi.staff_id=$2 ORDER BY pp.year DESC,pp.month DESC LIMIT 12`, tenantId, staffId) : Promise.resolve([]),
      includeSensitive ? this.prisma.$queryRawUnsafe<any[]>(`SELECT sp.amount,sp.method,sp.status,sp.paid_at AS "paidAt",sp.note FROM salary_payments sp WHERE sp.tenant_id=$1 AND sp.staff_id=$2 ORDER BY sp.paid_at DESC LIMIT 12`, tenantId, staffId) : Promise.resolve([]),
    ...
    const safeEmployment = employment.map((item: any) => includeSensitive ? item : { ...item, grossSalary: undefined });
    const timeline = [
