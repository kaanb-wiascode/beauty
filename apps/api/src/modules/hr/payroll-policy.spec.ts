...
describe('PayrollPolicyService',()=>{
  const tenant={getTenantId:()=> 'tenant-a',getCompanyId:()=> 'company-a'} as never;
  const assignedScope={getBranchScopedWhere:jest.fn().mockResolvedValue({tenantId:'tenant-a',branchId:{in:['branch-a','branch-b']}})} as never;
  ...
  it('returns a disabled no-effect policy when company settings do not exist',async()=>{
    ...
    const service=new PayrollPolicyService(prisma,tenant,assignedScope);
  ...
  it('calculates policy preview only for assigned company branches',async()=>{
    ...
    expect(String(query.mock.calls[0][0])).toContain('s."branchId"=ANY($3::text[])');
    expect(query.mock.calls[0][3]).toEqual(['branch-a','branch-b']);
  });

  it('rejects policy evaluation for a branchless payroll period',async()=>{
    const tx={$queryRawUnsafe:jest.fn().mockResolvedValueOnce([{id:'period-a',year:2026,month:9,status:'DRAFT',branchId:null}])};
    const prisma={$queryRawUnsafe:jest.fn().mockResolvedValue([]),$transaction:jest.fn(async(fn:any)=>fn(tx))} as never;
    const service=new PayrollPolicyService(prisma,tenant,assignedScope);
    jest.spyOn(service,'getSettings').mockResolvedValue({enabled:false,applyOvertime:false,applyUnpaidLeaveDeduction:false,standardMonthlyMinutes:null,overtimeMultiplier:null,monthlyDayDivisor:null,updatedAt:null});
    await expect(service.attachEvaluation('period-a','staff-a')).rejects.toBeInstanceOf(BadRequestException);
    expect(String(tx.$queryRawUnsafe.mock.calls[0][0])).toContain('branch_id=ANY($4::text[])');
  });
});
