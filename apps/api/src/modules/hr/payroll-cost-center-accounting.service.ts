      ...
      async (tx) => {
        ...
        const groups = await tx.$queryRawUnsafe<any[]>(
          `SELECT cost_center_id AS "costCenterId",COALESCE(SUM(employer_cost),0)::numeric AS amount
           ...
           GROUP BY cost_center_id ORDER BY cost_center_id NULLS LAST`,
          periodId,
          tenantId,
          companyId,
          period.branchId,
        ...
        const current = await tx.journalEntryLine.findMany({
          where: { journalEntryId: period.journalEntryId, accountId: account.id },
        ...
        if (current.length === groups.length) {
          const linked = await tx.$queryRawUnsafe<any[]>(
            `SELECT jel.id,ccel.cost_center_id AS "costCenterId",jel.debit::numeric AS debit
             ...
             WHERE jel."journalEntryId"=$1::text AND jel."accountId"=$2::text`,
            period.journalEntryId,
            account.id,
          );
          const expected = groups
            .map((g) => `${g.costCenterId ?? 'UNALLOCATED'}:${this.round(Number(g.amount)).toFixed(2)}`)
            .sort();
          const actual = linked
            .map((l) => `${l.costCenterId ?? 'UNALLOCATED'}:${this.round(Number(l.debit)).toFixed(2)}`)
            .sort();
        ...
        await tx.$executeRawUnsafe(
          `DELETE FROM cost_center_expense_links WHERE journal_entry_line_id=$1::text`,
          current[0].id,
        );
        ...
        for (const group of groups) {
          ...
          const line = await tx.journalEntryLine.create({
            data: {
              ...
              debit: amount,
              credit: 0,
              memo: group.costCenterId
                ? 'Personel gideri - maliyet merkezi'
                : 'Personel gideri - dağıtılmamış',
            },
          });
