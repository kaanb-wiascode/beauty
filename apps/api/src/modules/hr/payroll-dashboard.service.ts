  ...
  async summary(year?: number, month?: number) {
    ...
    const [periods, totals, settlements, costCenters] = await Promise.all([
      ...
      this.prisma.$queryRawUnsafe<any[]>(
        `WITH salary AS (
           ...
           WHERE sp.tenant_id=$1::text AND sp.company_id=$2::text AND pp.year=$3 AND pp.month=$4
             AND ($5::text[] IS NULL OR sp.branch_id=ANY($5::text[]))
         ), liabilities AS (
           SELECT COALESCE(SUM(plp.amount) FILTER(WHERE plp.type='TAX' AND plp.status='PAID'),0)::numeric AS tax,
                  COALESCE(SUM(plp.amount) FILTER(WHERE plp.type='SOCIAL_SECURITY' AND plp.status='PAID'),0)::numeric AS social,
                  COALESCE(SUM(plp.amount) FILTER(WHERE plp.type='OTHER' AND plp.status='PAID'),0)::numeric AS other
           ...
           WHERE plp.tenant_id=$1::text AND plp.company_id=$2::text AND pp.year=$3 AND pp.month=$4
             AND ($5::text[] IS NULL OR plp.branch_id=ANY($5::text[]))
         ) SELECT salary.paid AS "salaryPaid",liabilities.tax AS "taxPaid",liabilities.social AS "socialPaid",liabilities.other AS "otherPaid" FROM salary CROSS JOIN liabilities`,
        tenantId,
        companyId,
        y,
        m,
      ...
      this.prisma.$queryRawUnsafe<any[]>(
        `SELECT pi.cost_center_id AS "costCenterId",COALESCE(cc.code,'UNALLOCATED') AS code,COALESCE(cc.name,'Dağıtılmamış') AS name,
         ...
         WHERE pi.tenant_id=$1::text AND pi.company_id=$2::text AND pp.year=$3 AND pp.month=$4
           AND ($5::text[] IS NULL OR pi.branch_id=ANY($5::text[]))
         GROUP BY pi.cost_center_id,cc.code,cc.name ORDER BY amount DESC`,
        tenantId,
        companyId,
        y,
        m,
    ...
    return {
      ...
      settlements: {
        ...
        salaryRemaining: Number(t.net ?? 0) - Number(s.salaryPaid ?? 0),
        taxPaid: s.taxPaid ?? 0,
        taxRemaining: Number(t.tax ?? 0) - Number(s.taxPaid ?? 0),
        socialPaid: s.socialPaid ?? 0,
        socialRemaining: Number(t.social ?? 0) - Number(s.socialPaid ?? 0),
        otherPaid: s.otherPaid ?? 0,
        otherRemaining: Number(t.other ?? 0) - Number(s.otherPaid ?? 0),
      },
      costCenters,
      periods,
    };
