...
export class CfoDashboardService {
  ...
  private normalizeQuery(query: CfoDashboardQuery) {
    ...
    if (!Number.isInteger(lookbackDays) || lookbackDays < 7 || lookbackDays > 730) {
      throw new BadRequestException('lookbackDays must be an integer between 7 and 730.');
    }
    return {
      asOf,
      lookbackDays,
      from: this.addDays(asOf, -lookbackDays),
    };
  }
  ...
  private async endingReceivables(asOf: Date) {
    ...
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
      `SELECT COALESCE(SUM(jel.debit-jel.credit),0)::numeric AS amount
       ...
       JOIN chart_of_accounts coa ON coa.id=jel."accountId" AND coa.code='120'
  ...
  private async endingPayables(asOf: Date) {
    const { companyId, branchId } = this.context();
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
      `SELECT COALESCE(SUM(jel.credit-jel.debit),0)::numeric AS amount
       FROM journal_entry_lines jel
       JOIN journal_entries je ON je.id=jel."journalEntryId" AND je.status='POSTED'
       JOIN chart_of_accounts coa ON coa.id=jel."accountId" AND coa.code='320'
       WHERE je."companyId"=$1::text
         AND ($2::text IS NULL OR je."branchId"=$2::text)
         AND je."entryDate"<$3::timestamptz`,
      companyId,
  ...
  private async inventoryValue() {
    const { companyId, branchId } = this.context();
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
      `SELECT COALESCE(SUM(st.quantity*st.cost_per_unit),0)::numeric AS amount
       FROM inventory_stock st
       JOIN inventory_warehouses w ON w.id=st.warehouse_id
       WHERE w.company_id=$1::text
    ...
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
      `WITH refunds AS (
       ...
       WHERE b."companyId"=$1::text
         ...
         AND s."confirmedAt">=$3::timestamptz
         AND s."confirmedAt"<$4::timestamptz`,
      companyId,
      branchId,
      from,
      this.addDays(asOf, 1),
  ...
  private async periodPurchases(from: Date, asOf: Date) {
    const { companyId, branchId } = this.context();
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
      `SELECT COALESCE(SUM(gri.quantity*gri.unit_cost),0)::numeric AS amount
       FROM inventory_goods_receipt_items gri
       JOIN inventory_goods_receipts gr ON gr.id=gri.goods_receipt_id
       ...
       WHERE gr.company_id=$1::text
         AND ($2::text IS NULL OR w.branch_id=$2::text)
         AND gr.received_at>=$3::timestamptz
         AND gr.received_at<$4::timestamptz`,
      companyId,
      branchId,
      from,
      this.addDays(asOf, 1),
  ...
  private async realizedCashFlow(from: Date, asOf: Date) {
    const { companyId, branchId } = this.context();
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
      `SELECT COALESCE(SUM(jel.debit-jel.credit),0)::numeric AS net_change
       FROM journal_entry_lines jel
       JOIN journal_entries je ON je.id=jel."journalEntryId" AND je.status='POSTED'
       ...
       WHERE je."companyId"=$1::text
         AND ($2::text IS NULL OR je."branchId"=$2::text)
         AND je."entryDate">=$3::timestamptz
         AND je."entryDate"<$4::timestamptz`,
      companyId,
      branchId,
      from,
      this.addDays(asOf, 1),
  ...
  async workingCapital(query: CfoDashboardQuery = {}) {
    ...
    const [receivables, payables, inventory, sales, purchases] = await Promise.all([
      this.endingReceivables(asOf),
      this.endingPayables(asOf),
      this.inventoryValue(),
      this.periodSales(from, asOf),
      this.periodPurchases(from, asOf),
    ]);
    ...
    return {
      ...
      dsoDays: dso,
      dpoDays: dpo,
      cashConversionGapDays: dso !== null && dpo !== null ? this.round(dso - dpo) : null,
    };
  }

  async cashRunway(query: CfoDashboardQuery = {}) {
    const { asOf, lookbackDays, from } = this.normalizeQuery(query);
    const [forecast, realizedNetCashFlow, settings] = await Promise.all([
      this.cashFlow.thirteenWeek(asOf, 'BASE'),
      this.realizedCashFlow(from, asOf),
      this.treasuryRisk.getSettings(),
    ]);
    ...
    const liquidityAboveFloor = Math.max(
      0,
      this.round(forecast.openingLiquidity - Number(settings.minimumLiquidity ?? 0)),
    );
    ...
    return {
      ...
      minimumLiquidity: Number(settings.minimumLiquidity ?? 0),
      liquidityAboveFloor,
      realizedNetCashFlow,
      averageWeeklyNetCashFlow: weeklyNetCashFlow,
      averageWeeklyBurn: weeklyBurn,
      runwayWeeks,
      runwayStatus:
        runwayWeeks === null
          ? 'SELF_FUNDING'
  ...
  async dashboard(query: CfoDashboardQuery = {}) {
    ...
    const [workingCapital, runway, forecast, alerts, stress, priorities] = await Promise.all([
    ...
    return {
      ...
      liquidity: {
        opening: forecast.openingLiquidity,
        thirteenWeekClosing: forecast.forecastClosingLiquidity,
        thirteenWeekLowest: forecast.lowestLiquidity,
        projectedNetCashFlow: forecast.projectedNetCashFlow,
        risk: forecast.liquidityRisk,
      },
      runway,
      workingCapital,
      receivablesRisk: {
        overdueOutstanding: stress.overdueOutstanding,
        overdueInstallmentCount: stress.overdueInstallmentCount,
        severeRecoverableCash: stress.stressScenarios.find((item) => item.scenario === 'SEVERE')
          ?.recoverableCash ?? 0,
      },
      treasuryAlerts: {
        alertCount: alerts.alertCount,
        firstRiskWeek: alerts.firstRiskWeek,
      },
      payments: {
        initialPaymentCapacity: priorities.initialPaymentCapacity,
        remainingPaymentCapacity: priorities.remainingPaymentCapacity,
        recommendedNowCount: priorities.recommendations.filter((item) => item.recommendedPayNow).length,
      },
    };
  }
}
