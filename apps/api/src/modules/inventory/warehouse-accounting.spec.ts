...
describe('WarehouseAccountingService', () => {
  ...
  function scope(branchIds: string[] | null = ['branch-a']) {
    return {
  ...
  it('scopes warehouse valuation by tenant company and assigned branches', async () => {
    const query = jest.fn().mockResolvedValue([]);
    const service = new WarehouseAccountingService(
    ...
    expect(String(query.mock.calls[0][0])).toContain('w.branch_id=ANY($3::text[])');
  ...
  it('reports warehouse subledger against the 150 control account within assigned branches', async () => {
    const query = jest.fn()
    ...
    expect(String(query.mock.calls[1][0])).toContain('je."branchId"=ANY($3::text[])');
  });

  it('uses company-wide scope only when the inventory scope adapter allows it', async () => {
    const query = jest.fn().mockResolvedValue([]);
    const service = new WarehouseAccountingService(
      { $queryRawUnsafe: query } as never,
      tenant,
