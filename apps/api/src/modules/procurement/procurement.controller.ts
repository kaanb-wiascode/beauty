...
export class ProcurementController {
  ...
  @Post('purchase-requests/:id/approve') approvePurchaseRequest(@Param('id') id: string) { return this.requests.approvePurchaseRequest(id); }
  @Post('purchase-requests/:id/convert') convertPurchaseRequest(@Param('id') id: string, @Body() body: unknown) { return this.requests.convertPurchaseRequest(id, convertPurchaseRequestSchema.parse(body)); }
  @Post('purchase-orders/:id/submit-approval') submitPurchaseOrderApproval(@Param('id') id: string) { return this.approvals.submit(id); }
  @Get('purchase-orders/:id/approvals') getPurchaseOrderApprovals(@Param('id') id: string) { return this.approvals.getState(id); }
  @Post('purchase-orders/:id/approvals/:level/approve') approvePurchaseOrderLevel(@Param('id') id: string, @Param('level') level: string, @Req() req: { user?: { sub?: string } }) { return this.approvals.approve(id, Number(level), this.userId(req)); }
  @Post('purchase-orders/:id/approvals/:level/reject') rejectPurchaseOrderLevel(@Param('id') id: string, @Param('level') level: string, @Req() req: { user?: { sub?: string } }) { return this.approvals.reject(id, Number(level), this.userId(req)); }
  @Post('purchase-orders/:id/order') orderPurchaseOrder(@Param('id') id: string) { return this.service.orderPurchaseOrder(id); }
  @Post('purchase-orders/:id/receive') receivePurchaseOrder(@Param('id') id: string, @Body() body: unknown) { return this.service.receivePurchaseOrder(id, receiveSchema.parse(body)); }
  @Post('goods-receipts/:id/reverse') reverseGoodsReceipt(@Param('id') id: string, @Body() body: unknown) { return this.service.reverseGoodsReceipt(id, reverseReceiptSchema.parse(body)); }
  @Post('goods-receipts/:id/partial-return') partialReturn(@Param('id') id: string, @Body() body: unknown) { return this.returns.partialReturn(id, partialReturnSchema.parse(body)); }
  ...
  @Get('return-requests') listReturnRequests(@Query() query: unknown) { const parsed = returnRequestListSchema.parse(query); return this.returnRequests.list(parsed.status); }
  @Post('return-requests/:id/approve') approveReturnRequest(@Param('id') id: string, @Req() req: { user?: { sub?: string } }) { return this.returnRequests.approve(id, this.userId(req)); }
  @Post('return-requests/:id/reject') rejectReturnRequest(@Param('id') id: string, @Body() body: unknown, @Req() req: { user?: { sub?: string } }) { return this.returnRequests.reject(id, this.userId(req), rejectReturnSchema.parse(body).reason); }
  @Post('return-requests/:id/execute') executeReturnRequest(@Param('id') id: string) { return this.returnRequests.execute(id); }
  ...
  @Get('goods-receipts') listGoodsReceipts(@Query() query: unknown) { const parsed = receiptListSchema.parse(query); return this.service.listGoodsReceipts(parsed.purchaseOrderId); }
  @Get('purchase-returns') listPurchaseReturns(@Query() query: unknown) { const parsed = returnListSchema.parse(query); return this.returns.listReturns(parsed.goodsReceiptId); }
  @Get('supplier-credit-notes') listSupplierCreditNotes(@Query() query: unknown) { const parsed = creditNoteListSchema.parse(query); return this.returns.listCreditNotes(parsed.supplierBillId); }
}
