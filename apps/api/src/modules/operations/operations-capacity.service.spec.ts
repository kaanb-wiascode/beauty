...
describe('OperationsCapacityService', () => {
  ...
  it('counts only allocation overlap inside the requested window', async () => {
    ...
    expect(result.totals.blockedMinutes).toBe(0);
  ...
  it('removes unavailable resources from effective capacity', async () => {
    ...
    expect(result.totals.capacityMinutes).toBe(60);
    expect(result.totals.blockedMinutes).toBe(60);
    expect(result.categories[0]?.totalResources).toBe(2);
    expect(result.categories[0]?.unavailableResources).toBe(1);
  });

  it('merges overlapping resource blocks before reducing capacity', async () => {
    queryRawUnsafe
      .mockResolvedValueOnce([
        {
          resourceType: 'ROOM',
      ...
      .mockResolvedValueOnce([
        {
          roomId: 'room-1',
          assetId: null,
          blockedFrom: new Date('2026-09-15T09:10:00.000Z'),
          blockedTo: new Date('2026-09-15T09:40:00.000Z'),
        },
        {
          roomId: 'room-1',
          assetId: null,
          blockedFrom: new Date('2026-09-15T09:30:00.000Z'),
          blockedTo: new Date('2026-09-15T09:50:00.000Z'),
        },
      ]);
    ...
    expect(result.totals.blockedMinutes).toBe(40);
    expect(result.totals.capacityMinutes).toBe(20);
    expect(result.totals.remainingMinutes).toBe(20);
    expect(result.resources[0]?.blockedMinutes).toBe(40);
  });
});
