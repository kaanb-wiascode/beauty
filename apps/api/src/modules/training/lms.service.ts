...
export class LmsService {
  ...
  private async calculateResult(assignmentId:string,tx:any){
    const c=this.context(),a=await this.assignment(assignmentId,tx);if(!a.courseVersionId)throw new BadRequestException('Legacy assignment has no pinned course version.');const vr=await tx.$queryRawUnsafe<any[]>(`SELECT id,requires_theory AS "requiresTheory",requires_practical AS "requiresPractical",theory_pass_score AS "theoryPassScore",practical_pass_score AS "practicalPassScore" FROM training_course_versions WHERE id=$1::text AND tenant_id=$2::text AND company_id=$3::text LIMIT 1`,a.courseVersionId,c.tenantId,c.companyId);if(!vr.length)throw new NotFoundException('Pinned course version not found.');const v=vr[0];
    const theory=await tx.$queryRawUnsafe<any[]>(`WITH active AS (SELECT id,pass_score FROM training_exams WHERE course_version_id=$1::text AND tenant_id=$2::text AND company_id=$3::text AND is_active=true),best AS (SELECT exam_id,MAX(score) AS score FROM training_exam_attempts WHERE assignment_id=$4::text GROUP BY exam_id) SELECT COUNT(active.id)::int AS exams,COUNT(best.exam_id)::int AS attempted,COALESCE(AVG(best.score),0)::numeric AS score,COALESCE(BOOL_AND(best.score>=active.pass_score),false) AS all_passed FROM active LEFT JOIN best ON best.exam_id=active.id`,a.courseVersionId,c.tenantId,c.companyId,assignmentId);const t=theory[0]??{exams:0,attempted:0,score:0,all_passed:false};const theoryScore=Number(t.score??0);const theoryPassed=!v.requiresTheory||(Number(t.exams)>0&&Number(t.attempted)===Number(t.exams)&&Boolean(t.all_passed)&&theoryScore>=Number(v.theoryPassScore??0));
    const practical=await tx.$queryRawUnsafe<any[]>(`SELECT score,passed FROM training_practical_assessments WHERE assignment_id=$1::text AND tenant_id=$2::text AND company_id=$3::text ORDER BY assessed_at DESC,created_at DESC LIMIT 1`,assignmentId,c.tenantId,c.companyId);const p=practical[0];const practicalScore=p?Number(p.score):null;const practicalPassed=!v.requiresPractical||(!!p&&Boolean(p.passed)&&Number(p.score)>=Number(v.practicalPassScore??0));
