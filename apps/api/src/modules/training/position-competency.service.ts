...
export class PositionCompetencyService {
  ...
  async createMapping(input:{position:string;profileId:string;effectiveFrom?:string;effectiveTo?:string|null},actor:string){
    ...
    return this.prisma.$transaction(async tx=>{
      ...
      await tx.$executeRawUnsafe(`WITH _advisory_lock AS (SELECT pg_advisory_xact_lock(hashtext($1))) SELECT 1 FROM _advisory_lock`,`position-competency:${c.tenantId}:${c.companyId}:${positionKey}`);
  ...
  async process(actor:string,limit=200){const c=this.c(),safe=Math.min(Math.max(Math.trunc(Number(limit)||200),1),500);return this.prisma.$transaction(async tx=>{
    ...
    for(const row of rows){
      await tx.$executeRawUnsafe(`WITH _advisory_lock AS (SELECT pg_advisory_xact_lock(hashtext($1))) SELECT 1 FROM _advisory_lock`,`staff-competency-profile:${c.tenantId}:${c.companyId}:${row.staffId}`);
