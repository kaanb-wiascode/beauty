...
type Marker = {
  ...
  actorUserId: string;
  ruleKey: 'LEAD_FIRST_TOUCH' | 'OPPORTUNITY_STAGE_FOLLOW_UP';
};
...
export class CrmAutomationMessageActionService {
  ...
  async process(scope: CrmAutomationScope) {
    ...
    for (const marker of markers) {
      ...
      if (rule.config.messageEnabled !== true) {
        await this.mark(marker, rowScope, 'DISABLED');
        skipped++;
        continue;
      }
      ...
      if (!['WHATSAPP', 'SMS', 'EMAIL'].includes(channel) || !body) {
        await this.mark(marker, rowScope, 'INVALID_CONFIG');
        skipped++;
        continue;
      }
      const consent = await this.compliance.canSendAutomation(rowScope, marker, channel);
      if (!consent.allowed) {
        await this.mark(marker, rowScope, consent.status === 'OPTED_OUT' ? 'CONSENT_OPTED_OUT' : 'CONSENT_REQUIRED');
        skipped++;
        continue;
      }
      const recipient = await this.recipient(marker, rowScope, channel);
      if (!recipient) {
        await this.mark(marker, rowScope, 'NO_RECIPIENT');
        skipped++;
        continue;
      }
  ...
  private async deliver(
    ...
    if (!messageId) {
      await this.mark(marker, scope, 'MESSAGE_CREATE_FAILED');
      return 'FAILED' as const;
    }
    const provider = this.providers.resolve(channel);
    if (!provider) {
      await this.failMessage(messageId, 'No configured provider is available.');
      await this.mark(marker, scope, 'PROVIDER_UNAVAILABLE', messageId);
      return 'FAILED' as const;
    }
    try {
      ...
      await this.mark(marker, scope, 'SENT', messageId);
      return 'SENT' as const;
    } catch (error) {
      const message = error instanceof Error ? error.message.slice(0, 1000) : 'Provider send failed.';
      await this.failMessage(messageId, message);
      await this.mark(marker, scope, 'FAILED', messageId);
      return 'FAILED' as const;
    }
  }

  private failMessage(id: string, error: string) {
    return this.prisma.$executeRawUnsafe(
      `UPDATE crm_messages SET status='FAILED',error_message=$2,version=version+1,updated_at=NOW() WHERE id=$1::text`,
      id,
      error,
    );
  }
  ...
  private mark(
    ...
    return this.prisma.$executeRawUnsafe(
      ...
      marker.actorUserId,
      JSON.stringify({
        messageSourceEventId: marker.id,
        automationKey: `AUTO_MESSAGE:${marker.id}`,
        rule: marker.ruleKey,
