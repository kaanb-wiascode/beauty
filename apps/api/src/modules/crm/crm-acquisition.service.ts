...
export class CrmAcquisitionService {
  ...
  async resolveRefs(input: AcquisitionInput, tx: DbClient = this.prisma): Promise<ResolvedAcquisitionRefs> {
    ...
    if (!requested.some((value) => value !== undefined && value !== null)) {
      return { acquisitionChannelId: input.acquisitionChannelId ?? null, acquisitionSourceId: input.acquisitionSourceId ?? null, acquisitionCampaignRefId: input.acquisitionCampaignRefId ?? null, acquisitionAdSetRefId: input.acquisitionAdSetRefId ?? null, acquisitionAdRefId: input.acquisitionAdRefId ?? null };
    }
    const rows = await tx.$queryRawUnsafe<Array<ResolvedAcquisitionRefs>>(`
      WITH selected AS (SELECT $3::uuid requested_channel_id,$4::uuid requested_source_id,$5::uuid requested_campaign_id,$6::uuid requested_ad_set_id,$7::uuid requested_ad_id), chain AS (
        SELECT COALESCE(ch.id,src_ch.id,cmp_ch.id,aset_ch.id,ad_ch.id) AS "acquisitionChannelId", COALESCE(src.id,cmp_src.id,aset_src.id,ad_src.id) AS "acquisitionSourceId", COALESCE(cmp.id,aset_cmp.id,ad_cmp.id) AS "acquisitionCampaignRefId",COALESCE(aset.id,ad_aset.id) AS "acquisitionAdSetRefId",ad.id AS "acquisitionAdRefId",s.*
        FROM selected s
      ...
      WHERE (requested_channel_id IS NULL OR "acquisitionChannelId"=requested_channel_id) AND (requested_source_id IS NULL OR "acquisitionSourceId"=requested_source_id) AND (requested_campaign_id IS NULL OR "acquisitionCampaignRefId"=requested_campaign_id) AND (requested_ad_set_id IS NULL OR "acquisitionAdSetRefId"=requested_ad_set_id) AND (requested_ad_id IS NULL OR "acquisitionAdRefId"=requested_ad_id) LIMIT 1`,c.tenantId,c.companyId,input.acquisitionChannelId??null,input.acquisitionSourceId??null,input.acquisitionCampaignRefId??null,input.acquisitionAdSetRefId??null,input.acquisitionAdRefId??null);
  ...
  async persistLeadRefs(leadId:string,input:AcquisitionInput,expectedVersion?:number):Promise<ResolvedAcquisitionRefs> {
    ...
    return this.prisma.$transaction(async tx=>{
      ...
      const merged={ acquisitionChannelId: input.acquisitionChannelId===undefined?current.acquisitionChannelId:input.acquisitionChannelId, acquisitionSourceId: input.acquisitionSourceId===undefined?current.acquisitionSourceId:input.acquisitionSourceId, acquisitionCampaignRefId: input.acquisitionCampaignRefId===undefined?current.acquisitionCampaignRefId:input.acquisitionCampaignRefId, acquisitionAdSetRefId: input.acquisitionAdSetRefId===undefined?current.acquisitionAdSetRefId:input.acquisitionAdSetRefId, acquisitionAdRefId: input.acquisitionAdRefId===undefined?current.acquisitionAdRefId:input.acquisitionAdRefId };
