...
export class CrmLeadMergeService {
  ...
  async merge(sourceLeadId:string,targetLeadId:string,actorUserId:string,reason:string){
    ...
    return this.prisma.$transaction(async tx=>{
      ...
      if(sourceOpportunity){
        await tx.$executeRawUnsafe(`UPDATE crm_opportunities SET lead_id=$2::text,version=version+1,updated_at=NOW() WHERE id=$1::text`,sourceOpportunity.id,targetLeadId);
      }
      ...
      const updated=await tx.$queryRawUnsafe<Array<{id:string;mergedIntoLeadId:string;mergedAt:Date}>>(
        `UPDATE crm_leads SET merged_into_lead_id=$2::text,merged_at=NOW(),merged_by_user_id=$3::text,version=version+1,updated_at=NOW()
         WHERE id=$1::text AND tenant_id=$4::text AND company_id=$5::text AND branch_id=$6::text AND merged_into_lead_id IS NULL
         RETURNING id,merged_into_lead_id AS "mergedIntoLeadId",merged_at AS "mergedAt"`,sourceLeadId,targetLeadId,actorUserId,c.tenantId,c.companyId,c.branchId);
      if(!updated[0]) throw new ConflictException('Source lead changed before merge could complete.');

      const metadata=JSON.stringify({sourceLeadId,targetLeadId,reason:reason.trim().slice(0,1000),movedOpportunityId:sourceOpportunity?.id??null});
      ...
      return {...updated[0],targetLeadId,movedOpportunityId:sourceOpportunity?.id??null};
    });
  }
}
