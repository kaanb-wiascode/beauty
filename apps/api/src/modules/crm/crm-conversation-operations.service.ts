      ...
      if (!current[0]) {
        ...
        const rows = await tx.$queryRawUnsafe<Array<{ id: string; version: number }>>(
          `INSERT INTO crm_conversation_sla_policies(
           ...
           ) VALUES($1::text,$2::text,$3::text,$4,$5,$6,$7,$8::text,$8::text)
           RETURNING id,version`,
          scope.tenantId, scope.companyId, scope.branchId,
          input.whatsappTargetMinutes, input.smsTargetMinutes, input.emailTargetMinutes,
          input.criticalAfterMinutes, actorUserId,
        );
      ...
      if (!current[0]) {
        ...
        const rows = await tx.$queryRawUnsafe<Array<{ id: string; version: number }>>(
          `INSERT INTO crm_conversation_assignments(
           ...
           RETURNING id,version`,
          scope.tenantId, scope.companyId, scope.branchId,
          subject.customerId, subject.leadId, subject.opportunityId, assignedUserId, actorUserId,
        );
