...
export class CrmLostReasonController{
 ...
 @Post('leads/:id/lost') @RequirePermission('crm','manage') loseLead(@Param('id') id:string,@Body() body:unknown,@Req() req:{user?:{sub?:string}}){return this.transitions.loseLead(uuid.parse(id),loseSchema.parse(body),this.actor(req));}
 @Post('opportunities/:id/lost') @RequirePermission('crm','manage') loseOpportunity(@Param('id') id:string,@Body() body:unknown,@Req() req:{user?:{sub?:string}}){this.actor(req);return this.transitions.loseOpportunity(uuid.parse(id),loseSchema.parse(body));}
}
