...
const loseSchema=z.object({version:z.coerce.number().int().min(1),lostReasonId:uuid,lostReasonNote:z.string().trim().max(2000).nullable().optional()});
...
export class CrmLostReasonController{
 ...
 @Patch('lost-reasons/:id/active') @RequirePermission('crm','manage') setActive(@Param('id') id:string,@Body() b:unknown){return this.reasons.setActive(uuid.parse(id),activeSchema.parse(b).isActive);}
