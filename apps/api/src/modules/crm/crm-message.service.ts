...
export class CrmMessageService {
  ...
  list(input: MessageSubject & { limit?: number }) {
    ...
    return this.prisma.$queryRawUnsafe<MessageRow[]>(
      `SELECT id,
              customer_id AS "customerId",lead_id AS "leadId",opportunity_id AS "opportunityId",
  ...
  async logManual(
    ...
    const rows = await this.prisma.$queryRawUnsafe<MessageRow[]>(
      `INSERT INTO crm_messages(
         tenant_id,company_id,branch_id,customer_id,lead_id,opportunity_id,
         direction,channel,status,provider_key,recipient,subject,body,created_by_user_id,sent_at,delivered_at
  ...
  async createDraft(
    ...
    try {
      const rows = await this.prisma.$queryRawUnsafe<MessageRow[]>(
        `INSERT INTO crm_messages(
           tenant_id,company_id,branch_id,customer_id,lead_id,opportunity_id,
           direction,channel,status,provider_key,recipient,subject,body,idempotency_key,created_by_user_id
  ...
  async send(id: string, expectedVersion: number) {
    ...
    if (!provider) {
      throw new ServiceUnavailableException(
        `No configured provider is available for ${existing.channel}.`,
      );
    }
    ...
    const claimed = await this.prisma.$queryRawUnsafe<MessageRow[]>(
      `UPDATE crm_messages SET status='QUEUED',provider_key=$6,version=version+1,error_message=NULL,updated_at=NOW()
       WHERE id=$1::text AND tenant_id=$2::text AND company_id=$3::text AND branch_id=$4::text
         AND version=$5::int AND status IN ('DRAFT','FAILED')
       RETURNING id,customer_id AS "customerId",lead_id AS "leadId",opportunity_id AS "opportunityId",
         direction,channel,status,provider_key AS "providerKey",recipient,subject,body,
         idempotency_key AS "idempotencyKey",external_message_id AS "externalMessageId",
         error_message AS "errorMessage",version,created_by_user_id AS "createdByUserId",
    ...
    try {
      ...
      const rows = await this.prisma.$queryRawUnsafe<MessageRow[]>(
        `UPDATE crm_messages SET status=$5,external_message_id=$6,
      ...
      await this.prisma.$executeRawUnsafe(
        `UPDATE crm_messages SET status='FAILED',error_message=$5,version=version+1,updated_at=NOW()
         WHERE id=$1::text AND tenant_id=$2::text AND company_id=$3::text AND branch_id=$4::text`,
        id,
        scope.tenantId,
        scope.companyId,
  ...
  async get(id: string) {
    ...
    const rows = await this.prisma.$queryRawUnsafe<MessageRow[]>(
      `SELECT id,customer_id AS "customerId",lead_id AS "leadId",opportunity_id AS "opportunityId",
              ...
              error_message AS "errorMessage",version,created_by_user_id AS "createdByUserId",
              sent_at AS "sentAt",delivered_at AS "deliveredAt",created_at AS "createdAt",updated_at AS "updatedAt"
       FROM crm_messages WHERE id=$1::text AND tenant_id=$2::text AND company_id=$3::text AND branch_id=$4::text LIMIT 1`,
      id,
      scope.tenantId,
      scope.companyId,
      scope.branchId,
  ...
  private async findByIdempotency(
    ...
    const rows = await this.prisma.$queryRawUnsafe<MessageRow[]>(
      `SELECT id,customer_id AS "customerId",lead_id AS "leadId",opportunity_id AS "opportunityId",
              ...
              error_message AS "errorMessage",version,created_by_user_id AS "createdByUserId",
              sent_at AS "sentAt",delivered_at AS "deliveredAt",created_at AS "createdAt",updated_at AS "updatedAt"
       FROM crm_messages WHERE tenant_id=$1::text AND company_id=$2::text AND branch_id=$3::text AND idempotency_key=$4 LIMIT 1`,
      scope.tenantId,
      scope.companyId,
      scope.branchId,
      idempotencyKey,
  ...
  private async resolveRecipient(subject: MessageSubject, channel: CrmMessageChannel) {
    ...
    if (!subject.customerId && !subject.leadId && !subject.opportunityId) {
      throw new BadRequestException('A customer, lead, or opportunity subject is required.');
    }
    const rows = await this.prisma.$queryRawUnsafe<Array<{ phone: string | null; email: string | null }>>(
      `SELECT COALESCE(c.phone,l.phone,oc.phone,ol.phone) AS phone,
              COALESCE(c.email,l.email,oc.email,ol.email) AS email
       FROM (SELECT 1) seed
       ...
       LEFT JOIN crm_leads ol ON ol.id=o.lead_id AND ol.tenant_id=$1::text AND ol.company_id=$2::text AND ol.branch_id=$3::text
       LIMIT 1`,
      scope.tenantId,
      scope.companyId,
      scope.branchId,
