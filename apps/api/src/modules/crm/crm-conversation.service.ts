...
export class CrmConversationService {
  ...
  list(actorUserId: string, limit = 100, mode: ConversationMode = 'ALL') {
    ...
    return this.prisma.$queryRawUnsafe(
      `WITH scoped AS (
         SELECT m.*,
                CASE WHEN m.customer_id IS NOT NULL THEN 'CUSTOMER' WHEN m.opportunity_id IS NOT NULL THEN 'OPPORTUNITY' ELSE 'LEAD' END AS subject_type,
         ...
         SELECT subject_type,subject_id,COUNT(*)::int AS message_count,
                ...
                MAX(created_at) AS last_message_at,MAX(created_at) FILTER(WHERE direction='INBOUND') AS last_inbound_at,
         ...
         SELECT DISTINCT ON(subject_type,subject_id) subject_type,subject_id,id AS last_message_id,direction AS last_direction,
                channel AS last_channel,status AS last_status,body AS last_body,subject AS last_subject,recipient AS last_recipient
       ...
       LEFT JOIN crm_conversation_assignments a ON a.tenant_id=$1::text AND a.company_id=$2::text AND a.branch_id=$3::text
        AND ((g.subject_type='CUSTOMER' AND a.customer_id=g.subject_id) OR (g.subject_type='LEAD' AND a.lead_id=g.subject_id) OR (g.subject_type='OPPORTUNITY' AND a.opportunity_id=g.subject_id))
       ...
       LEFT JOIN crm_conversation_states s ON s.tenant_id=$1::text AND s.company_id=$2::text AND s.branch_id=$3::text
        AND ((g.subject_type='CUSTOMER' AND s.customer_id=g.subject_id) OR (g.subject_type='LEAD' AND s.lead_id=g.subject_id) OR (g.subject_type='OPPORTUNITY' AND s.opportunity_id=g.subject_id))
       LEFT JOIN LATERAL (
  ...
  async detail(subjectType: SubjectType, subjectId: string, actorUserId: string, limit = 200) {
    const scope = this.scope(); await this.assertSubject(scope, subjectType, subjectId);
    const safeLimit = Math.min(Math.max(limit, 1), 300); const column = this.subjectColumn(subjectType);
    const messages = await this.prisma.$queryRawUnsafe(
      `SELECT id,direction,channel,status,provider_key AS "providerKey",recipient,subject,body,
       ...
       FROM crm_messages WHERE tenant_id=$1::text AND company_id=$2::text AND branch_id=$3::text AND ${column}=$4::text
       ORDER BY created_at ASC,id ASC LIMIT $5`, scope.tenantId, scope.companyId, scope.branchId, subjectId, safeLimit);
  ...
  private markReadInternal(scope: Scope, subjectType: SubjectType, subjectId: string, actorUserId: string) {
    ...
    return this.prisma.$executeRawUnsafe(
      `INSERT INTO crm_conversation_reads(tenant_id,company_id,branch_id,user_id,${column},read_at)
       ...
       DO UPDATE SET read_at=NOW(),updated_at=NOW()`, scope.tenantId, scope.companyId, scope.branchId, actorUserId, subjectId);
  }
