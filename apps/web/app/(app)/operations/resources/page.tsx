            ...
            <div className="divide-y divide-[var(--line)]">
              {rooms.map((room) => (
                <div key={room.id} className="grid gap-3 px-6 py-4 sm:grid-cols-[minmax(0,1fr)_150px_90px] sm:items-center">
                  <div><p className="text-sm font-semibold text-[var(--ink)]">{room.name}</p><p className="mt-1 text-xs text-[var(--muted)]">{room.code} · {userLabel(room.roomType)} · kapasite {room.capacity}</p></div>
