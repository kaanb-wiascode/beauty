  ...
  return (
    <div className="mx-auto max-w-[1420px] space-y-5 pb-10">
      <header className="rounded-[24px] border border-[var(--line)] bg-[var(--surface)] p-6 shadow-sm">
        ...
        <p className="mt-2 max-w-3xl text-sm text-[var(--muted)]">Yaklaşan randevular için açıklanabilir no-show ve gecikme sinyalleri üretir. Scheduling, güvenlik, yetkinlik ve izin kuralları üzerinde karar vermez; deterministik kurallar otorite olmaya devam eder.</p>
      </header>
      ...
      {data ? (
        <>
          <section className="grid gap-4 md:grid-cols-3">
            ...
            <div className="rounded-[20px] border border-[var(--line)] bg-[var(--surface)] p-5"><p className="text-xs text-[var(--muted)]">Model</p><p className="mt-2 text-sm font-semibold text-[var(--ink)]">{data.model}</p></div>
          </section>

          <section className="rounded-[24px] border border-[var(--line)] bg-[var(--surface)] p-6 shadow-sm">
            <h2 className="text-sm font-semibold text-[var(--ink)]">Manager Insights</h2>
          ...
          <section className="overflow-hidden rounded-[24px] border border-[var(--line)] bg-[var(--surface)] shadow-sm">
            ...
            {data.appointments.length ? <div className="divide-y divide-[var(--line)]">{data.appointments.map((item) => (
              <div key={item.appointmentId} className="grid gap-4 px-6 py-4 lg:grid-cols-[1.2fr_1fr_1fr]">
                <div><p className="text-sm font-semibold text-[var(--ink)]">{item.customerName} · {item.serviceName}</p><p className="mt-1 text-xs text-[var(--muted)]">{new Date(item.startAt).toLocaleString("tr-TR")} · {item.staffName} · Onay: {item.confirmationStatus ?? "Yok"}</p></div>
