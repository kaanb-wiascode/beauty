...
const selectValues: Record<string, string[]> = { status: ["PENDING","APPROVED","PAID","PRESENT","ABSENT","DRAFT"], type:["ANNUAL","SICK","EXCUSE","UNPAID","OTHER"], method:["BANK","CASH"] };
function display(v:unknown){ if(v===null||v===undefined||v==="") return "—"; if(typeof v==="string" && v.includes("T")) return new Date(v).toLocaleString("tr-TR"); return String(v); }
function toRows(value: unknown): Row[] { if(Array.isArray(value)) return value.filter((item): item is Row => typeof item === "object" && item !== null && !Array.isArray(item)); if(typeof value === "object" && value !== null && "data" in value && Array.isArray(value.data)) return value.data.filter((item): item is Row => typeof item === "object" && item !== null && !Array.isArray(item)); return []; }
function toStaffRows(value: unknown): StaffRow[] { return toRows(value).filter((item): item is StaffRow => typeof item.id === "string" && typeof item.firstName === "string" && typeof item.lastName === "string"); }
...
export default function HRSection(){
  ...
  const load=async()=>{ if(!c)return; setLoading(true); setError(""); try{
    ...
    const r=await api<unknown>(needsPeriod?withQuery(c.get,{year,month}):c.get);
    setRows(toRows(r));
    if(section!=="employees"&&section!=="personnel-files"){
      const s=await api<unknown>("/hr/employees"); setStaff(toStaffRows(s));
    } else setStaff(toStaffRows(r));
  ...
  const normalize=(raw:FormState)=>{const b={...raw}; if(section==="employees"){
      const profile: Record<string, unknown>={}; for(const k of ["personnelNumber","identityNumber","position","department","employmentType","hireDate","salary","iban","bankName"]){if(b[k]!==undefined&&b[k]!=="")profile[k]=numberFields.has(k)?Number(b[k]):b[k];}
