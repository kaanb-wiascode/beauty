...
const SESSION_STATUS_LABELS:Record<TrainingSession["status"],string>={SCHEDULED:"Planlandı",IN_PROGRESS:"Devam Ediyor",COMPLETED:"Tamamlandı",CANCELLED:"İptal Edildi"};
const SOURCE_LABELS:Record<string,string>={MANUAL:"Manuel",QUALITY_RULE:"Kalite Kuralı",COMPETENCY_GAP:"Yetkinlik Açığı",DEVELOPMENT_PLAN:"Gelişim Planı"};
const DELIVERY_LABELS:Record<string,string>={ONLINE:"Çevrim İçi",CLASSROOM:"Sınıf Eğitimi",BLENDED:"Karma Eğitim",SELF_PACED:"Bireysel Eğitim",IN_PERSON:"Yüz Yüze"};
...
export default function TrainingOperationsPage(){
  ...
  if(loading)return <div className="flex min-h-[420px] items-center justify-center rounded-[22px] border border-[var(--line)] bg-[var(--surface)]"><Spinner label="Eğitim Yönetimi Hazırlanıyor..."/></div>;

  return <div className="mx-auto max-w-[1440px] space-y-6 pb-10">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div><p className="text-[11px] font-semibold uppercase tracking-[0.15em] text-[var(--muted-soft)]">Eğitim Ve Yetkinlik</p><h1 className="mt-1 text-[32px] font-semibold tracking-[-0.045em] text-[var(--ink)]">Eğitim Yönetimi</h1><p className="mt-2 max-w-[820px] text-[13px] leading-6 text-[var(--muted)]">Eğitim Atamalarını, Yaklaşan Oturumları, Gelişim Planlarını Ve Yetkinlik Değerlendirmelerini Tek Ekrandan İzleyin.</p></div>
      <div className="flex flex-wrap gap-2">
        ...
        {canManage?<><Button variant="secondary" disabled={action!==null} onClick={()=>void run("expired",()=>api("/training/assignments/process-expired",{method:"POST",body:{limit:100}}),"Süresi Geçen Eğitim Atamaları İşlendi.")}>{action==="expired"?"Taranıyor...":"Süresi Geçenleri Kontrol Et"}</Button><Button variant="secondary" disabled={action!==null} onClick={()=>void run("reviews",()=>api("/training/competency-reviews/schedules/process-due",{method:"POST",body:{limit:100}}),"Vadesi Gelen Yetkinlik Değerlendirmeleri Açıldı.")}>{action==="reviews"?"Açılıyor...":"Değerlendirmeleri Başlat"}</Button><Button disabled={action!==null} onClick={()=>void run("quality-rules",()=>api("/training/quality-rules/process",{method:"POST",body:{limit:100}}),"Kalite Kaynaklı Eğitim Kuralları İşlendi.")}>{action==="quality-rules"?"İşleniyor...":"Eğitim Kurallarını Çalıştır"}</Button></>:null}
      </div>
    </header>
    {error?<Alert onClose={()=>setError("")}>{error}</Alert>:null}{success?<Alert tone="success" onClose={()=>setSuccess("")}>{success}</Alert>:null}
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><FinanceMetric label="Aktif Atama" value={openAssignments.length} detail={`${dueSoon} Atama 7 Gün İçinde Tamamlanmalı`} tone={dueSoon?"warning":"info"}/><FinanceMetric label="Yaklaşan Oturum" value={upcomingSessions.length} detail="Önümüzdeki 30 Gün" tone="info"/><FinanceMetric label="Açık Yetkinlik Değerlendirmesi" value={openReviews.length} detail={`${overdueReviews.length} Değerlendirme Gecikmiş`} tone={overdueReviews.length?"danger":"neutral"}/><FinanceMetric label="Süresi Dolan Eğitim" value={expiredAssignments.length} detail="Takip Gerektiren Atamalar" tone={expiredAssignments.length?"danger":"success"}/></section>

    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(360px,0.85fr)]">
      <FinancePanel title="Eğitim Atamaları" description="Manuel, Kalite Kuralı Ve Yetkinlik Açığı Kaynaklı Son Atamalar.">{assignments.length?<DataView><div className="divide-y divide-[var(--line)]">{assignments.slice(0,20).map(item=><article key={item.id} className="grid gap-3 px-4 py-4 sm:px-5 lg:grid-cols-[minmax(0,1fr)_150px_130px] lg:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${assignmentTone(item.status)}`}>{STATUS_LABELS[item.status]}</span><span className="rounded-full bg-[var(--surface-2)] px-2.5 py-1 text-[10px] font-medium text-[var(--muted)]">{SOURCE_LABELS[item.sourceType]??"Sistem Ataması"}</span></div><h3 className="mt-2 truncate text-[13px] font-semibold text-[var(--ink)]">{item.courseCode} · {item.courseTitle}</h3><p className="mt-1 truncate text-[10px] text-[var(--muted-soft)]">{item.staffName||"Şube Kapsamlı Atama"} · {item.branchName||"Seçili Şube"}</p></div><div><p className="text-[9px] font-semibold uppercase tracking-[0.12em] text-[var(--muted-soft)]">Atanma</p><p className="mt-1 text-[11px] font-medium text-[var(--ink)]">{formatDate(item.assignedAt)}</p></div><div><p className="text-[9px] font-semibold uppercase tracking-[0.12em] text-[var(--muted-soft)]">Son Tarih</p><p className="mt-1 text-[11px] font-medium text-[var(--ink)]">{formatDate(item.dueAt)}</p></div></article>)}</div></DataView>:<FinanceEmpty title="Eğitim Ataması Yok" description="Atamalar Oluşturulduğunda Burada Görünecek."/>}</FinancePanel>
      <FinancePanel title="Yaklaşan Eğitim Takvimi" description="Planlanan Ve Devam Eden Eğitim Oturumları.">{upcomingSessions.length?<div className="space-y-3">{upcomingSessions.slice(0,8).map(session=><div key={session.id} className="rounded-[18px] border border-[var(--line)] bg-[var(--surface)] p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-[12px] font-semibold text-[var(--ink)]">{session.title}</p><p className="mt-1 truncate text-[10px] text-[var(--muted)]">{session.courseCode} · {session.courseTitle}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-[9px] font-semibold ${sessionTone(session.status)}`}>{SESSION_STATUS_LABELS[session.status]}</span></div><div className="mt-3 flex items-center justify-between text-[10px] text-[var(--muted-soft)]"><span>{formatDate(session.startsAt)}</span><span>{session.enrolledCount}/{session.capacity??"∞"} Katılımcı</span></div>{session.location?<p className="mt-2 truncate text-[10px] text-[var(--muted)]">{session.location}</p>:null}</div>)}</div>:<FinanceEmpty title="Yaklaşan Oturum Yok" description="Önümüzdeki 30 Gün İçin Planlanmış Eğitim Bulunmuyor."/>}</FinancePanel>
    </div>

    <div className="grid gap-6 xl:grid-cols-2">
      <FinancePanel title="Yetkinlik Değerlendirmeleri" description="Düzenli Değerlendirme Planlarının Açtığı Personel Değerlendirmeleri.">{openReviews.length?<div className="space-y-3">{openReviews.slice(0,10).map(review=>{const overdue=new Date(review.dueAt).getTime()<referenceTime;return <div key={review.id} className="rounded-[18px] border border-[var(--line)] bg-[var(--surface)] p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-[12px] font-semibold text-[var(--ink)]">{review.profileCode} · {review.profileName}</p><p className="mt-1 truncate text-[10px] text-[var(--muted-soft)]">Personel Değerlendirmesi · Profil Sürümü {review.profileVersion}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-[9px] font-semibold ${overdue?"bg-[var(--danger-soft)] text-[var(--danger)]":"bg-[var(--warning-soft)] text-[var(--warning)]"}`}>{overdue?"Gecikmiş":"Açık"}</span></div><div className="mt-3 flex items-center justify-between text-[10px] text-[var(--muted-soft)]"><span>Açılış {formatDate(review.openedAt)}</span><span>Son Tarih {formatDate(review.dueAt)}</span></div></div>})}</div>:<FinanceEmpty title="Açık Değerlendirme Yok" description="Düzenli Değerlendirme Planları Çalıştığında Burada Görünür."/>}</FinancePanel>
      <FinancePanel title="Yetkinlik Yönetimi" description={`${activeDefinitions.length} Aktif Yetkinlik · ${activeProfiles.length} Aktif Profil · ${activeSchedules.length} Aktif Değerlendirme Planı`}>{activeProfiles.length?<div className="space-y-3">{activeProfiles.slice(0,10).map(profile=><div key={profile.id} className="rounded-[18px] border border-[var(--line)] bg-[var(--surface)] p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[var(--accent)]">{profile.code} · Sürüm {profile.version}</p><p className="mt-1 truncate text-[12px] font-semibold text-[var(--ink)]">{profile.name}</p></div><span className="shrink-0 rounded-full bg-[var(--surface-2)] px-2 py-1 text-[9px] font-medium text-[var(--muted)]">{profile.requirements.length} Kriter</span></div><p className="mt-3 text-[10px] text-[var(--muted-soft)]">Geçerlilik {formatDate(profile.effectiveFrom)}{profile.effectiveTo?` → ${formatDate(profile.effectiveTo)}`:" → Süresiz"}</p></div>)}</div>:<FinanceEmpty title="Aktif Yetkinlik Profili Yok" description="Yetkinlik Profilleri Oluşturulduğunda Burada Görünür."/>}</FinancePanel>
    </div>

    <div className="grid gap-6 xl:grid-cols-2">
      <FinancePanel title="Gelişim Planları" description="Personel Bazlı Gelişim Planlarının İlerleme Görünümü.">{plans.length?<div className="space-y-3">{plans.slice(0,10).map(plan=>{const percent=plan.itemCount>0?Math.round(plan.completedItemCount/plan.itemCount*100):0;return <div key={plan.id} className="rounded-[18px] border border-[var(--line)] bg-[var(--surface)] p-4"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate text-[12px] font-semibold text-[var(--ink)]">{plan.title}</p><p className="mt-1 text-[10px] text-[var(--muted-soft)]">{plan.completedItemCount}/{plan.itemCount} Adım Tamamlandı</p></div><span className="text-[12px] font-semibold text-[var(--ink)]">%{percent}</span></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[var(--surface-2)]"><div className="h-full rounded-full bg-[var(--accent)]" style={{width:`${Math.min(percent,100)}%`}}/></div></div>})}</div>:<FinanceEmpty title="Gelişim Planı Yok" description="Personel Gelişim Planları Burada Takip Edilecek."/>}</FinancePanel>
      <FinancePanel title="Eğitim Kataloğu" description="Aktif Eğitimler Ve Eğitim Türleri.">{courses.length?<div className="grid gap-3 sm:grid-cols-2">{courses.filter(course=>course.isActive).slice(0,12).map(course=><div key={course.id} className="rounded-[18px] border border-[var(--line)] bg-[var(--surface)] p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[var(--accent)]">{course.code}</p><p className="mt-1 line-clamp-2 text-[12px] font-semibold text-[var(--ink)]">{course.title}</p></div><span className="shrink-0 rounded-full bg-[var(--surface-2)] px-2 py-1 text-[9px] font-medium text-[var(--muted)]">{DELIVERY_LABELS[course.deliveryType]??"Eğitim"}</span></div><p className="mt-3 text-[9px] font-semibold uppercase tracking-[0.12em] text-[var(--muted-soft)]">{course.category}</p></div>)}</div>:<FinanceEmpty title="Aktif Eğitim Yok" description="Eğitim Kataloğu Burada Görünür."/>}</FinancePanel>
    </div>
  </div>;
}