...
export default function TrainingCourseAuthoringPage(){
  ...
  function archiveCourse(){if(!course||!confirm(`${course.code} kursu arşivlensin mi? Aktif taslak ve yayın sürümleri RETIRED durumuna taşınacak.`))return;return run("archive",()=>api(`/training/courses/${course.id}/archive`,{method:"POST"}),"Kurs arşivlendi ve aktif sürümleri emekliye ayrıldı.");}
  ...
  async function uploadDocument(versionId:string,file:File){
    ...
    if(!response.ok)throw new Error("Doküman private storage alanına yüklenemedi.");
  ...
  function parseQuestionForm(form:QuestionForm){
    ...
    if(form.questionType==="TRUE_FALSE")correctAnswer=form.correctAnswer.toLowerCase()==="true";
  ...
  return <div className="mx-auto max-w-[1440px] space-y-6 pb-10">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div><Link href="/training/courses" className="text-[11px] font-semibold text-[var(--accent)] hover:underline">← Kurs Yönetimi</Link><p className="mt-3 text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--muted-soft)]">{course.code}</p><div className="mt-1 flex flex-wrap items-center gap-3"><h1 className="text-[32px] font-semibold tracking-[-0.045em] text-[var(--ink)]">{course.title}</h1><span className={`rounded-full px-2.5 py-1 text-[9px] font-semibold ${course.isActive?"bg-[var(--success-soft)] text-[var(--success)]":"bg-[var(--surface-2)] text-[var(--muted)]"}`}>{course.isActive?"Aktif":"Arşiv"}</span></div><p className="mt-2 max-w-[820px] text-[13px] leading-6 text-[var(--muted)]">Taslak metadata, ders materyalleri ve teori assessment yapısını hazırlayın; yayın geçmişini değiştirmeden yeni sürümü kontrollü biçimde yayınlayın.</p></div>
      <div className="flex flex-wrap gap-2"><Button variant="secondary" disabled={action!==null} onClick={()=>void load()}>Yenile</Button>{canManage&&course.isActive&&!draftVersion?<Button disabled={action!==null} onClick={()=>void createDraft()}>{action==="draft"?"Oluşturuluyor...":"Taslak Sürüm Oluştur"}</Button>:null}{canManage&&draft?<Button disabled={action!==null} onClick={()=>void publish()}>{action==="publish"?"Yayınlanıyor...":`v${draft.version} Yayınla`}</Button>:null}{canManage&&course.isActive?<Button variant="secondary" disabled={action!==null} onClick={()=>void archiveCourse()}>{action==="archive"?"Arşivleniyor...":"Kursu Arşivle"}</Button>:canManage?<Button disabled={action!==null} onClick={()=>void restoreCourse()}>{action==="restore"?"Aktifleştiriliyor...":"Kursu Aktifleştir"}</Button>:null}</div>
    </header>
    {error?<Alert onClose={()=>setError("")}>{error}</Alert>:null}{success?<Alert tone="success" onClose={()=>setSuccess("")}>{success}</Alert>:null}
    {!course.isActive?<Alert>Bu kurs arşivde. Aktif DRAFT/PUBLISHED sürümleri RETIRED durumuna alınmıştır; yeni taslak oluşturmak için kursu yeniden aktifleştirin.</Alert>:null}

    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><FinanceMetric label="Sürüm" value={draft?`v${draft.version}`:published?`v${published.version}`:"—"} detail={draft?"Taslak Düzenleniyor":published?"Yayındaki Sürüm":course.isActive?"Sürüm Yok":"Arşiv"} tone={draft?"warning":published?"success":"neutral"}/><FinanceMetric label="Ders" value={draft?.lessons.length??0} detail="Taslak İçerik" tone="info"/><FinanceMetric label="Sınav" value={draft?.exams.length??0} detail="Teori Assessment" tone="neutral"/><FinanceMetric label="Soru" value={draft?.exams.reduce((sum,item)=>sum+item.questions.length,0)??0} detail="Toplam Assessment Sorusu" tone="neutral"/></section>

    {!draft?<FinancePanel title="Taslak Bulunmuyor" description="Yayınlanan ve emekliye ayrılan sürümler immutable tutulur."><FinanceEmpty title={!course.isActive?"Kurs Arşivde":published?`v${published.version} Yayında`:"Henüz Yayın Yok"} description={!course.isActive?"Kursu yeniden aktifleştirerek yeni bir taslak sürüm açabilirsiniz.":"Yeni değişiklik yapmak için taslak sürüm oluşturun."}/></FinancePanel>:
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <FinancePanel title="Teori Assessment" description="Sınavları ve soruları düzenleyin; soru sırası yayın öncesinde değiştirilebilir.">
