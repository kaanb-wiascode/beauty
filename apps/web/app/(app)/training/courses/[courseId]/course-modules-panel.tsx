...
export function CourseModulesPanel({versionId,lessons,disabled=false}:Props){
  ...
  const lessonModule=useMemo(()=>{
    ...
    for(const module of modules)for(const lessonId of module.lessonIds??[])mapping.set(lessonId,module.id);
    return mapping;
  },[modules]);
  ...
  function edit(module:CourseModule){setEditingId(module.id);setTitle(module.title);setDescription(module.description??"");}
  function cancelEdit(){setEditingId(null);setTitle("");setDescription("");}

  function remove(module:CourseModule){
    if(!confirm(`“${module.title}” modülü silinsin mi? İçindeki dersler silinmez, modülsüz duruma taşınır.`))return;
    void run(`delete:${module.id}`,()=>api(`/training/authoring/modules/${module.id}`,{method:"DELETE"}));
  }

  function move(module:CourseModule,direction:-1|1){
    const index=modules.findIndex(item=>item.id===module.id);const target=index+direction;
    if(target<0||target>=modules.length)return;
    const ids=modules.map(item=>item.id);[ids[index],ids[target]]=[ids[target],ids[index]];
    void run(`move:${module.id}`,()=>api(`/training/authoring/versions/${versionId}/modules/reorder`,{method:"POST",body:{moduleIds:ids}}));
  }
  ...
  return <FinancePanel title="Modül Yapısı" description="Dersleri modüller altında gruplayın. Modüller yalnızca aktif taslak sürümde değiştirilebilir.">
    <div className="space-y-4">
      ...
      {modules.length?<div className="grid gap-3 md:grid-cols-2">{modules.map((module,index)=><div key={module.id} className="rounded-[16px] border border-[var(--line)] p-4">
        <div className="flex items-start justify-between gap-3"><div><p className="text-[12px] font-semibold text-[var(--ink)]">{module.sequence}. {module.title}</p>{module.description?<p className="mt-1 text-[10px] leading-5 text-[var(--muted)]">{module.description}</p>:null}<p className="mt-2 text-[9px] uppercase tracking-[0.12em] text-[var(--muted-soft)]">{module.lessonIds?.length??0} ders</p></div><div className="flex flex-wrap justify-end gap-1"><Button variant="secondary" disabled={disabled||busy!==null||index===0} onClick={()=>move(module,-1)}>↑</Button><Button variant="secondary" disabled={disabled||busy!==null||index===modules.length-1} onClick={()=>move(module,1)}>↓</Button><Button variant="secondary" disabled={disabled||busy!==null} onClick={()=>edit(module)}>Düzenle</Button><Button variant="secondary" disabled={disabled||busy!==null} onClick={()=>remove(module)}>Sil</Button></div></div>
