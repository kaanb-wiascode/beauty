...
export default function LearningPathsPage() {
  ...
  const load = useCallback(async () => {
    ...
    catch (e) { setError(e instanceof ApiError ? userErrorMessage(e.message, "Eğitim yolları yüklenemedi.") : "Eğitim yolları yüklenemedi."); }
  ...
  async function create() {
    ...
    try {
      ...
      await load();
    } catch (e) { setError(e instanceof ApiError ? userErrorMessage(e.message, "Eğitim yolu oluşturulamadı.") : "Eğitim yolu oluşturulamadı."); }
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-6 pb-10">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <Link href="/training" className="text-[11px] font-semibold text-[var(--accent)] hover:underline">← Eğitim yönetimi</Link>
          <p className="mt-4 text-[11px] font-semibold uppercase tracking-[0.15em] text-[var(--muted-soft)]">Eğitim yolları ve akademiler</p>
          <h1 className="mt-1 text-[32px] font-semibold tracking-[-0.045em] text-[var(--ink)]">Akademiler ve eğitim yolları</h1>
          <p className="mt-2 max-w-[900px] text-[13px] leading-6 text-[var(--muted)]">Eğitimleri belirli bir sıraya göre düzenleyin, taslak ve yayındaki sürümleri ayrı yönetin, ön koşullara bağlı gelişim yolları oluşturun.</p>
        </div>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <FinanceMetric label="Eğitim yolu" value={metrics.total} detail="Tanımlı eğitim yolu" tone="info" />
        <FinanceMetric label="Yayında" value={metrics.published} detail="Yayında olan sürüm" tone="success" />
        <FinanceMetric label="Taslak" value={metrics.draft} detail="Düzenlenebilir sürüm" tone="warning" />
        <FinanceMetric label="İçerik Bekliyor" value={metrics.empty} detail="Henüz sürüm oluşturulmamış" tone={metrics.empty ? "warning" : "success"} />
      </section>

      {canManage ? (
        <FinancePanel title="Yeni eğitim yolu" description="Rol akademisi, kariyer yolu veya zorunlu eğitim programı için temel yapıyı ve ilk taslak sürümü birlikte oluşturur.">
      ...
      <FinancePanel title="Eğitim yolları" description="Yayındaki sürüm personele atanabilir; taslak sürüm içerik ve ön koşul düzenlemeleri için kullanılır.">
        {loading ? <div className="flex min-h-[260px] items-center justify-center"><Spinner label="Eğitim yolları yükleniyor..." /></div> : !rows.length ? <FinanceEmpty title="Eğitim yolu bulunamadı" description="İlk rol akademisini veya gelişim programını oluşturun." /> : (
            ...
            {rows.map((row) => (
              <Link key={row.id} href={`/training/learning-paths/${row.id}`} className="rounded-[18px] border border-[var(--line)] bg-[var(--surface-2)] p-4 transition hover:border-[var(--accent)]/40">
                ...
                <div className="mt-4 flex flex-wrap gap-2 text-[10px] text-[var(--muted-soft)]">
                  <span>Yayındaki sürüm: {row.publishedItemCount} adım</span><span>•</span><span>Taslak: {row.draftVersionId ? `Sürüm ${row.draftVersion} · ${row.draftItemCount} adım` : "yok"}</span>
                </div>
              </Link>
            ))}
          </div>
