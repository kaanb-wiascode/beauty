...
export default function TrainingLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="space-y-5">
      <nav aria-label="Eğitim ve gelişim navigasyonu" className="overflow-x-auto rounded-[18px] border border-[var(--line)] bg-[var(--surface)] p-1.5">
        <div className="flex min-w-max gap-1">
          {visibleItems.map((item) => {
            const active = isActive(pathname, item.href);
            return <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={cx("rounded-[13px] px-3.5 py-2 text-[12px] font-medium transition-colors",active?"bg-[var(--accent-soft)] text-[var(--accent)]":"text-[var(--muted)] hover:bg-[var(--surface-2)] hover:text-[var(--ink)]")}>{item.label}</Link>;
          })}
        </div>
      </nav>
      {children}
