...
const TYPE_LABELS:Record<ItemType,string>={COMPETENCY:"Yetkinlik",COURSE:"Kurs",PROGRAM:"Learning Path",ACTION:"Aksiyon",COACHING:"Koçluk",MENTORING:"Mentorluk",PROJECT:"Proje",STRETCH_ASSIGNMENT:"Stretch Assignment"};
...
export default function DevelopmentPlanDetailPage(){
  ...
  async function addItem(){
    ...
    else if(type==="PROGRAM"){if(!form.resourceId)return setError("Learning Path seçin.");payload.programId=form.resourceId;}
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6 pb-10">
    ...
    {canManage&&!closed?<FinancePanel title="Gelişim Maddesi Ekle" description="Kurs, Learning Path, yetkinlik veya saha gelişim aktivitesini aynı IDP içinde tanımlayın."><div className="grid gap-3 lg:grid-cols-2 xl:grid-cols-4">
      ...
      {form.itemType==="PROGRAM"?<label><span className="mb-1 block text-[10px] font-semibold uppercase text-[var(--muted-soft)]">Learning Path</span><Select value={form.resourceId} onChange={(e)=>setForm((v)=>({...v,resourceId:e.target.value}))}><option value="">Path seçin</option>{paths.filter((p)=>p.isActive&&p.publishedVersionId).map((p)=><option key={p.id} value={p.id}>{p.code} · {p.title}</option>)}</Select></label>:null}
