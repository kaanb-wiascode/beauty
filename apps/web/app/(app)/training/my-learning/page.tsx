...
type Reminder={id:string;assignmentId:string;reminderType:"UPCOMING"|"DUE_TODAY"|"OVERDUE";dueAt?:string|null;createdAt:string;courseCode:string;courseTitle:string};
...
const REMINDER:Record<Reminder["reminderType"],string>={UPCOMING:"Son Tarih Yaklaşıyor",DUE_TODAY:"Bugün Son Gün",OVERDUE:"Süresi Geçti"};
...
export default function MyLearningPage(){
  ...
  const load=useCallback(async()=>{
    ...
    try{const [assignments,alerts]=await Promise.all([api<Assignment[]>("/training/learner/me/assignments"),api<Reminder[]>("/training/learner/me/reminders")]);setItems(assignments);setReminders(alerts);}
  ...
  async function acknowledge(id:string){try{await api(`/training/learner/me/reminders/${id}/acknowledge`,{method:"POST"});setReminders(current=>current.filter(item=>item.id!==id));}catch(requestError){setError(errorMessage(requestError,"Hatırlatma kapatılamadı."));}}
  ...
  return <div className="space-y-6 pb-10">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><FinanceMetric label="Aktif Eğitim" value={active.length} detail="Devam Bekleyen" tone="info"/><FinanceMetric label="Tamamlanan" value={completed.length} detail="Geçmiş Atamalar" tone="success"/><FinanceMetric label="Geciken" value={overdue.length} detail="Son Tarihi Geçen" tone={overdue.length?"warning":"neutral"}/><FinanceMetric label="Hatırlatma" value={reminders.length} detail="Açık Uyarılar" tone={reminders.length?"warning":"neutral"}/></section>
    {reminders.length?<FinancePanel title="Hatırlatmalar" description="Yaklaşan veya süresi geçen eğitimlerinizi önceliklendirin."><div className="space-y-2">{reminders.map(reminder=><div key={reminder.id} className="flex flex-col gap-3 rounded-[16px] border border-[var(--line)] p-4 sm:flex-row sm:items-center sm:justify-between"><Link href={`/training/my-learning/${reminder.assignmentId}`} className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[var(--accent)]">{REMINDER[reminder.reminderType]}</p><p className="mt-1 truncate text-[13px] font-semibold text-[var(--ink)]">{reminder.courseCode} · {reminder.courseTitle}</p><p className="mt-1 text-[10px] text-[var(--muted)]">Son tarih {formatDate(reminder.dueAt)}</p></Link><Button variant="secondary" onClick={()=>void acknowledge(reminder.id)}>Kapat</Button></div>)}</div></FinancePanel>:null}
