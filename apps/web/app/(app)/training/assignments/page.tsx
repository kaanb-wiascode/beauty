...
export default function TrainingAssignmentsPage(){
  ...
  async function submit(event:FormEvent){
    ...
    try{
      ...
      setSuccess(`v${result.courseVersion} üzerinden ${result.created} atama oluşturuldu. ${result.duplicates?`${result.duplicates} tekrar atlandı.`:""}`.trim());
  ...
  return <div className="space-y-6 pb-10">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--muted-soft)]">Learning Operations</p><h1 className="mt-1 text-[32px] font-semibold tracking-[-0.045em] text-[var(--ink)]">Eğitim Atamaları</h1><p className="mt-2 max-w-[850px] text-[13px] leading-6 text-[var(--muted)]">Yayınlanmış kurs sürümlerini personele, şubeye veya pozisyona toplu atayın. Atamalar yayın sürümüne pinlenir; sonraki kurs yayınları mevcut learner history’yi değiştirmez.</p></div>{canManage?<Button variant="secondary" disabled={busy} onClick={()=>void generateReminders()}>Hatırlatmaları Üret</Button>:null}</header>
    {error?<Alert onClose={()=>setError("")}>{error}</Alert>:null}{success?<Alert tone="success" onClose={()=>setSuccess("")}>{success}</Alert>:null}
    <section className="grid gap-3 sm:grid-cols-3"><FinanceMetric label="Son Kayıtlar" value={assignments.length} detail="Listelenen Atama" tone="neutral"/><FinanceMetric label="Aktif" value={activeCount} detail="Assigned + In Progress" tone="info"/><FinanceMetric label="Süresi Geçmiş" value={overdueCount} detail="Henüz Expired İşlenmemiş" tone={overdueCount?"warning":"neutral"}/></section>
    ...
    {canManage?<FinancePanel title="Toplu Atama" description="Hedef seçimi aktif tenant/company/branch scope ile sınırlandırılır."><form onSubmit={submit} className="grid gap-4 lg:grid-cols-2">
      ...
      <div className="lg:col-span-2 flex items-center justify-between gap-3"><p className="text-[10px] text-[var(--muted-soft)]">Seçili hedef: {targetIds.length}. Aynı gönderim tekrar edilirse idempotency anahtarı duplicate kayıtları engeller.</p><Button type="submit" disabled={busy||!courseId||!targetIds.length}>{busy?"İşleniyor...":"Eğitimi Ata"}</Button></div>
    </form></FinancePanel>:<FinanceEmpty title="Salt Okunur Yetki" description="Toplu eğitim ataması için training.manage izni gerekir."/>}

    <FinancePanel title="Son Eğitim Atamaları" description="Personel, kurs, şube, durum ve deadline görünümü.">{assignments.length?<div className="overflow-x-auto"><table className="min-w-full text-left text-[11px]"><thead><tr className="border-b border-[var(--line)] text-[var(--muted-soft)]"><th className="px-3 py-2 font-medium">Personel</th><th className="px-3 py-2 font-medium">Kurs</th><th className="px-3 py-2 font-medium">Şube</th><th className="px-3 py-2 font-medium">Durum</th><th className="px-3 py-2 font-medium">Son Tarih</th></tr></thead><tbody>{assignments.map(item=><tr key={item.id} className="border-b border-[var(--line)] last:border-0"><td className="px-3 py-3 font-medium text-[var(--ink)]">{item.staffName??"Şube Geneli"}</td><td className="px-3 py-3 text-[var(--muted)]">{item.courseCode} · {item.courseTitle}</td><td className="px-3 py-3 text-[var(--muted)]">{item.branchName}</td><td className="px-3 py-3 text-[var(--muted)]">{item.status}</td><td className="px-3 py-3 text-[var(--muted)]">{item.dueAt?new Date(item.dueAt).toLocaleString("tr-TR"):"—"}</td></tr>)}</tbody></table></div>:<FinanceEmpty title="Atama Yok" description="Henüz eğitim ataması bulunmuyor."/>}</FinancePanel>
  </div>;
}