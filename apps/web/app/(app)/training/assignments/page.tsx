...
export default function TrainingAssignmentsPage(){
  ...
  return <div className="space-y-6 pb-10">
    ...
    <section className="grid gap-3 sm:grid-cols-3"><FinanceMetric label="Son Kayıtlar" value={assignments.length} detail="Listelenen Atama" tone="neutral"/><FinanceMetric label="Aktif" value={activeCount} detail="Atanmış veya devam eden" tone="info"/><FinanceMetric label="Süresi Geçmiş" value={overdueCount} detail="Süresi geçtiği halde henüz kapatılmamış" tone={overdueCount?"warning":"neutral"}/></section>

    {canManage?<FinancePanel title="Toplu Atama" description="Hedefler yalnızca erişiminiz olan şirket ve şubelerden seçilebilir."><form onSubmit={submit} className="grid gap-4 lg:grid-cols-2">
      <label><span className="mb-2 block text-[12px] font-medium text-[var(--muted)]">Eğitim</span><Select required value={courseId} onChange={e=>setCourseId(e.target.value)} className="min-h-11 w-full rounded-[14px] border border-[var(--line)] bg-[var(--surface)] px-3.5 text-[13px]"><option value="">Eğitim seçin</option>{courses.map(course=><option key={course.id} value={course.id}>{course.code} · {course.title}</option>)}</Select></label>
