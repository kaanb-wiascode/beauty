...
export default function QuestionBankPage(){
  ...
  async function create(event:FormEvent){
    ...
    try{await api("/training/lms/question-bank",{method:"POST",body:payload()});setForm(EMPTY);setSuccess("Soru taslağı oluşturuldu. Yayınlandıktan sonra farklı kurs sürümlerindeki sınavlarda yeniden kullanılabilir.");await load();}
  ...
  async function publish(question:BankQuestion){
    ...
    try{await api(`/training/lms/question-bank/${question.id}/publish`,{method:"POST"});setSuccess(`${question.code} · Sürüm ${question.version} yayınlandı.`);await load();}
  ...
  async function addToExam(question:BankQuestion){
    ...
    try{const result=await api<{duplicate?:boolean}>(`/training/lms/question-bank/${question.id}/exams/${targetExam.id}`,{method:"POST",body:{sequence:targetExam.questionCount+1,points:question.defaultPoints}});setSuccess(result.duplicate?"Bu soru sürümü seçili sınavda zaten bulunuyor.":`${question.code} · Sürüm ${question.version}, ${targetExam.courseCode} · ${targetExam.title} sınavına eklendi.`);await load();}
    catch(requestError){setError(errorMessage(requestError,"Soru sınava eklenemedi."));}
    finally{setBusy(null);}
  }

  if(loading)return <div className="flex min-h-[420px] items-center justify-center rounded-[22px] border border-[var(--line)] bg-[var(--surface)]"><Spinner label="Soru bankası hazırlanıyor..."/></div>;

  return <div className="mx-auto max-w-[1440px] space-y-6 pb-10">
    <header><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--muted-soft)]">Sınav ve soru yönetimi</p><h1 className="mt-1 text-[32px] font-semibold tracking-[-0.045em] text-[var(--ink)]">Soru Bankası</h1><p className="mt-2 max-w-[880px] text-[13px] leading-6 text-[var(--muted)]">Yeniden kullanılabilir soru sürümlerini yönetin. Yayınlanan sorular taslak kurs sınavlarına eklenebilir ve geçmiş sınav kayıtları sonradan yapılan değişikliklerden etkilenmez.</p></header>
    {error?<Alert onClose={()=>setError("")}>{error}</Alert>:null}{success?<Alert tone="success" onClose={()=>setSuccess("")}>{success}</Alert>:null}
    <section className="grid gap-3 sm:grid-cols-4"><FinanceMetric label="Gösterilen" value={questions.length} detail="Soru sürümü" tone="neutral"/><FinanceMetric label="Yayında" value={published.length} detail="Yeniden kullanılabilir" tone="success"/><FinanceMetric label="Taslak" value={drafts.length} detail="Yayın Bekliyor" tone={drafts.length?"warning":"neutral"}/><FinanceMetric label="Kategori" value={categories} detail="Aktif Görünüm" tone="info"/></section>

    <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      <FinancePanel title="Yeni Banka Sorusu" description="Aynı soru koduyla yeni kayıt oluşturulduğunda yeni bir sürüm açılır; yeni sürüm yayınlandığında önceki sürüm kullanımdan kaldırılır."><form onSubmit={create} className="space-y-4">
      ...
      <FinancePanel title="Kurs sınavına ekle" description="Yalnızca taslak kurs sürümlerindeki sınavlara soru eklenebilir."><div className="space-y-4"><Field label="Hedef Sınav"><Select className="control h-11 w-full" value={targetExamId} onChange={event=>setTargetExamId(event.target.value)}><option value="">Taslak sınav seçin</option>{draftExams.map(exam=><option key={exam.id} value={exam.id}>{exam.courseCode} · {exam.courseTitle} · Sürüm {exam.version} · {exam.title} ({exam.questionCount} soru)</option>)}</Select></Field>{targetExam?<Alert tone="success">Seçili hedef: {targetExam.courseCode} · Sürüm {targetExam.version} / {targetExam.title}. Soru eklendiğinde cevap ve seçenekler bu sürüm için ayrı olarak kaydedilir.</Alert>:<FinanceEmpty title="Taslak sınav yok" description="Önce kurs yazarlığı alanında teori sınavı oluşturun."/>}</div></FinancePanel>
    </div>

    <FinancePanel title="Soru sürümleri" description="Taslak, yayınlanmış ve emekli sürümleri tek katalogdan yönetin."><div className="mb-4 flex flex-wrap gap-2">{["","DRAFT","PUBLISHED","RETIRED"].map(value=><Button key={value||"ALL"} variant={status===value?"primary":"secondary"} onClick={()=>setStatus(value)}>{value?STATUS[value as BankQuestion["status"]]:"Tümü"}</Button>)}</div><div className="space-y-3">{questions.length?questions.map(question=>{const options=displayOptions(question.options);return <article key={question.id} className="rounded-[18px] border border-[var(--line)] p-4"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[var(--accent)]">{question.code} · Sürüm {question.version}</span><span className="rounded-full bg-[var(--surface-2)] px-2 py-0.5 text-[9px] font-semibold text-[var(--muted)]">{STATUS[question.status]}</span><span className="text-[9px] font-medium text-[var(--muted-soft)]">{question.category}</span></div><p className="mt-2 text-[13px] font-semibold leading-6 text-[var(--ink)]">{question.prompt}</p>{options.length?<div className="mt-3 flex flex-wrap gap-2">{options.map(option=><span key={option} className="rounded-[10px] border border-[var(--line)] px-2.5 py-1.5 text-[10px] text-[var(--muted)]">{option}</span>)}</div>:null}<p className="mt-3 text-[10px] text-[var(--muted-soft)]">{userLabel(question.questionType)} · {question.defaultPoints} puan · Cevap: {displayAnswer(question.correctAnswer)}{question.tags?.length?` · ${question.tags.join(", ")}`:""}</p></div><div className="flex shrink-0 flex-wrap gap-2">{question.status==="DRAFT"?<Button disabled={busy!==null} onClick={()=>void publish(question)}>{busy===`publish:${question.id}`?"Yayınlanıyor...":"Yayınla"}</Button>:null}{question.status==="PUBLISHED"?<Button variant="secondary" disabled={!targetExam||busy!==null} onClick={()=>void addToExam(question)}>{busy===`attach:${question.id}`?"Ekleniyor...":"Sınava Ekle"}</Button>:null}</div></div></article>}):<FinanceEmpty title="Soru Bulunmuyor" description="Seçili filtre için banka sorusu bulunmuyor."/>}</div></FinancePanel>
  </div>;
}
