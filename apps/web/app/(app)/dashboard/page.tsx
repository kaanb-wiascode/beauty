  ...
  return <div className={`${styles.page} mx-auto max-w-[1420px] pb-8`}>
    ...
    <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <aside className="dashboard-rail xl:sticky xl:top-0 xl:h-[calc(100vh-24px)] xl:overflow-y-auto xl:pr-1">
        ...
        <Panel title="Hızlı İşlemler" subtitle="Sık kullanılan işlemlere hızlıca erişin"><div className="quick-actions"><Quick icon={<CalendarIcon/>} label="Yeni Randevu" onClick={()=>setAction("appointment")}/><Quick icon={<PeopleIcon/>} label="Yeni Müşteri" onClick={()=>setAction("customer")}/><Quick icon={<SparkleIcon/>} label="Yeni Hizmet" onClick={()=>setAction("service")}/><Quick icon={<WalletIcon/>} label="Ödeme Al" onClick={()=>setAction("payment")}/></div></Panel>
...
function Quick({icon,label,onClick}:{icon:ReactNode;label:string;onClick:()=>void}){return <button type="button" onClick={onClick} className="quick-action"><div className="quick-action-top"><span className="quick-action-icon">{icon}</span><span className="quick-action-arrow"><ArrowRightIcon/></span></div><b className="quick-action-label">{label}</b></button>}
