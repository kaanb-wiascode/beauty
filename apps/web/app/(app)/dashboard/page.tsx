  ...
  return <div className={`${styles.page} mx-auto max-w-[1420px] pb-8`}>
    ...
    <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <aside className="dashboard-rail xl:sticky xl:top-0 xl:h-[calc(100vh-24px)] xl:overflow-y-auto xl:pr-1">
        ...
        <Panel title="Hızlı İşlemler" subtitle="Sık kullanılan işlemlere hızlıca erişin"><div className="quick-actions"><Quick icon={<CalendarIcon/>} label="Yeni Randevu" description="Randevu oluştur" onClick={()=>setAction("appointment")}/><Quick icon={<PeopleIcon/>} label="Yeni Müşteri" description="Müşteri kaydet" onClick={()=>setAction("customer")}/><Quick icon={<SparkleIcon/>} label="Yeni Hizmet" description="Hizmet oluştur" onClick={()=>setAction("service")}/><Quick icon={<WalletIcon/>} label="Ödeme Al" description="Tahsilat kaydet" onClick={()=>setAction("payment")}/></div></Panel>
...
function Quick({icon,label,description,onClick}:{icon:ReactNode;label:string;description:string;onClick:()=>void}){return <button type="button" onClick={onClick} className="quick-action"><span className="quick-action-icon">{icon}</span><div className="quick-action-copy"><b>{label}</b><small>{description}</small></div><ArrowRightIcon/></button>}
