...
export default function StaffReportPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    ...
    {loading ? <Spinner label="Personel Raporu Hazırlanıyor..." /> : <>
      ...
      <Panel>
        <div className="flex flex-col gap-3 border-b border-[var(--line)] px-5 py-4 lg:flex-row lg:items-center lg:justify-between"><div><h2 className="text-[16px] font-semibold text-[var(--ink)]">Personel Detayları</h2><p className="mt-1 text-[12px] text-[var(--muted)]">Personel adına tıklayarak ayrıntılı randevu görünümünü açabilirsiniz.</p></div><div className="flex gap-2"><input aria-label="Personel Ara" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Bu Sayfada Ara..." className="h-9 rounded-xl border border-[var(--line)] bg-[var(--surface)] px-3 text-[12px]" /><details className="relative"><summary className="flex h-9 cursor-pointer list-none items-center rounded-xl border border-[var(--line)] px-3 text-[12px]">Sütunlar</summary><div className="absolute right-0 z-20 mt-2 min-w-44 rounded-xl border border-[var(--line)] bg-[var(--surface)] p-2 shadow-lg">{COLUMNS.map((column) => <label key={column} className="flex items-center gap-2 px-2 py-2 text-[12px]"><input type="checkbox" checked={table.visibleColumns.has(column)} onChange={() => table.toggleColumn(column)} />{LABELS[column]}</label>)}</div></details></div></div>
