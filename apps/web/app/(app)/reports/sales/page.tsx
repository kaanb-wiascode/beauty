...
import { ReportSaleDrilldownPanel } from "../report-sale-drilldown-panel";
...
type Row = { _rowId?: string; confirmedAt: string; customerName: string; revenue: number; collected: number; refunded: number; outstanding: number; itemCount: number };
...
export default function SalesReportPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    ...
    {loading ? <Spinner label="Satış raporu hazırlanıyor..." /> : <>
      ...
      {selected?._rowId ? <ReportSaleDrilldownPanel rowId={selected._rowId} title={selected.customerName} filters={reportRangeToQuery(range)} onClose={() => setSelected(null)} /> : null}
    </>}
  </div>;
}
