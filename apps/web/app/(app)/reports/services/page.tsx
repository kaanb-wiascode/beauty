...
const LABELS: Record<ColumnKey, string> = { name: "Hizmet", appointmentCount: "Randevu", completedAppointments: "Tamamlanan", completionRate: "Başarı", collected: "Tahsilat" };
...
export default function ServiceReportPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const result = await fetchReportPreview<TableReportPreview<Row>>({ reportKey: "service.performance", filters: reportRangeToQuery(range), columns: COLUMNS, sort: table.sort, page: meta.page, limit: 25 });
        ...
        if (!cancelled) setError(err instanceof ApiError ? err.message : "Hizmet Raporu Yüklenemedi.");
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    ...
    {loading ? <Spinner label="Hizmet Raporu Hazırlanıyor..." /> : <>
      ...
      <Panel>
        <div className="flex flex-col gap-3 border-b border-[var(--line)] px-5 py-4 lg:flex-row lg:items-center lg:justify-between"><div><h2 className="text-[16px] font-semibold text-[var(--ink)]">Hizmet Detayları</h2><p className="mt-1 text-[12px] text-[var(--muted)]">Hizmet adına tıklayarak yetkili randevu drill-down görünümünü açabilirsiniz.</p></div><div className="flex gap-2"><input aria-label="Hizmet Ara" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Bu Sayfada Ara..." className="h-9 rounded-xl border border-[var(--line)] bg-[var(--surface)] px-3 text-[12px]" /><details className="relative"><summary className="flex h-9 cursor-pointer list-none items-center rounded-xl border border-[var(--line)] px-3 text-[12px]">Sütunlar</summary><div className="absolute right-0 z-20 mt-2 min-w-44 rounded-xl border border-[var(--line)] bg-[var(--surface)] p-2 shadow-lg">{COLUMNS.map((column) => <label key={column} className="flex items-center gap-2 px-2 py-2 text-[12px]"><input type="checkbox" checked={table.visibleColumns.has(column)} onChange={() => table.toggleColumn(column)} />{LABELS[column]}</label>)}</div></details></div></div>
        {filtered.length === 0 ? <Empty /> : <TableWrap><thead><tr>{table.visibleColumnList.map((column) => <Th key={column}><SortButton label={LABELS[column]} active={table.sort.key === column} direction={table.sort.direction} onClick={() => table.toggleSort(column)} /></Th>)}</tr></thead><tbody>{filtered.map((row) => <tr key={row._rowId ?? `${row.branchId}-${row.name}`}>{table.visibleColumns.has("name") ? <Td label="Hizmet"><button type="button" disabled={!row._rowId} onClick={() => row._rowId && setSelected(row)} className="font-medium text-left enabled:hover:text-[var(--accent)] disabled:cursor-default">{row.name}</button></Td> : null}{table.visibleColumns.has("appointmentCount") ? <Td label="Randevu">{row.appointmentCount}</Td> : null}{table.visibleColumns.has("completedAppointments") ? <Td label="Tamamlanan">{row.completedAppointments}</Td> : null}{table.visibleColumns.has("completionRate") ? <Td label="Başarı">%{row.completionRate}</Td> : null}{table.visibleColumns.has("collected") ? <Td label="Tahsilat" className="font-semibold">{money(row.collected)}</Td> : null}</tr>)}</tbody></TableWrap>}
        <Pagination page={meta.page} totalPages={meta.totalPages} onChange={(page) => setMeta((current) => ({ ...current, page }))} />
      </Panel>
      {selected?._rowId ? <ReportDrilldownPanel reportKey="service.performance" rowId={selected._rowId} title={selected.name} filters={reportRangeToQuery(range)} onClose={() => setSelected(null)} /> : null}
    </>}
  </div>;
}
...
function Metric({ label, value, detail }: { label: string; value: string; detail: string }) { return <GlassCard><div className="flex items-start justify-between gap-3"><p className="text-[11px] text-[var(--muted)]">{label}</p><CardInfo help={getCardHelp(label, detail)} /></div><p className="mt-1.5 text-[24px] font-semibold text-[var(--ink)]">{value}</p><p className="mt-1 text-[10px] text-[var(--muted-soft)]">{detail}</p></GlassCard>; }
