  ...
  if (initialLoading && !integrations.length) {
    return (
      <div className="mx-auto max-w-[1500px] py-20">
        <Spinner label="Bağlantı işlemleri hazırlanıyor..." />
      </div>
    );
  }
      ...
      <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-[14px] leading-6 text-[var(--muted)]">
            Banka ve ödeme bağlantılarının güncelliğini, başarı oranını, uyarılarını ve işlem geçmişini tek ekrandan izleyin.
          </p>
        </div>
      ...
      {current && health && alerts ? (
        <>
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            ...
            <FinanceMetric label="Ortalama işlem süresi" value={duration(health.observability.averageDurationMs)} detail={`${health.observability.failures} başarısız güncelleme`} />
            ...
            <FinanceMetric label="Eşleştirilmemiş banka işlemleri" value={String(health.banking?.unmatchedTransactionCount ?? 0)} detail="Mutabakat bekleyen" tone={(health.banking?.unmatchedTransactionCount ?? 0) > 0 ? "warning" : "neutral"} />
          </section>
...
function auditActionLabel(action: string) {
  ...
  if (normalized.includes("CONNECT")) return "Bağlantı işlemi";
  if (normalized.includes("SYNC")) return "Veri güncelleme işlemi";
  if (normalized.includes("CREDENTIAL")) return "Bağlantı bilgisi işlemi";
  if (normalized.includes("CREATE")) return "Oluşturma işlemi";
  if (normalized.includes("UPDATE")) return "Güncelleme işlemi";
  if (normalized.includes("DELETE")) return "Silme işlemi";
  return "Sistem işlemi";
}
