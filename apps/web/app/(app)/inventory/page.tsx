...
function ProductFormContent({ tab, form, update, categories, suppliers }: { tab: ProductTab; form: ProductFormState; update: <K extends keyof ProductFormState>(key: K, value: ProductFormState[K]) => void; categories: InventoryCategory[]; suppliers: InventorySupplier[] }) {
  if (tab === "Genel") return <div className="grid gap-5 md:grid-cols-2"><Field label="Ürün Adı" required><TextInput value={form.name} onValueChange={(value) => update("name", value)} placeholder="Cilt Yenileyici Serum" required/></Field><Field label="Stok Kodu"><TextInput value={form.sku} onValueChange={(value) => update("sku", value)} placeholder="SER-001"/></Field><Field label="Barkod"><TextInput value={form.barcode} onValueChange={(value) => update("barcode", value)} placeholder="8691234567890"/></Field><Field label="Marka"><TextInput value={form.brand} onValueChange={(value) => update("brand", value)} placeholder="Marka"/></Field><Field label="Üretici"><TextInput value={form.manufacturer} onValueChange={(value) => update("manufacturer", value)} placeholder="Üretici Firma"/></Field><Field label="Model / Seri"><TextInput value={form.model} onValueChange={(value) => update("model", value)} placeholder="Model"/></Field><Field label="Ürün Birimi"><SelectNative value={form.unit} onValueChange={(value) => update("unit", value)}>{INVENTORY_UNITS.map((unit) => <option key={unit} value={unit}>{inventoryUnitLabel(unit)}</option>)}</SelectNative></Field><Field label="Paket İçeriği"><TextInput type="number" value={form.packageQuantity} onValueChange={(value) => update("packageQuantity", value)} placeholder="30"/></Field><Field label="Menşei"><TextInput value={form.originCountry} onValueChange={(value) => update("originCountry", value)} placeholder="Türkiye"/></Field><Field label="Açıklama"><TextInput value={form.description} onValueChange={(value) => update("description", value)} placeholder="Ürün Açıklaması"/></Field></div>;
  if (tab === "Kategori") return <div className="space-y-5"><div className="grid gap-4 md:grid-cols-2"><Field label="Kategori" required><SelectNative value={form.categoryId} onValueChange={(value) => update("categoryId", value)}><option value="">Kategori Seçin</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}{category.code ? ` · ${category.code}` : ""}</option>)}</SelectNative></Field><Field label="Etiket / Alt Kategori"><TextInput placeholder="Serumlar"/></Field></div><Hint>Kategori Ağacı Ürünlerin Raporlama, Satın Alma Ve Stok Politikalarında Temel Alınır. Örnek: Cilt Bakımı → Serumlar.</Hint></div>;
  if (tab === "Stok") return <div className="grid gap-5 md:grid-cols-2"><Toggle label="Stok Takibi" value={form.trackStock} onChange={(value) => update("trackStock", value)}/><Toggle label="Parti Ve Son Kullanma Takibi" value={form.trackExpiry} onChange={(value) => update("trackExpiry", value)}/><Field label="Minimum Stok"><TextInput type="number" value={form.minimumQuantity} onValueChange={(value) => update("minimumQuantity", value)} placeholder="10"/></Field><Field label="İdeal Stok"><TextInput type="number" value={form.targetQuantity} onValueChange={(value) => update("targetQuantity", value)} placeholder="30"/></Field><Field label="Başlangıç Stoğu"><TextInput type="number" value={form.initialQuantity} onValueChange={(value) => update("initialQuantity", value)} placeholder="0"/></Field><Field label="İlk Maliyet"><TextInput type="number" value={form.purchasePrice} onValueChange={(value) => update("purchasePrice", value)} placeholder="420"/></Field></div>;
  if (tab === "Tedarik") return <div className="space-y-5"><div className="grid gap-4 md:grid-cols-2"><Field label="Ana Tedarikçi"><SelectNative value={form.supplierId} onValueChange={(value) => update("supplierId", value)}><option value="">Tedarikçi Seçin</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</SelectNative></Field><Field label="Tedarikçi Ürün Kodu"><TextInput value={form.supplierProductCode} onValueChange={(value) => update("supplierProductCode", value)} placeholder="ABC-93821"/></Field><Field label="Birim Alış Fiyatı"><TextInput type="number" value={form.supplierUnitCost} onValueChange={(value) => update("supplierUnitCost", value)} placeholder="420"/></Field><Field label="Minimum Sipariş"><TextInput type="number" value={form.supplierMinimumOrderQuantity} onValueChange={(value) => update("supplierMinimumOrderQuantity", value)} placeholder="10"/></Field><Field label="Sipariş Katı"><TextInput type="number" value={form.supplierOrderMultiple} onValueChange={(value) => update("supplierOrderMultiple", value)} placeholder="5"/></Field><Field label="Tedarik Süresi (Gün)"><TextInput type="number" value={form.supplierLeadTimeDays} onValueChange={(value) => update("supplierLeadTimeDays", value)} placeholder="7"/></Field><Field label="Hazırlık Süresi (Gün)"><TextInput type="number" value={form.supplierPreparationDays} onValueChange={(value) => update("supplierPreparationDays", value)} placeholder="2"/></Field><Field label="Kargo Süresi (Gün)"><TextInput type="number" value={form.supplierShippingDays} onValueChange={(value) => update("supplierShippingDays", value)} placeholder="3"/></Field></div><Hint>Toplam Teslim Süresi Tedarik, Hazırlık Ve Kargo Sürelerinin Toplamıdır. Sistem Tüketim Hızını Ve Güvenlik Stoğunu Kullanarak Sipariş Önerisi Oluşturabilir.</Hint></div>;
  if (tab === "Finans") return <div className="grid gap-5 md:grid-cols-2"><Field label="Alış Fiyatı"><TextInput type="number" value={form.purchasePrice} onValueChange={(value) => update("purchasePrice", value)} placeholder="420"/></Field><Field label="Satış Fiyatı"><TextInput type="number" value={form.salePrice} onValueChange={(value) => update("salePrice", value)} placeholder="750"/></Field><Field label="KDV"><TextInput type="number" value={form.taxRate} onValueChange={(value) => update("taxRate", value)} placeholder="20"/></Field><Field label="Para Birimi"><SelectNative value={form.currency} onValueChange={(value) => update("currency", value)}><option value="TRY">Türk Lirası</option><option value="EUR">Avro</option><option value="USD">Amerikan Doları</option></SelectNative></Field><div className="md:col-span-2"><Toggle label="İade Edilebilir" value={form.returnable} onChange={(value) => update("returnable", value)}/></div></div>;
  if (tab === "Parti Ve Son Kullanma") return <div className="grid gap-5 md:grid-cols-2"><Toggle label="Parti Ve Son Kullanma Takibi Aktif" value={form.trackExpiry} onChange={(value) => update("trackExpiry", value)}/><Field label="Parti Numarası"><TextInput value={form.lotNumber} onValueChange={(value) => update("lotNumber", value)} placeholder="PARTİ-2026-0831"/></Field><Field label="Üretim Tarihi"><TextInput type="date" value={form.manufacturedAt} onValueChange={(value) => update("manufacturedAt", value)}/></Field><Field label="Son Kullanma Tarihi"><TextInput type="date" value={form.expiresAt} onValueChange={(value) => update("expiresAt", value)}/></Field><div className="md:col-span-2"><Hint tone="warning">Son Kullanma Tarihi En Yakın Ürünün Önce Tüketilmesi Hedeflenir. Uyarılar Sistem Kurallarına Göre Oluşturulur.</Hint></div></div>;
  return <div className="space-y-5"><Hint>Bu Ürün Hizmet Tamamlandığında Stok Tüketimiyle İlişkilendirilebilir. Gerçek Tüketim Kayıtları Sistem Tarafından Ayrı Olarak İzlenir.</Hint><Field label="İç Kullanım Notu"><TextInput value={form.serviceNotes} onValueChange={(value) => update("serviceNotes", value)} placeholder="Örnek: Cilt Bakımında 5 Mililitre Kullanılır"/></Field></div>;
}
...
function AssetFormContent({ tab, form, update, categories, suppliers, warehouses, activeBranchId }: { tab: AssetTab; form: AssetFormState; update: <K extends keyof AssetFormState>(key: K, value: AssetFormState[K]) => void; categories: InventoryCategory[]; suppliers: InventorySupplier[]; warehouses: InventoryWarehouse[]; activeBranchId: string | null }) {
  ...
  if (tab === "Genel") return <div className="grid gap-5 md:grid-cols-2"><Field label="Varlık Adı" required><TextInput value={form.name} onValueChange={(value) => update("name", value)} placeholder="Cilt Bakım Cihazı" required/></Field><Field label="Varlık Kodu" required><TextInput value={form.assetCode} onValueChange={(value) => update("assetCode", value)} placeholder="VAR-00124" required/></Field><Field label="Varlık Tipi"><SelectNative value={form.assetType} onValueChange={(value) => update("assetType", value)}>{["EQUIPMENT", "FURNITURE", "IT", "VEHICLE", "OTHER"].map((value) => <option key={value} value={value}>{inventoryAssetTypeLabel(value)}</option>)}</SelectNative></Field><Field label="Kategori"><SelectNative value={form.categoryId} onValueChange={(value) => update("categoryId", value)}><option value="">Kategori Seçin</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</SelectNative></Field><Field label="Marka"><TextInput value={form.brand} onValueChange={(value) => update("brand", value)} placeholder="Marka"/></Field><Field label="Model"><TextInput value={form.model} onValueChange={(value) => update("model", value)} placeholder="Model"/></Field><Field label="Seri Numarası"><TextInput value={form.serialNumber} onValueChange={(value) => update("serialNumber", value)} placeholder="SERİ-839292"/></Field><Field label="Durum"><SelectNative value={form.status} onValueChange={(value) => update("status", value)}>{["ACTIVE", "INACTIVE", "MAINTENANCE", "RETIRED"].map((value) => <option key={value} value={value}>{inventoryAssetStatusLabel(value)}</option>)}</SelectNative></Field><Field label="Fiziksel Durum"><SelectNative value={form.condition} onValueChange={(value) => update("condition", value)}>{["GOOD", "FAIR", "POOR", "BROKEN"].map((value) => <option key={value} value={value}>{inventoryAssetConditionLabel(value)}</option>)}</SelectNative></Field></div>;
  if (tab === "Satın Alma") return <div className="grid gap-5 md:grid-cols-2"><Field label="Satın Alma Tarihi"><TextInput type="date" value={form.purchaseDate} onValueChange={(value) => update("purchaseDate", value)}/></Field><Field label="Tedarikçi"><SelectNative value={form.supplierId} onValueChange={(value) => update("supplierId", value)}><option value="">Tedarikçi Seçin</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</SelectNative></Field><Field label="Fatura No"><TextInput value={form.invoiceNumber} onValueChange={(value) => update("invoiceNumber", value)} placeholder="FAT-83921"/></Field><Field label="Satın Alma Bedeli"><TextInput type="number" value={form.purchasePrice} onValueChange={(value) => update("purchasePrice", value)} placeholder="185000"/></Field><Field label="Para Birimi"><SelectNative value={form.currency} onValueChange={(value) => update("currency", value)}><option value="TRY">Türk Lirası</option><option value="EUR">Avro</option><option value="USD">Amerikan Doları</option></SelectNative></Field></div>;
  if (tab === "Konum Ve Zimmet") return <div className="grid gap-5 md:grid-cols-2"><Field label="Şube"><SelectNative value={activeBranchId ?? form.branchId} disabled={Boolean(activeBranchId)} onValueChange={(value) => update("branchId", value)}>{activeBranchId ? null : <option value="">Merkez / Ortak</option>}{branchWarehouses.map((warehouse) => <option key={warehouse.branchId ?? warehouse.id} value={warehouse.branchId ?? ""}>{warehouse.name.replace(" Stok", "")}</option>)}</SelectNative></Field><Field label="Depo / Konum"><SelectNative value={form.warehouseId} onValueChange={(value) => update("warehouseId", value)}><option value="">Seçin</option>{locationWarehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</SelectNative></Field><Field label="Zimmetli Personel"><TextInput placeholder="Personel Zimmet İşlemlerinden Yönetilir" disabled/></Field><Hint>{activeBranchId ? "Aktif Şube Seçiliyken Envanter Varlığı Yalnız Bu Şubeye Ve Şubenin Depolarına Kaydedilebilir." : "Şube Transferleri Ve Zimmet Geçmişi Varlık Kartında Ayrı Kayıtlarla İzlenir."}</Hint></div>;
  if (tab === "Garanti Ve Bakım") return <div className="grid gap-5 md:grid-cols-2"><Field label="Garanti Başlangıcı"><TextInput type="date" value={form.warrantyStart} onValueChange={(value) => update("warrantyStart", value)}/></Field><Field label="Garanti Bitişi"><TextInput type="date" value={form.warrantyEnd} onValueChange={(value) => update("warrantyEnd", value)}/></Field><Field label="Bakım Aralığı (Gün)"><TextInput type="number" value={form.maintenanceIntervalDays} onValueChange={(value) => update("maintenanceIntervalDays", value)} placeholder="180"/></Field><Field label="Sonraki Bakım"><TextInput type="date" value={form.nextMaintenanceAt} onValueChange={(value) => update("nextMaintenanceAt", value)}/></Field><div className="md:col-span-2"><Hint>Garanti Bitişi Ve Yaklaşan Bakım Tarihleri Sistem Kurallarına Göre İzlenir.</Hint></div></div>;
  ...
  return <Field label="Notlar"><TextInput value={form.notes} onValueChange={(value) => update("notes", value)} placeholder="Varlıkla İlgili Ek Bilgiler"/></Field>;
}
...
function SelectNative({
  ...
  const options = Children.toArray(children).flatMap((child) => {
    if (!isValidElement<{ value?: string | number; children?: ReactNode; disabled?: boolean }>(child) || child.type !== "option") {
      return [];
    }
    ...
    return [{
      value: optionValue,
      label: reactNodeText(child.props.children) || optionValue,
...
function reactNodeText(node: ReactNode): string {
  return Children.toArray(node)
    .map((part) => {
      ...
      if (isValidElement<{ children?: ReactNode }>(part)) return reactNodeText(part.props.children);
      return "";
    })
    .join("");
}
