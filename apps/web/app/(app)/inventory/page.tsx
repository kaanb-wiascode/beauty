...
function AssetFormContent({ tab, form, update, categories, suppliers, warehouses }: { tab: AssetTab; form: AssetFormState; update: <K extends keyof AssetFormState>(key: K, value: AssetFormState[K]) => void; categories: InventoryCategory[]; suppliers: InventorySupplier[]; warehouses: InventoryWarehouse[] }) {
  ...
  if (tab === "Konum Ve Zimmet") return <div className="grid gap-5 md:grid-cols-2"><Field label="Şube"><SelectNative value={form.branchId} onChange={(event) => update("branchId", event.target.value)}><option value="">Merkez / Ortak</option>{warehouses.filter((warehouse) => warehouse.branchId).map((warehouse) => <option key={warehouse.branchId ?? warehouse.id} value={warehouse.branchId ?? ""}>{warehouse.name.replace(" Stok", "")}</option>)}</SelectNative></Field><Field label="Depo / Konum"><SelectNative value={form.warehouseId} onChange={(event) => update("warehouseId", event.target.value)}><option value="">Seçin</option>{warehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</SelectNative></Field><Field label="Zimmetli Personel"><TextInput placeholder="Personel Zimmet İşlemlerinden Yönetilir" disabled/></Field><Hint>Şube Transferleri Ve Zimmet Geçmişi Varlık Kartında Ayrı Kayıtlarla İzlenir.</Hint></div>;
