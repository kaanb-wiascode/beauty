...
const productTabs = ["Genel", "Kategori", "Stok", "Tedarik", "Finans", "Lot & SKT", "Kullanım"] as const;
const assetTabs = ["Genel", "Satın Alma", "Lokasyon & Zimmet", "Garanti & Bakım", "Belgeler", "Notlar"] as const;
...
export default function InventoryPage() {
  ...
  return <div className="mx-auto max-w-[1480px] space-y-6 pb-10">
    <header className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
      <div>
        <p className="mb-2 text-[11px] font-semibold uppercase tracking-[.15em] text-[var(--muted-soft)]">ENVANTER YÖNETİMİ</p>
        <h1 className="text-[34px] font-semibold tracking-[-.04em] text-[var(--ink)]">Stok & Envanter</h1>
        <p className="mt-2 max-w-2xl text-[14px] leading-6 text-[var(--muted)]">Ürünleri, sarf malzemelerini, şube stoklarını ve şirket varlıklarını tek bir yaşam döngüsü üzerinden yönetin.</p>
      </div>
      <div className="flex flex-wrap gap-2">
        <Button onClick={() => { setProductTab("Genel"); setProductOpen(true); }}><Icon name="box"/>Yeni Ürün</Button>
        <Button variant="secondary" onClick={() => { setAssetTab("Genel"); setAssetOpen(true); }}><Icon name="asset"/>Yeni Envanter</Button>
      </div>
    </header>
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
      ...
      <Metric icon="calendar" label="Yaklaşan SKT" value={data?.expiringLots ?? 0} tone="amber"/>
    ...
    <div className="flex flex-col gap-3 rounded-[20px] border border-[var(--line)] bg-[var(--surface)] p-3 shadow-[0_8px_28px_rgba(17,70,104,.035)] lg:flex-row lg:items-center">
      ...
      <div className="min-w-0 flex-1 lg:ml-auto lg:max-w-[360px]">
        <SearchField value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Ürün, SKU, barkod ara..." aria-label="Envanter ara" onKeyDown={(event) => { if (event.key === "Escape" && search) setSearch(""); }}/>
      </div>
    </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <aside className="space-y-5">
          <section className="overflow-hidden rounded-[22px] border border-[var(--line)] bg-[var(--surface)]">
            ...
            {(data?.critical ?? []).slice(0, 5).map((product) => <div key={product.id} className="border-b border-[var(--line)] px-5 py-4 last:border-0"><div className="flex justify-between gap-3"><span className="truncate text-[12px] font-medium text-[var(--ink)]">{product.name}</span><span className="text-[11px] font-semibold text-[var(--warning)]">{formatInventoryQuantity(product.quantity)} {inventoryUnitLabel(product.unit)}</span></div><div className="mt-2 flex items-center gap-2"><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-[var(--surface-2)]"><div className="h-full rounded-full bg-[var(--warning)]" style={{ width: `${Math.min(100, (Number(product.quantity) / (Number(product.targetQuantity) || Number(product.minimumQuantity) || 1)) * 100)}%` }}/></div><span className="text-[9px] text-[var(--muted-soft)]">min {formatInventoryQuantity(product.minimumQuantity)}</span></div></div>)}
            {!data?.critical?.length ? <div className="p-8 text-center text-[12px] text-[var(--muted)]">Kritik stok bulunmuyor.</div> : null}
          </section>
          <section className="overflow-hidden rounded-[22px] border border-[var(--line)] bg-[var(--surface)]">
            <div className="border-b border-[var(--line)] p-5"><h2 className="text-[15px] font-semibold text-[var(--ink)]">Lokasyonlar</h2><p className="mt-1 text-[11px] text-[var(--muted)]">Ana depo + şubeler</p></div>
    ...
    {productOpen ? <FormOverlay title="Yeni Ürün Ekle" description="Ürünü; kategori, stok politikası, tedarik, finans ve lot/SKT bilgileriyle eksiksiz oluşturun." tabs={productTabs} activeTab={productTab} setActiveTab={setProductTab} onClose={() => setProductOpen(false)} onSubmit={createProduct} saving={saving} footer="Ürünü Kaydet"><ProductFormContent tab={productTab} form={form} update={updateProduct} categories={categories} suppliers={suppliers}/></FormOverlay> : null}
    {assetOpen ? <FormOverlay title="Yeni Envanter Ekle" description="Şirketin taşınır varlığını satın alma, lokasyon, zimmet, garanti ve bakım bilgileriyle kaydedin." tabs={assetTabs} activeTab={assetTab} setActiveTab={setAssetTab} onClose={() => setAssetOpen(false)} onSubmit={createAsset} saving={saving} footer="Envanteri Kaydet"><AssetFormContent tab={assetTab} form={asset} update={updateAsset} categories={categories} suppliers={suppliers} warehouses={data?.warehouses ?? []}/></FormOverlay> : null}
    {categoryOpen ? <SimpleOverlay title="Yeni Kategori" onClose={() => setCategoryOpen(false)} onSubmit={createCategory} footer="Kategori Oluştur" saving={saving}><div className="grid gap-4 sm:grid-cols-2"><Field label="Kategori adı" required><TextInput name="name" required placeholder="Cilt Bakımı"/></Field><Field label="Kategori kodu"><TextInput name="code" placeholder="SKINCARE"/></Field><Field label="Varsayılan birim"><SelectNative name="defaultUnit">{INVENTORY_UNITS.map((unit) => <option key={unit}>{unit}</option>)}</SelectNative></Field><Field label="Açıklama"><TextInput name="description" placeholder="Kategori açıklaması"/></Field></div></SimpleOverlay> : null}
    {supplierOpen ? <SimpleOverlay title="Yeni Tedarikçi" onClose={() => setSupplierOpen(false)} onSubmit={createSupplier} footer="Tedarikçiyi Kaydet" saving={saving}><div className="grid gap-4 sm:grid-cols-2"><Field label="Firma adı" required><TextInput name="name" required placeholder="ABC Kozmetik"/></Field><Field label="Yetkili"><TextInput name="contactName" placeholder="Ad Soyad"/></Field><Field label="Telefon"><TextInput name="phone" placeholder="+90"/></Field><Field label="E-posta"><TextInput name="email" type="email" placeholder="satınalma@..."/></Field><Field label="Vergi no"><TextInput name="taxNumber"/></Field><Field label="Adres"><TextInput name="address"/></Field></div></SimpleOverlay> : null}
  </div>;
}
...
function ProductFormContent({ tab, form, update, categories, suppliers }: { tab: ProductTab; form: ProductFormState; update: <K extends keyof ProductFormState>(key: K, value: ProductFormState[K]) => void; categories: InventoryCategory[]; suppliers: InventorySupplier[] }) {
  if (tab === "Genel") return <div className="grid gap-5 md:grid-cols-2"><Field label="Ürün adı" required><TextInput value={form.name} onChange={(event) => update("name", event.target.value)} placeholder="Cilt Yenileyici Serum" required/></Field><Field label="SKU"><TextInput value={form.sku} onChange={(event) => update("sku", event.target.value)} placeholder="SER-001"/></Field><Field label="Barkod"><TextInput value={form.barcode} onChange={(event) => update("barcode", event.target.value)} placeholder="8691234567890"/></Field><Field label="Marka"><TextInput value={form.brand} onChange={(event) => update("brand", event.target.value)} placeholder="Marka"/></Field><Field label="Üretici"><TextInput value={form.manufacturer} onChange={(event) => update("manufacturer", event.target.value)} placeholder="Üretici firma"/></Field><Field label="Model / Seri"><TextInput value={form.model} onChange={(event) => update("model", event.target.value)} placeholder="Model"/></Field><Field label="Ürün tipi"><SelectNative value={form.unit} onChange={(event) => update("unit", event.target.value)}>{INVENTORY_UNITS.map((unit) => <option key={unit}>{unit}</option>)}</SelectNative></Field><Field label="Paket içeriği"><TextInput type="number" value={form.packageQuantity} onChange={(event) => update("packageQuantity", event.target.value)} placeholder="30"/></Field><Field label="Menşei"><TextInput value={form.originCountry} onChange={(event) => update("originCountry", event.target.value)} placeholder="Türkiye"/></Field><Field label="Açıklama"><TextInput value={form.description} onChange={(event) => update("description", event.target.value)} placeholder="Ürün açıklaması"/></Field></div>;
  if (tab === "Kategori") return <div className="space-y-5"><div className="grid gap-4 md:grid-cols-2"><Field label="Kategori" required><SelectNative value={form.categoryId} onChange={(event) => update("categoryId", event.target.value)}><option value="">Kategori seçin</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}{category.code ? ` · ${category.code}` : ""}</option>)}</SelectNative></Field><Field label="Etiket / alt kategori"><TextInput placeholder="Serumlar"/></Field></div><Hint>Kategori ağacı ürünlerin raporlama, satın alma ve stok politikalarında temel alınır. Örn. Cilt Bakımı → Serumlar.</Hint></div>;
  if (tab === "Stok") return <div className="grid gap-5 md:grid-cols-2"><Toggle label="Stok takibi" value={form.trackStock} onChange={(value) => update("trackStock", value)}/><Toggle label="Lot / SKT takibi" value={form.trackExpiry} onChange={(value) => update("trackExpiry", value)}/><Field label="Minimum stok"><TextInput type="number" value={form.minimumQuantity} onChange={(event) => update("minimumQuantity", event.target.value)} placeholder="10"/></Field><Field label="İdeal stok"><TextInput type="number" value={form.targetQuantity} onChange={(event) => update("targetQuantity", event.target.value)} placeholder="30"/></Field><Field label="Başlangıç stoğu"><TextInput type="number" value={form.initialQuantity} onChange={(event) => update("initialQuantity", event.target.value)} placeholder="0"/></Field><Field label="İlk maliyet"><TextInput type="number" value={form.purchasePrice} onChange={(event) => update("purchasePrice", event.target.value)} placeholder="420"/></Field></div>;
  if (tab === "Tedarik") return <div className="space-y-5"><div className="grid gap-4 md:grid-cols-2"><Field label="Ana tedarikçi"><SelectNative value={form.supplierId} onChange={(event) => update("supplierId", event.target.value)}><option value="">Tedarikçi seçin</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</SelectNative></Field><Field label="Tedarikçi ürün kodu"><TextInput value={form.supplierProductCode} onChange={(event) => update("supplierProductCode", event.target.value)} placeholder="ABC-93821"/></Field><Field label="Birim alış fiyatı"><TextInput type="number" value={form.supplierUnitCost} onChange={(event) => update("supplierUnitCost", event.target.value)} placeholder="420"/></Field><Field label="Minimum sipariş"><TextInput type="number" value={form.supplierMinimumOrderQuantity} onChange={(event) => update("supplierMinimumOrderQuantity", event.target.value)} placeholder="10"/></Field><Field label="Sipariş katı"><TextInput type="number" value={form.supplierOrderMultiple} onChange={(event) => update("supplierOrderMultiple", event.target.value)} placeholder="5"/></Field><Field label="Tedarik süresi (gün)"><TextInput type="number" value={form.supplierLeadTimeDays} onChange={(event) => update("supplierLeadTimeDays", event.target.value)} placeholder="7"/></Field><Field label="Hazırlık süresi (gün)"><TextInput type="number" value={form.supplierPreparationDays} onChange={(event) => update("supplierPreparationDays", event.target.value)} placeholder="2"/></Field><Field label="Kargo süresi (gün)"><TextInput type="number" value={form.supplierShippingDays} onChange={(event) => update("supplierShippingDays", event.target.value)} placeholder="3"/></Field></div><Hint>Toplam teslim süresi = tedarik + hazırlık + kargo. Sistem tüketim hızını ve güvenlik stoğunu kullanarak sipariş önerisi oluşturabilir.</Hint></div>;
  if (tab === "Finans") return <div className="grid gap-5 md:grid-cols-2"><Field label="Alış fiyatı"><TextInput type="number" value={form.purchasePrice} onChange={(event) => update("purchasePrice", event.target.value)} placeholder="420"/></Field><Field label="Satış fiyatı"><TextInput type="number" value={form.salePrice} onChange={(event) => update("salePrice", event.target.value)} placeholder="750"/></Field><Field label="KDV"><TextInput type="number" value={form.taxRate} onChange={(event) => update("taxRate", event.target.value)} placeholder="20"/></Field><Field label="Para birimi"><SelectNative value={form.currency} onChange={(event) => update("currency", event.target.value)}><option>TRY</option><option>EUR</option><option>USD</option></SelectNative></Field><div className="md:col-span-2"><Toggle label="İade edilebilir" value={form.returnable} onChange={(value) => update("returnable", value)}/></div></div>;
  if (tab === "Lot & SKT") return <div className="grid gap-5 md:grid-cols-2"><Toggle label="Lot / son kullanma takibi aktif" value={form.trackExpiry} onChange={(value) => update("trackExpiry", value)}/><Field label="Lot numarası"><TextInput value={form.lotNumber} onChange={(event) => update("lotNumber", event.target.value)} placeholder="LOT-2026-0831"/></Field><Field label="Üretim tarihi"><TextInput type="date" value={form.manufacturedAt} onChange={(event) => update("manufacturedAt", event.target.value)}/></Field><Field label="Son kullanma tarihi"><TextInput type="date" value={form.expiresAt} onChange={(event) => update("expiresAt", event.target.value)}/></Field><div className="md:col-span-2"><Hint tone="warning">FEFO ile son kullanma tarihi en yakın lotun önce tüketilmesi hedeflenir. Uyarı davranışı backend lot/SKT kuralları tarafından belirlenir.</Hint></div></div>;
  return <div className="space-y-5"><Hint>Bu ürün hizmet tamamlandığında stok tüketimiyle ilişkilendirilebilir. Gerçek tüketim ilişkisi ayrı domain kaydıyla yönetilmelidir.</Hint><Field label="İç kullanım notu"><TextInput value={form.serviceNotes} onChange={(event) => update("serviceNotes", event.target.value)} placeholder="Örn. Hydrafacial'da 5 ml kullanılır"/></Field></div>;
}

function AssetFormContent({ tab, form, update, categories, suppliers, warehouses }: { tab: AssetTab; form: AssetFormState; update: <K extends keyof AssetFormState>(key: K, value: AssetFormState[K]) => void; categories: InventoryCategory[]; suppliers: InventorySupplier[]; warehouses: InventoryWarehouse[] }) {
  if (tab === "Genel") return <div className="grid gap-5 md:grid-cols-2"><Field label="Varlık adı" required><TextInput value={form.name} onChange={(event) => update("name", event.target.value)} placeholder="Hydrafacial Cihazı" required/></Field><Field label="Varlık kodu" required><TextInput value={form.assetCode} onChange={(event) => update("assetCode", event.target.value)} placeholder="AST-00124" required/></Field><Field label="Varlık tipi"><SelectNative value={form.assetType} onChange={(event) => update("assetType", event.target.value)}><option>EQUIPMENT</option><option>FURNITURE</option><option>IT</option><option>VEHICLE</option><option>OTHER</option></SelectNative></Field><Field label="Kategori"><SelectNative value={form.categoryId} onChange={(event) => update("categoryId", event.target.value)}><option value="">Kategori seçin</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</SelectNative></Field><Field label="Marka"><TextInput value={form.brand} onChange={(event) => update("brand", event.target.value)} placeholder="Hydrafacial"/></Field><Field label="Model"><TextInput value={form.model} onChange={(event) => update("model", event.target.value)} placeholder="MD-1500"/></Field><Field label="Seri numarası"><TextInput value={form.serialNumber} onChange={(event) => update("serialNumber", event.target.value)} placeholder="HDF-839292"/></Field><Field label="Durum"><SelectNative value={form.status} onChange={(event) => update("status", event.target.value)}><option>ACTIVE</option><option>INACTIVE</option><option>MAINTENANCE</option><option>RETIRED</option></SelectNative></Field><Field label="Kondisyon"><SelectNative value={form.condition} onChange={(event) => update("condition", event.target.value)}><option>GOOD</option><option>FAIR</option><option>POOR</option><option>BROKEN</option></SelectNative></Field></div>;
  if (tab === "Satın Alma") return <div className="grid gap-5 md:grid-cols-2"><Field label="Satın alma tarihi"><TextInput type="date" value={form.purchaseDate} onChange={(event) => update("purchaseDate", event.target.value)}/></Field><Field label="Tedarikçi"><SelectNative value={form.supplierId} onChange={(event) => update("supplierId", event.target.value)}><option value="">Tedarikçi seçin</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</SelectNative></Field><Field label="Fatura no"><TextInput value={form.invoiceNumber} onChange={(event) => update("invoiceNumber", event.target.value)} placeholder="INV-83921"/></Field><Field label="Satın alma bedeli"><TextInput type="number" value={form.purchasePrice} onChange={(event) => update("purchasePrice", event.target.value)} placeholder="185000"/></Field><Field label="Para birimi"><SelectNative value={form.currency} onChange={(event) => update("currency", event.target.value)}><option>TRY</option><option>EUR</option><option>USD</option></SelectNative></Field></div>;
  if (tab === "Lokasyon & Zimmet") return <div className="grid gap-5 md:grid-cols-2"><Field label="Şube"><SelectNative value={form.branchId} onChange={(event) => update("branchId", event.target.value)}><option value="">Merkez / ortak</option>{warehouses.filter((warehouse) => warehouse.branchId).map((warehouse) => <option key={warehouse.branchId ?? warehouse.id} value={warehouse.branchId ?? ""}>{warehouse.name.replace(" Stok", "")}</option>)}</SelectNative></Field><Field label="Depo / lokasyon"><SelectNative value={form.warehouseId} onChange={(event) => update("warehouseId", event.target.value)}><option value="">Seçin</option>{warehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</SelectNative></Field><Field label="Zimmetli personel"><TextInput placeholder="Personel ilişkisi ayrı zimmet akışıyla yönetilir" disabled/></Field><Hint>Şube transferleri ve zimmet geçmişi varlık kartında ayrı ilişki kayıtlarıyla izlenmelidir.</Hint></div>;
  if (tab === "Garanti & Bakım") return <div className="grid gap-5 md:grid-cols-2"><Field label="Garanti başlangıcı"><TextInput type="date" value={form.warrantyStart} onChange={(event) => update("warrantyStart", event.target.value)}/></Field><Field label="Garanti bitişi"><TextInput type="date" value={form.warrantyEnd} onChange={(event) => update("warrantyEnd", event.target.value)}/></Field><Field label="Bakım periyodu (gün)"><TextInput type="number" value={form.maintenanceIntervalDays} onChange={(event) => update("maintenanceIntervalDays", event.target.value)} placeholder="180"/></Field><Field label="Sonraki bakım"><TextInput type="date" value={form.nextMaintenanceAt} onChange={(event) => update("nextMaintenanceAt", event.target.value)}/></Field><div className="md:col-span-2"><Hint>Garanti bitişi ve yaklaşan bakım tarihleri mevcut backend kuralları doğrultusunda izlenir.</Hint></div></div>;
  if (tab === "Belgeler") return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><DocTile title="Fatura"/><DocTile title="Garanti belgesi"/><DocTile title="Servis sözleşmesi"/></div><p className="text-[11px] leading-5 text-[var(--muted)]">Dosya yükleme alanı belge depolama servisi bağlandığında aktif edilebilir. Şu an bu kartlar yükleme işlevi sunmaz.</p></div>;
  return <Field label="Notlar"><TextInput value={form.notes} onChange={(event) => update("notes", event.target.value)} placeholder="Varlıkla ilgili ek bilgiler"/></Field>;
}
...
function ProductRow({ product }: { product: InventoryProduct }) {
  ...
  return <div className="flex items-center gap-4 px-5 py-4 transition hover:bg-[var(--surface-2)]/35"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[12px] bg-[var(--surface-2)] text-[var(--muted)]"><Icon name="box"/></span><div className="min-w-0 flex-1"><div className="truncate text-[13px] font-semibold text-[var(--ink)]">{product.name}</div><div className="mt-1 truncate text-[10px] text-[var(--muted)]">{product.categoryName || "Kategorisiz"}{product.sku ? ` · SKU ${product.sku}` : ""}{product.barcode ? ` · ${product.barcode}` : ""}</div></div><div className="hidden min-w-[100px] text-right md:block"><div className="text-[12px] font-semibold text-[var(--ink)]">{formatInventoryQuantity(product.quantity)} {inventoryUnitLabel(product.unit)}</div><div className="mt-1 text-[10px] text-[var(--muted-soft)]">min {formatInventoryQuantity(product.minimumQuantity)}</div></div><span className={`rounded-full px-2.5 py-1 text-[9px] font-semibold ${critical ? "bg-[var(--warning-soft)] text-[var(--warning)]" : "bg-[var(--success-soft)] text-[var(--success)]"}`}>{critical ? "Kritik" : "Normal"}</span></div>;
}

function AssetGrid({ assets }: { assets: InventoryAsset[] }) {
  return <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{assets.map((item) => <article key={item.id} className="rounded-[22px] border border-[var(--line)] bg-[var(--surface)] p-5 shadow-[0_8px_24px_rgba(17,70,104,.035)]"><div className="flex items-start justify-between gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-[12px] bg-[var(--success-soft)] text-[var(--success)]"><Icon name="asset"/></span><span className="rounded-full bg-[var(--success-soft)] px-2.5 py-1 text-[9px] font-semibold text-[var(--success)]">{item.status}</span></div><h3 className="mt-4 text-[15px] font-semibold text-[var(--ink)]">{item.name}</h3><p className="mt-1 text-[10px] text-[var(--muted)]">{item.assetCode} · {item.categoryName || item.assetType}</p><div className="mt-5 grid grid-cols-2 gap-3 text-[11px]"><Info label="Seri no" value={item.serialNumber || "—"}/><Info label="Lokasyon" value={item.branchName || "Merkez"}/><Info label="Satın alma" value={item.purchasePrice ? formatInventoryMoney(item.purchasePrice, item.currency) : "—"}/><Info label="Garanti" value={item.warrantyEnd ? dateLabel(item.warrantyEnd) : "—"}/></div><div className="mt-4 flex items-center gap-2 border-t border-[var(--line)] pt-4 text-[10px] text-[var(--muted)]"><Icon name="clock"/> Sonraki bakım: {item.nextMaintenanceAt ? dateLabel(item.nextMaintenanceAt) : "planlanmadı"}</div></article>)}{!assets.length ? <div className="col-span-full rounded-[22px] border border-dashed border-[var(--line)] p-12 text-center text-[12px] text-[var(--muted)]">Henüz envanter varlığı yok. “Yeni Envanter” ile ilk kaydı oluşturun.</div> : null}</section>;
}
...
function SupplierPanel({ suppliers, onAdd }: { suppliers: InventorySupplier[]; onAdd: () => void }) {
  return <section className="rounded-[22px] border border-[var(--line)] bg-[var(--surface)] p-5"><div className="flex items-center justify-between"><div><h2 className="text-[15px] font-semibold text-[var(--ink)]">Tedarikçiler</h2><p className="mt-1 text-[11px] text-[var(--muted)]">Fiyat, süre ve satın alma ilişkilerinin temeli.</p></div><Button onClick={onAdd}><Icon name="plus"/>Yeni tedarikçi</Button></div><div className="mt-5 divide-y divide-[var(--line)]">{suppliers.map((supplier) => <div key={supplier.id} className="flex items-center gap-4 py-4"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--accent-soft)] text-[11px] font-semibold text-[var(--accent)]">{supplier.name.slice(0, 2).toUpperCase()}</span><div className="min-w-0 flex-1"><p className="text-[12px] font-semibold text-[var(--ink)]">{supplier.name}</p><p className="mt-1 text-[10px] text-[var(--muted)]">{supplier.contactName || "Yetkili belirtilmedi"}{supplier.phone ? ` · ${supplier.phone}` : ""}</p></div><span className="hidden text-[10px] text-[var(--muted)] sm:block">{supplier.email || "E-posta yok"}</span></div>)}</div></section>;
}
