...
function ProductFormContent({ tab, form, update, categories, suppliers }: { tab: ProductTab; form: ProductFormState; update: <K extends keyof ProductFormState>(key: K, value: ProductFormState[K]) => void; categories: InventoryCategory[]; suppliers: InventorySupplier[] }) {
  if (tab === "Genel") return <div className="grid gap-5 md:grid-cols-2"><Field label="Ürün adı" required><TextInput value={form.name} onChange={(event) => update("name", event.target.value)} placeholder="Tek kullanımlık eldiven" required/></Field><Field label="Stok kodu"><TextInput value={form.sku} onChange={(event) => update("sku", event.target.value)} placeholder="SER-001"/></Field><Field label="Barkod"><TextInput value={form.barcode} onChange={(event) => update("barcode", event.target.value)} placeholder="8691234567890"/></Field><Field label="Marka"><TextInput value={form.brand} onChange={(event) => update("brand", event.target.value)} placeholder="Marka"/></Field><Field label="Üretici"><TextInput value={form.manufacturer} onChange={(event) => update("manufacturer", event.target.value)} placeholder="Üretici Firma"/></Field><Field label="Model / seri"><TextInput value={form.model} onChange={(event) => update("model", event.target.value)} placeholder="Model"/></Field><Field label="Ürün birimi"><SelectNative value={form.unit} onValueChange={(value) => update("unit", value)}>{INVENTORY_UNITS.map((unit) => <option key={unit} value={unit}>{inventoryUnitLabel(unit)}</option>)}</SelectNative></Field><Field label="Paket içeriği"><TextInput type="number" value={form.packageQuantity} onChange={(event) => update("packageQuantity", event.target.value)} placeholder="30"/></Field><Field label="Menşei"><TextInput value={form.originCountry} onChange={(event) => update("originCountry", event.target.value)} placeholder="Türkiye"/></Field><Field label="Açıklama"><TextInput value={form.description} onChange={(event) => update("description", event.target.value)} placeholder="Ürün açıklaması"/></Field></div>;
  if (tab === "Kategori") return <div className="space-y-5"><div className="grid gap-4 md:grid-cols-2"><Field label="Kategori" required><SelectNative value={form.categoryId} onValueChange={(value) => update("categoryId", value)}><option value="">Kategori seçin</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}{category.code ? ` · ${category.code}` : ""}</option>)}</SelectNative></Field><Field label="Etiket / alt kategori"><TextInput placeholder="Tek kullanımlık ürünler"/></Field></div><Hint>Kategori ağacı ürünlerin raporlama, satın alma ve stok politikalarında temel alınır. Örnek: Sarf malzemeleri → Tek kullanımlık ürünler.</Hint></div>;
  ...
  if (tab === "Tedarik") return <div className="space-y-5"><div className="grid gap-4 md:grid-cols-2"><Field label="Ana tedarikçi"><SelectNative value={form.supplierId} onValueChange={(value) => update("supplierId", value)}><option value="">Tedarikçi seçin</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</SelectNative></Field><Field label="Tedarikçi ürün kodu"><TextInput value={form.supplierProductCode} onChange={(event) => update("supplierProductCode", event.target.value)} placeholder="ABC-93821"/></Field><Field label="Birim alış fiyatı"><TextInput type="number" value={form.supplierUnitCost} onChange={(event) => update("supplierUnitCost", event.target.value)} placeholder="420"/></Field><Field label="Minimum sipariş"><TextInput type="number" value={form.supplierMinimumOrderQuantity} onChange={(event) => update("supplierMinimumOrderQuantity", event.target.value)} placeholder="10"/></Field><Field label="Sipariş katı"><TextInput type="number" value={form.supplierOrderMultiple} onChange={(event) => update("supplierOrderMultiple", event.target.value)} placeholder="5"/></Field><Field label="Tedarik süresi (gün)"><TextInput type="number" value={form.supplierLeadTimeDays} onChange={(event) => update("supplierLeadTimeDays", event.target.value)} placeholder="7"/></Field><Field label="Hazırlık süresi (gün)"><TextInput type="number" value={form.supplierPreparationDays} onChange={(event) => update("supplierPreparationDays", event.target.value)} placeholder="2"/></Field><Field label="Kargo süresi (gün)"><TextInput type="number" value={form.supplierShippingDays} onChange={(event) => update("supplierShippingDays", event.target.value)} placeholder="3"/></Field></div><Hint>Toplam teslim süresi tedarik, hazırlık ve kargo sürelerinin toplamıdır. Sistem tüketim hızını ve güvenlik stoğunu kullanarak sipariş önerisi oluşturabilir.</Hint></div>;
  if (tab === "Finans") return <div className="grid gap-5 md:grid-cols-2"><Field label="Alış fiyatı"><TextInput type="number" value={form.purchasePrice} onChange={(event) => update("purchasePrice", event.target.value)} placeholder="420"/></Field><Field label="Satış fiyatı"><TextInput type="number" value={form.salePrice} onChange={(event) => update("salePrice", event.target.value)} placeholder="750"/></Field><Field label="KDV"><TextInput type="number" value={form.taxRate} onChange={(event) => update("taxRate", event.target.value)} placeholder="20"/></Field><Field label="Para birimi"><SelectNative value={form.currency} onValueChange={(value) => update("currency", value)}><option value="TRY">Türk Lirası</option><option value="EUR">Avro</option><option value="USD">Amerikan Doları</option></SelectNative></Field><div className="md:col-span-2"><Toggle label="İade edilebilir" value={form.returnable} onChange={(value) => update("returnable", value)}/></div></div>;
  if (tab === "Parti Ve Son Kullanma") return <div className="grid gap-5 md:grid-cols-2"><Toggle label="Parti ve son kullanma takibi aktif" value={form.trackExpiry} onChange={(value) => update("trackExpiry", value)}/><Field label="Parti numarası"><TextInput value={form.lotNumber} onChange={(event) => update("lotNumber", event.target.value)} placeholder="PARTİ-2026-0831"/></Field><Field label="Üretim tarihi"><DatePicker value={form.manufacturedAt} onChange={(value) => update("manufacturedAt", value)} ariaLabel="Üretim tarihi" /></Field><Field label="Son kullanma tarihi"><DatePicker value={form.expiresAt} min={form.manufacturedAt || undefined} onChange={(value) => update("expiresAt", value)} ariaLabel="Son kullanma tarihi" /></Field><div className="md:col-span-2"><Hint tone="warning">Son kullanma tarihi en yakın ürünün önce tüketilmesi hedeflenir. Uyarılar sistem kurallarına göre oluşturulur.</Hint></div></div>;
  return <div className="space-y-5"><Hint>Bu ürün hizmet tamamlandığında stok tüketimiyle ilişkilendirilebilir. Gerçek tüketim kayıtları sistem tarafından ayrı olarak izlenir.</Hint><Field label="İç kullanım notu"><TextInput value={form.serviceNotes} onChange={(event) => update("serviceNotes", event.target.value)} placeholder="Örnek: Her işlemde 5 mililitre kullanılır"/></Field></div>;
}
...
function AssetFormContent({ tab, form, update, categories, suppliers, warehouses, activeBranchId }: { tab: AssetTab; form: AssetFormState; update: <K extends keyof AssetFormState>(key: K, value: AssetFormState[K]) => void; categories: InventoryCategory[]; suppliers: InventorySupplier[]; warehouses: InventoryWarehouse[]; activeBranchId: string | null }) {
  ...
  if (tab === "Genel") return <div className="grid gap-5 md:grid-cols-2"><Field label="Varlık adı" required><TextInput value={form.name} onChange={(event) => update("name", event.target.value)} placeholder="Operasyon cihazı" required/></Field><Field label="Varlık kodu" required><TextInput value={form.assetCode} onChange={(event) => update("assetCode", event.target.value)} placeholder="VAR-00124" required/></Field><Field label="Varlık tipi"><SelectNative value={form.assetType} onValueChange={(value) => update("assetType", value)}>{["EQUIPMENT", "FURNITURE", "IT", "VEHICLE", "OTHER"].map((value) => <option key={value} value={value}>{inventoryAssetTypeLabel(value)}</option>)}</SelectNative></Field><Field label="Kategori"><SelectNative value={form.categoryId} onValueChange={(value) => update("categoryId", value)}><option value="">Kategori seçin</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</SelectNative></Field><Field label="Marka"><TextInput value={form.brand} onChange={(event) => update("brand", event.target.value)} placeholder="Marka"/></Field><Field label="Model"><TextInput value={form.model} onChange={(event) => update("model", event.target.value)} placeholder="Model"/></Field><Field label="Seri numarası"><TextInput value={form.serialNumber} onChange={(event) => update("serialNumber", event.target.value)} placeholder="SERİ-839292"/></Field><Field label="Durum"><SelectNative value={form.status} onValueChange={(value) => update("status", value)}>{["ACTIVE", "INACTIVE", "MAINTENANCE", "RETIRED"].map((value) => <option key={value} value={value}>{inventoryAssetStatusLabel(value)}</option>)}</SelectNative></Field><Field label="Fiziksel durum"><SelectNative value={form.condition} onValueChange={(value) => update("condition", value)}>{["GOOD", "FAIR", "POOR", "BROKEN"].map((value) => <option key={value} value={value}>{inventoryAssetConditionLabel(value)}</option>)}</SelectNative></Field></div>;
  if (tab === "Satın Alma") return <div className="grid gap-5 md:grid-cols-2"><Field label="Satın alma tarihi"><DatePicker value={form.purchaseDate} onChange={(value) => update("purchaseDate", value)} ariaLabel="Satın alma tarihi" /></Field><Field label="Tedarikçi"><SelectNative value={form.supplierId} onValueChange={(value) => update("supplierId", value)}><option value="">Tedarikçi seçin</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</SelectNative></Field><Field label="Fatura no"><TextInput value={form.invoiceNumber} onChange={(event) => update("invoiceNumber", event.target.value)} placeholder="FAT-83921"/></Field><Field label="Satın alma bedeli"><TextInput type="number" value={form.purchasePrice} onChange={(event) => update("purchasePrice", event.target.value)} placeholder="185000"/></Field><Field label="Para birimi"><SelectNative value={form.currency} onValueChange={(value) => update("currency", value)}><option value="TRY">Türk Lirası</option><option value="EUR">Avro</option><option value="USD">Amerikan Doları</option></SelectNative></Field></div>;
  if (tab === "Konum Ve Zimmet") return <div className="grid gap-5 md:grid-cols-2"><Field label="Şube"><SelectNative value={activeBranchId ?? form.branchId} disabled={Boolean(activeBranchId)} onValueChange={(value) => update("branchId", value)}>{activeBranchId ? null : <option value="">Merkez / Ortak</option>}{branchWarehouses.map((warehouse) => <option key={warehouse.branchId ?? warehouse.id} value={warehouse.branchId ?? ""}>{warehouse.name.replace(" Stok", "")}</option>)}</SelectNative></Field><Field label="Depo / Konum"><SelectNative value={form.warehouseId} onValueChange={(value) => update("warehouseId", value)}><option value="">Seçin</option>{locationWarehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</SelectNative></Field><Field label="Zimmetli personel"><TextInput placeholder="Personel zimmet işlemlerinden yönetilir" disabled/></Field><Hint>{activeBranchId ? "Aktif Şube Seçiliyken Envanter Varlığı Yalnız Bu Şubeye Ve Şubenin Depolarına Kaydedilebilir." : "Şube Transferleri Ve Zimmet Geçmişi Varlık Kartında Ayrı Kayıtlarla İzlenir."}</Hint></div>;
  if (tab === "Garanti Ve Bakım") return <div className="grid gap-5 md:grid-cols-2"><Field label="Garanti başlangıcı"><DatePicker value={form.warrantyStart} max={form.warrantyEnd || undefined} onChange={(value) => update("warrantyStart", value)} ariaLabel="Garanti başlangıcı" /></Field><Field label="Garanti bitişi"><DatePicker value={form.warrantyEnd} min={form.warrantyStart || undefined} onChange={(value) => update("warrantyEnd", value)} ariaLabel="Garanti bitişi" /></Field><Field label="Bakım aralığı (gün)"><TextInput type="number" value={form.maintenanceIntervalDays} onChange={(event) => update("maintenanceIntervalDays", event.target.value)} placeholder="180"/></Field><Field label="Sonraki bakım"><DatePicker value={form.nextMaintenanceAt} onChange={(value) => update("nextMaintenanceAt", value)} ariaLabel="Sonraki bakım tarihi" /></Field><div className="md:col-span-2"><Hint>Garanti bitişi ve yaklaşan bakım tarihleri sistem kurallarına göre izlenir.</Hint></div></div>;
  if (tab === "Belgeler") return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><DocTile title="Fatura"/><DocTile title="Garanti belgesi"/><DocTile title="Servis sözleşmesi"/></div><p className="text-[11px] leading-5 text-[var(--muted)]">Belge yükleme özelliği etkinleştirildiğinde dosyalar bu alandan yönetilebilir. Şu anda bu kartlar yalnızca bilgilendirme amaçlıdır.</p></div>;
