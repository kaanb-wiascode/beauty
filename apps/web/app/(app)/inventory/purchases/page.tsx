...
export default function PurchasesPage() {
  ...
  const openOrderCount = useMemo(
    () => orders.filter((row) => row.status !== "RECEIVED" && row.status !== "CANCELLED").length,
    [orders],
  );
  ...
  async function submitApproval(id: string) {
    ...
    try {
      ...
      setError(requestError instanceof ApiError ? requestError.message : "Onay akışı başlatılamadı.");
    } finally {
      setBusyId(null);
    }
  }
  ...
  async function orderPurchaseOrder(id: string) {
    ...
    try {
      ...
      setError(requestError instanceof ApiError ? requestError.message : "Satın alma siparişi ilerletilemedi.");
    } finally {
      setBusyId(null);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-[1440px] space-y-6 pb-10">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="mb-2 text-[11px] font-semibold uppercase tracking-[.14em] text-[var(--muted-soft)]">SATIN ALMA</p>
          <h1 className="text-[30px] font-semibold tracking-[-.035em] text-[var(--ink)]">Satın Alma Operasyonu</h1>
          <p className="mt-1 text-[14px] text-[var(--muted)]">
            Stok ihtiyaçlarını talepten siparişe ve mal kabule kadar aynı operasyon yüzeyinde izleyin.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link href="/inventory/supplier-network" className="inline-flex min-h-10 items-center justify-center rounded-[14px] bg-white/70 px-4 py-2.5 text-[13px] font-medium text-[var(--ink)] shadow-[inset_0_0_0_1px_var(--line)] transition-colors hover:bg-white">
            Tedarikçi ağı
          </Link>
          <Link href="/inventory" className="inline-flex min-h-10 items-center justify-center rounded-[14px] bg-[var(--surface-2)] px-4 py-2.5 text-[13px] font-medium text-[var(--muted)] transition-colors hover:text-[var(--ink)]">
            Envantere dön
          </Link>
        </div>
      </header>
      ...
      {!canWrite ? (
        <Alert tone="success">Bu görünüm salt okunur. Satın alma aksiyonları için inventory.write izni gerekir.</Alert>
      ) : null}
      ...
      <DataView>
        <div className="flex flex-wrap gap-2 border-b border-[var(--line)] px-4 pt-4">
          <button type="button" onClick={() => changeView("requests")} className={`rounded-t-[12px] border-b-2 px-4 py-2.5 text-[12px] font-semibold transition ${view === "requests" ? "border-[var(--accent)] text-[var(--accent)]" : "border-transparent text-[var(--muted)] hover:text-[var(--ink)]"}`}>
            Satın alma talepleri · {requests.length}
          </button>
          <button type="button" onClick={() => changeView("orders")} className={`rounded-t-[12px] border-b-2 px-4 py-2.5 text-[12px] font-semibold transition ${view === "orders" ? "border-[var(--accent)] text-[var(--accent)]" : "border-transparent text-[var(--muted)] hover:text-[var(--ink)]"}`}>
        ...
        <DataViewToolbar
          search={
            <SearchField
              value={search}
              placeholder={view === "requests" ? "Ürün, lokasyon veya neden ara..." : "Tedarikçi, lokasyon veya durum ara..."}
          ...
          actions={
            <ToolbarSelect value={status} onChange={(event) => setStatus(event.target.value)} aria-label="Satın alma durumu">
          ...
          filters={
            <>
              <FilterChip active={!status} count={totalCount} onClick={() => setStatus("")}>Tümü</FilterChip>
              {view === "requests" ? (
                <FilterChip active={status === "PENDING"} count={pendingRequestCount} onClick={() => setStatus("PENDING")}>Onay bekleyen</FilterChip>
              ) : (
                <FilterChip active={status === "ORDERED"} count={orders.filter((row) => row.status === "ORDERED").length} onClick={() => setStatus("ORDERED")}>Siparişte</FilterChip>
              )}
            </>
          }
        />
        ...
        {view === "requests" ? (
          <RequestList rows={visibleRequests} canWrite={canWrite} busyId={busyId} onApprove={approveRequest} onConvert={openConvert} />
        ) : (
          <OrderList
            rows={visibleOrders}
            canWrite={canWrite}
        ...
        {!visibleCount ? (
          <div className="px-5 py-14 text-center">
            <p className="text-[13px] font-medium text-[var(--ink)]">Eşleşen satın alma kaydı yok.</p>
            <p className="mt-1 text-[11px] text-[var(--muted)]">Arama veya durum filtresini değiştirerek tekrar deneyin.</p>
          </div>
        ) : null}
      ...
      <Modal
        ...
        description={convertTarget ? `${convertTarget.productName} için tedarikçi ve birim maliyet seçin.` : undefined}
      >
        <form className="space-y-4" onSubmit={convertRequest}>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Birim maliyet" required>
              <TextInput type="number" min="0" step="0.01" inputMode="decimal" value={unitCost} onChange={(event) => setUnitCost(event.target.value)} placeholder="0,00" required />
            </Field>
          </div>
          <Field label="Not">
            <TextInput value={note} maxLength={500} onChange={(event) => setNote(event.target.value)} placeholder="Opsiyonel satın alma notu" />
          </Field>
          {convertTarget ? (
            <div className="rounded-[16px] bg-[var(--surface-2)] px-4 py-3 text-[11px] leading-5 text-[var(--muted)]">
              ...
              {unitCost && Number.isFinite(Number(unitCost)) ? formatMoney(Number(unitCost) * Number(convertTarget.requestedQuantity || 0)) : "—"}
            </div>
          ) : null}
          <div className="flex justify-end gap-3 pt-2">
            <Button variant="secondary" onClick={closeConvert} disabled={Boolean(busyId)}>Vazgeç</Button>
            <Button type="submit" disabled={Boolean(busyId) || !suppliers.length}>{busyId ? "Dönüştürülüyor..." : "Sipariş oluştur"}</Button>
          </div>
        </form>
      </Modal>
    </div>
    ...
    <>
      <div className="hidden md:block">
        <div className="grid grid-cols-[1.4fr_.9fr_.6fr_.6fr_.8fr_auto] border-b border-[var(--line)] bg-[var(--surface-2)]/40 px-5 py-3 text-[10px] font-semibold uppercase tracking-[.08em] text-[var(--muted-soft)]">
          <span>Ürün</span><span>Lokasyon</span><span>Mevcut</span><span>Talep</span><span>Durum</span><span>Aksiyon</span>
        </div>
        <div className="divide-y divide-[var(--line)]">
          {rows.map((request) => (
            <div key={request.id} className="grid grid-cols-[1.4fr_.9fr_.6fr_.6fr_.8fr_auto] items-center gap-3 px-5 py-4">
              <div className="min-w-0">
                <div className="truncate text-[13px] font-semibold text-[var(--ink)]">{request.productName}</div>
                <div className="mt-1 truncate text-[10px] text-[var(--muted-soft)]">{request.reason || "Stok seviyesi düşük"}</div>
              </div>
              <span className="text-[12px] text-[var(--muted)]">{request.warehouseName}</span>
              <span className="text-[12px] text-[var(--muted)]">{formatQuantity(request.currentQuantity)}</span>
              <span className="text-[12px] font-semibold text-[var(--ink)]">{formatQuantity(request.requestedQuantity)}</span>
              <StatusBadge status={request.status} />
              <RequestAction request={request} canWrite={canWrite} busy={busyId === request.id} disabled={Boolean(busyId) && busyId !== request.id} onApprove={onApprove} onConvert={onConvert} />
            </div>
          ))}
        </div>
      </div>
        ...
        {rows.map((request) => (
          <article key={request.id} className="space-y-3 px-4 py-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0"><p className="truncate text-[13px] font-semibold text-[var(--ink)]">{request.productName}</p><p className="mt-1 text-[11px] text-[var(--muted)]">{request.warehouseName}</p></div>
              <StatusBadge status={request.status} />
            </div>
            <div className="grid grid-cols-2 gap-3 rounded-[12px] bg-[var(--surface-2)]/55 p-3"><Quantity label="Mevcut" value={request.currentQuantity} /><Quantity label="Talep" value={request.requestedQuantity} accent /></div>
            <p className="text-[10px] leading-5 text-[var(--muted)]">{request.reason || "Stok seviyesi düşük"}</p>
            <RequestAction request={request} canWrite={canWrite} busy={busyId === request.id} disabled={Boolean(busyId) && busyId !== request.id} onApprove={onApprove} onConvert={onConvert} />
          </article>
        ))}
      </div>
    ...
    <>
      <div className="hidden md:block">
        <div className="grid grid-cols-[1.2fr_.9fr_.5fr_.75fr_.8fr_auto] border-b border-[var(--line)] bg-[var(--surface-2)]/40 px-5 py-3 text-[10px] font-semibold uppercase tracking-[.08em] text-[var(--muted-soft)]">
          <span>Tedarikçi</span><span>Lokasyon</span><span>Kalem</span><span>Tutar</span><span>Durum</span><span>Aksiyon</span>
        </div>
        <div className="divide-y divide-[var(--line)]">
          {rows.map((order) => (
            <div key={order.id} className="grid grid-cols-[1.2fr_.9fr_.5fr_.75fr_.8fr_auto] items-center gap-3 px-5 py-4">
              <div className="min-w-0"><p className="truncate text-[13px] font-semibold text-[var(--ink)]">{order.supplierName || "Tedarikçi seçilmedi"}</p><p className="mt-1 font-mono text-[10px] text-[var(--muted-soft)]">{shortId(order.id)}</p></div>
              <span className="text-[12px] text-[var(--muted)]">{order.warehouseName}</span>
              <span className="text-[12px] text-[var(--muted)]">{order.itemCount}</span>
              <span className="text-[12px] font-semibold text-[var(--ink)]">{formatMoney(order.totalAmount)}</span>
              <div><StatusBadge status={order.status} /><p className="mt-1.5 text-[9px] text-[var(--muted-soft)]">{order.receivedAt ? `Teslim ${formatDate(order.receivedAt)}` : order.orderedAt ? `Sipariş ${formatDate(order.orderedAt)}` : "Henüz sipariş edilmedi"}</p></div>
              <OrderAction order={order} canWrite={canWrite} busy={busyId === order.id} disabled={Boolean(busyId) && busyId !== order.id} onSubmitApproval={onSubmitApproval} onOrder={onOrder} onOpenApproval={onOpenApproval} onOpenReceipt={onOpenReceipt} />
            </div>
          ))}
        </div>
      </div>
      <div className="divide-y divide-[var(--line)] md:hidden">
        {rows.map((order) => (
          <article key={order.id} className="space-y-3 px-4 py-4">
            <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-[13px] font-semibold text-[var(--ink)]">{order.supplierName || "Tedarikçi seçilmedi"}</p><p className="mt-1 text-[11px] text-[var(--muted)]">{order.warehouseName}</p></div><StatusBadge status={order.status} /></div>
            <div className="grid grid-cols-2 gap-3 rounded-[12px] bg-[var(--surface-2)]/55 p-3"><div><p className="text-[9px] uppercase tracking-[.08em] text-[var(--muted-soft)]">Kalem</p><p className="mt-1 text-[12px] font-semibold text-[var(--ink)]">{order.itemCount}</p></div><div><p className="text-[9px] uppercase tracking-[.08em] text-[var(--muted-soft)]">Tutar</p><p className="mt-1 text-[12px] font-semibold text-[var(--accent)]">{formatMoney(order.totalAmount)}</p></div></div>
            <p className="text-[10px] text-[var(--muted-soft)]">{order.receivedAt ? `Teslim: ${formatDate(order.receivedAt)}` : order.orderedAt ? `Sipariş: ${formatDate(order.orderedAt)}` : "Henüz sipariş edilmedi"}</p>
            <OrderAction order={order} canWrite={canWrite} busy={busyId === order.id} disabled={Boolean(busyId) && busyId !== order.id} onSubmitApproval={onSubmitApproval} onOrder={onOrder} onOpenApproval={onOpenApproval} onOpenReceipt={onOpenReceipt} />
          </article>
        ))}
      </div>
...
function OrderAction({ order, canWrite, busy, disabled, onSubmitApproval, onOrder, onOpenApproval, onOpenReceipt }: { order: PurchaseOrder; canWrite: boolean; busy: boolean; disabled: boolean; onSubmitApproval: (id: string) => Promise<void>; onOrder: (id: string) => Promise<void>; onOpenApproval: (order: PurchaseOrder) => void; onOpenReceipt: (order: PurchaseOrder) => void; }) {
  if (!canWrite) {
    if (order.status === "PENDING") return <Button variant="ghost" className="min-h-8 px-3 py-1.5 text-[11px]" onClick={() => onOpenApproval(order)}>Onay akışı</Button>;
  ...
  if (order.status === "PENDING") return <Button variant="secondary" className="min-h-8 px-3 py-1.5 text-[11px]" disabled={busy || disabled} onClick={() => onOpenApproval(order)}>Onay akışı</Button>;
