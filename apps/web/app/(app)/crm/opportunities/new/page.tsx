...
export default function NewCustomerOpportunityPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      if (!customerId) {
        setError("Fırsat oluşturmak için bir müşteri seçilmelidir.");
        setLoading(false);
        return;
      }
  ...
  const customerDescription = useMemo(() => {
    if (!customer) return "Mevcut müşteri için CRM satış fırsatı oluşturun.";
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <PageHeader
        ...
        action={customer ? <Link href={`/customers/${customer.id}`}><Button variant="secondary">Müşteriye dön</Button></Link> : <Link href="/customers"><Button variant="secondary">Müşteriler</Button></Link>}
      />

      {!canManageCrm ? <Alert>Satış fırsatı oluşturmak için CRM yönetim yetkisi gereklidir.</Alert> : null}
      {error ? <Alert>{error}</Alert> : null}

      {customer && canManageCrm ? (
        <GlassCard>
