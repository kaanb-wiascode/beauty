...
export default function CrmConversationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [rows, people] = await Promise.all([
        api<Thread[]>(`/crm/conversations?limit=150&mode=${teamMode}`),
        canManage ? api<Assignee[]>("/crm/assignees") : Promise.resolve([]),
      ]);
      setThreads(rows);
      setAssignees(people);
      setSelected((current) => current ? rows.find((row) => row.subjectType === current.subjectType && row.subjectId === current.subjectId) ?? null : null);
    } catch (requestError) {
  ...
  async function setAssignment(userId: string) {
    ...
    try {
      ...
      setThreads((current) => current.map((item) => item.subjectType === selected.subjectType && item.subjectId === selected.subjectId ? { ...item, ...patch } : item));
  ...
  return <div className="space-y-6">
    <PageHeader title="Birleşik CRM Inbox" description="WhatsApp, SMS ve e-posta konuşmalarını müşteri, lead ve fırsat bazında ekip sahipliğiyle yönetin." action={<Button variant="secondary" onClick={() => void load()}>Yenile</Button>} />
    ...
    <div className="flex flex-wrap gap-2">
      <Select value={teamMode} onChange={(event) => setTeamMode(event.target.value as TeamMode)} className="max-w-[200px]">
    ...
    {loading ? <Spinner label="Konuşmalar yükleniyor..." /> : <div className="grid min-h-[560px] gap-4 xl:grid-cols-[430px_minmax(0,1fr)]">
      ...
      <section className="overflow-hidden rounded-[22px] border border-[var(--line)] bg-white shadow-[var(--shadow-soft)]">
        {!selected ? <div className="p-6"><EmptyState title="Bir konuşma seçin" description="Mesaj geçmişini görmek için soldaki konuşmalardan birini açın." /></div> : <>
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--line)] px-5 py-4"><div><h2 className="text-[14px] font-semibold">{selected.subjectLabel}</h2><p className="mt-1 text-[9px] text-[var(--muted)]">{subjectLabel[selected.subjectType]} · {selected.channels.map((item) => channelLabel[item]).join(" / ")}</p></div><div className="flex flex-wrap items-center gap-2">{canManage ? <Select disabled={assigning} value={selected.assignedUserId ?? ""} onChange={(event) => void setAssignment(event.target.value)} className="min-w-[180px]"><option value="">Atanmamış</option>{assignees.map((item) => <option key={item.id} value={item.id}>{assigneeName(item)}</option>)}</Select> : selected.assignedUserName ? <span className="text-[10px] text-[var(--muted)]">Sahibi: {selected.assignedUserName}</span> : null}<Link href={hrefFor(selected)}><Button variant="secondary">CRM Kaydını Aç</Button></Link></div></div>
