...
const webhookEventLabels: Record<WebhookEvent["eventType"], string> = { DELIVERY: "Teslimat bildirimi", INBOUND: "Gelen mesaj" };
const webhookOutcomeLabels: Record<WebhookEvent["outcome"], string> = { PROCESSED: "İşlendi", IGNORED: "İşleme alınmadı", FAILED: "Başarısız" };
...
export default function CrmCommunicationsPage() {
  ...
  return <div className="space-y-6">
    <PageHeader title="CRM İletişim Merkezi" description="WhatsApp, SMS ve e-posta temaslarını şube bazında tek zaman çizelgesinde izleyin." action={<Button variant="secondary" onClick={() => void load()}>Yenile</Button>} />
    ...
    {!canRead ? <Alert>CRM iletişim kayıtlarını görüntülemek için CRM görüntüleme yetkisi gerekir.</Alert> : null}
    {error ? <Alert>{error}</Alert> : null}
    {activeBranch && !providers.providers.length ? <Alert>Harici mesaj sağlayıcısı henüz bağlı değil. Müşteri profillerinden manuel iletişim kaydı tutulabilir; sağlayıcı bağlantısı kurulmadan sistem mesajı gönderilmiş sayılmaz.</Alert> : null}
      ...
      <section className="overflow-hidden rounded-[22px] border border-[var(--line)] bg-white shadow-[var(--shadow-soft)]">
        <div className="divide-y divide-[var(--line)]">{filtered.map((message) => (
          <article key={message.id} className="grid gap-3 px-5 py-4 md:grid-cols-[100px_100px_minmax(0,1fr)_170px_auto] md:items-start">
            ...
            <div><p className="text-[10px] font-semibold">{message.direction === "INBOUND" ? "Gelen" : "Giden"}</p><p className="mt-1 text-[9px] text-[var(--muted)]">{userLabel(message.status)}</p></div>
            <div className="min-w-0"><p className="truncate text-[11px] font-semibold">{message.subject || message.recipient}</p><p className="mt-1 line-clamp-2 text-[11px] leading-5 text-[var(--muted)]">{message.body}</p>{message.errorMessage ? <p className="mt-1 text-[9px] text-[#9c513f]">{userErrorMessage(message.errorMessage, "Mesaj gönderilemedi.")}</p> : null}</div>
    ...
    {!loading && activeBranch ? <GlassCard className="p-0">
      <div className="border-b border-[var(--line)] px-5 py-4">
        <h2 className="text-[14px] font-semibold">Mesaj teslimat geçmişi</h2>
        <p className="mt-1 text-[10px] text-[var(--muted)]">Harici mesaj sağlayıcısından gelen mesaj ve teslimat bildirimlerini izleyin. Teknik iletişim verileri bu ekranda gösterilmez.</p>
      </div>
      {webhookEvents.length ? <div className="divide-y divide-[var(--line)]">{webhookEvents.map((event) => (
        <div key={event.id} className="grid gap-2 px-5 py-3 md:grid-cols-[120px_100px_110px_minmax(0,1fr)_160px] md:items-center">
          ...
          <span className="text-[10px] text-[var(--muted)]">{webhookEventLabels[event.eventType]}</span>
          <span className={event.outcome === "PROCESSED" ? "text-[10px] font-semibold text-[var(--accent)]" : "text-[10px] font-semibold text-[#9c513f]"}>{webhookOutcomeLabels[event.outcome]}</span>
          <div className="min-w-0"><p className="truncate text-[10px]">{event.messageId ? `Mesaj: ${event.messageId}` : event.externalMessageId || event.externalEventId}</p>{event.errorMessage ? <p className="mt-1 truncate text-[9px] text-[#9c513f]">{userErrorMessage(event.errorMessage, "Bildirim işlenemedi.")}</p> : event.messageStatus ? <p className="mt-1 text-[9px] text-[var(--muted)]">Mesaj durumu: {userLabel(event.messageStatus)}{event.channel ? ` · ${channelLabels[event.channel]}` : ""}</p> : null}</div>
          <time className="text-[9px] text-[var(--muted)]">{formatDate(event.receivedAt)}</time>
        </div>
      ))}</div> : <EmptyState title="Teslimat bildirimi kaydı yok" description="Bu şubede henüz işlenmiş mesaj veya teslimat bildirimi bulunmuyor." />}
    </GlassCard> : null}
  </div>;
}
