...
import { LeadContextFields,contextUpdatePayload,emptyLeadContext,type CommercialOptions,type LeadContextForm } from "@/components/crm/lead-context-fields";
...
export default function LeadContextPage({params}:{params:Promise<{id:string}>}){
 ...
 return <div className="space-y-6"><Link href={`/crm/leads/${id}`} className="inline-flex text-[11px] font-semibold text-[#1674BD]">← Potansiyel Müşteri Detayına Dön</Link><PageHeader title="Lead Bağlamı" description={`${lead.firstName} ${lead.lastName} için iletişim tercihleri, edinim attribution verileri, hizmet/paket ilgisi ve satış sınıflandırması.`}/>{error?<Alert onClose={()=>setError("")}>{error}</Alert>:null}<LeadContextSummary lead={lead} options={options}/><form onSubmit={submit} className="space-y-5 rounded-[22px] border border-[var(--line)] bg-white p-5 shadow-[var(--shadow-soft)]"><div><h2 className="text-[13px] font-semibold">Bağlamı Düzenle</h2><p className="mt-1 text-[10px] text-[var(--muted)]">Aşağıdaki alanlar satış ve edinim raporlamasında yapılandırılmış veri olarak kullanılır.</p></div><LeadContextFields value={form} onChange={setForm} commercialOptions={options}/>{canManage?<div className="flex justify-end"><Button type="submit" disabled={saving}>{saving?"Kaydediliyor...":"Bağlamı Kaydet"}</Button></div>:null}</form></div>;
}
