  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[.16em] text-[#1674BD]">Müşteri İlişkileri</p>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5" aria-label="Müşteri İlişkileri Metrikleri">
        {[
          ...
          <article key={String(label)} className="relative overflow-hidden rounded-[20px] border border-[var(--line)] bg-white p-4 shadow-[var(--shadow-soft)]">
            <span aria-hidden="true" className={index === 3 && Number(value) > 0 ? "absolute right-3 top-3 h-2 w-2 rounded-full bg-[#b76d58]" : "absolute right-3 top-3 h-2 w-2 rounded-full bg-[#55D4E1]"} />
            ...
            {pipelineStages.map((stage) => {
              ...
              return (
                <div key={stage} className="rounded-[16px] border border-[#dcebf3] bg-[#f8fcfd] p-3.5">
                  <p className="min-h-8 text-[10px] font-semibold uppercase tracking-[.08em] text-[#5f7280]">{opportunityStageLabels[stage]}</p>
        ...
        {leads.length ? (
          <div className="divide-y divide-[var(--line)]">
            {leads.slice(0, 6).map((lead) => (
              <Link key={lead.id} href={`/crm/leads/${lead.id}`} className="grid gap-2 px-5 py-4 transition-colors hover:bg-[#f8fcfd] sm:grid-cols-[minmax(0,1fr)_150px_160px] sm:items-center">
