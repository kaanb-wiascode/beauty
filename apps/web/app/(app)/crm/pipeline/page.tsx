...
export default function CrmPipelinePage() {
  ...
  const [transitioning, setTransitioning] = useState<CrmOpportunity | null>(
    null,
  );
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      weighted: open.reduce(
        (sum, row) =>
          sum + (Number(row.estimatedValue ?? 0) * row.probability) / 100,
        0,
      ),
    };
  }, [rows]);
  ...
  async function transition(event: FormEvent) {
    ...
    if (targetStage === "LOST" && !lostReason.trim()) {
      setError("Kaybedilen fırsat için neden gereklidir.");
      return;
    }
    setSaving(true);
    setError("");
    try {
      ...
      showToast("Fırsat aşaması güncellendi.", "success");
      await load();
    } catch (requestError) {
      setError(
        requestError instanceof ApiError
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Fırsatları kontrollü aşama geçişleriyle ilerletin; değer ve kazanma olasılığını birlikte izleyin."
      />
      {error && !transitioning ? (
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        {[
          ["Açık fırsat", totals.count],
          ["Toplam pipeline", formatMoney(totals.raw, "TRY")],
          ["Ağırlıklı değer", formatMoney(totals.weighted, "TRY")],
        ].map(([label, value]) => (
          <article
            key={String(label)}
            className="rounded-[20px] border border-[var(--line)] bg-white p-4 shadow-[var(--shadow-soft)]"
          >
            <p className="text-[10px] text-[var(--muted)]">{label}</p>
            <strong className="mt-2 block text-[22px] tracking-[-.04em]">
              {value}
            </strong>
          </article>
        ))}
      ...
      {loading ? (
        <Spinner label="Pipeline hazırlanıyor..." />
      ) : rows.length ? (
        <div className="grid items-start gap-4 xl:grid-cols-3 2xl:grid-cols-6">
          {stages.map((stage) => {
            ...
            return (
              <section
                key={stage}
                className="overflow-hidden rounded-[20px] border border-[var(--line)] bg-[#f8f7fb]"
              >
                <header className="flex items-center justify-between border-b border-[var(--line)] bg-white px-4 py-3">
                  ...
                  <span className="rounded-full bg-[#eee9ff] px-2 py-0.5 text-[10px] font-bold text-[#7052df]">
                    {stageRows.length}
                  </span>
                </header>
                <div className="space-y-3 p-3">
                  {stageRows.map((row) => (
                    <article
                      key={row.id}
                      className="rounded-[16px] border border-[#e8e5ee] bg-white p-3 shadow-[0_3px_14px_rgba(42,35,60,.05)]"
                    >
                      <Link
                        href={
                          row.leadId ? `/crm/leads/${row.leadId}` : "/crm/leads"
                        }
                        className="block text-[12px] font-semibold leading-5 hover:text-[#7052df]"
                      >
                        {row.title}
                      </Link>
                      ...
                      {canManage && nextStages[row.stage].length ? (
                        <Button
                          variant="ghost"
                          className="mt-3 min-h-8 w-full px-2 py-1 text-[10px]"
                  ...
                  {!stageRows.length ? (
                    <p className="py-7 text-center text-[10px] text-[var(--muted-soft)]">
        ...
        <EmptyState
          title="Pipeline boş"
          description="Lead havuzundan bir müşteri adayını nitelendirerek ilk fırsatı oluşturun."
          action={
            <Link href="/crm/leads">
              <Button>Lead havuzuna git</Button>
            </Link>
          }
        />
      )}
      ...
      >
        <form onSubmit={transition} className="space-y-4">
          ...
          <Field label="Yeni aşama" required>
            ...
            >
              {transitioning
                ? nextStages[transitioning.stage].map((stage) => (
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" disabled={saving}>
              {saving ? "Güncelleniyor..." : "Aşamayı güncelle"}
            </Button>
          </div>
        </form>
      </Modal>
