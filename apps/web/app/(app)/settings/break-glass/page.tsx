...
export default function BreakGlassPage() {
  ...
  const activate = async (event: FormEvent) => {
    ...
    try {
      ...
      setProofId(null);
      setError(e instanceof ApiError ? e.message : "Break-glass erişimi açılamadı.");
    } finally { setBusy(false); }
  };
  ...
  const revoke = async (id: string) => {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.message : "Emergency erişim kapatılamadı.");
    } finally { setBusy(false); }
  };
  ...
  return (
    <main className="mx-auto w-full max-w-[1180px] space-y-6 pb-10">
      <header className="border-b border-[var(--line)] pb-5">
        ...
        <h1 className="text-[28px] font-semibold tracking-[-.04em] text-[var(--ink)]">Break-Glass Erişimi</h1>
      ...
      {!proofId ? (
        <form onSubmit={reauthenticate} className="max-w-xl space-y-4 rounded-[var(--radius-card)] border border-[var(--line)] bg-[var(--surface)] p-5">
          ...
          <TextInput value={mfaCode} onChange={(e) => setMfaCode(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="6 haneli MFA kodu" required />
