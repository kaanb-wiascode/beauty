  ...
  return <main className="mx-auto w-full max-w-[1240px] space-y-6 pb-10">
    ...
    <div className="grid gap-5 lg:grid-cols-[300px_1fr]">
      ...
      <section className="space-y-4">{selectedRole ? <>
        ...
        <div className="grid gap-3 md:grid-cols-[1fr_240px]"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Yetki ara…" className="min-h-10 rounded-xl border border-[var(--line)] bg-white px-3 text-sm" /><ValooSelect value={resourceFilter} onChange={setResourceFilter} placeholder="Tüm modüller" searchPlaceholder="Modül ara…" options={[{value:"ALL",label:"Tüm modüller"},...resources.map((resource)=>({value:resource,label:RESOURCE_LABELS[resource]??resource}))]}/></div>
