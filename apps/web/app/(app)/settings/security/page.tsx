...
export default function SecuritySettingsPage() {
  ...
  const revokeAll = async () => {
    if (!window.confirm("Bu şirket hesabındaki tüm açık oturumlarınız kapatılacak. Devam edilsin mi?")) return;
    try {
      setRevokingAll(true);
  ...
  const startMfaSetup = async () => {
    try {
      ...
      showToast(error instanceof ApiError ? error.message : "İki aşamalı doğrulama kurulumu başlatılamadı.", "error");
    } finally {
      setSavingMfa(false);
    }
  };
  ...
  const confirmMfa = async () => {
    try {
      ...
      setMfaSetup(null);
      setMfaCode("");
      showToast("İki aşamalı doğrulama etkinleştirildi.");
      await loadSecurity();
    } catch (error) {
      showToast(error instanceof ApiError ? error.message : "İki aşamalı doğrulama tamamlanamadı.", "error");
    } finally {
      setSavingMfa(false);
    }
  };
  ...
  return (
    <main className="mx-auto w-full max-w-[1240px] space-y-6 pb-10">
      <header className="border-b border-[var(--line)] pb-5">
        ...
        <p className="mt-1 max-w-2xl text-sm text-[var(--muted)]">Oturumlarınızı, iki aşamalı doğrulamayı ve şirket güvenlik ayarlarını tek yerden yönetin.</p>
      </header>
      ...
      <section className="rounded-[var(--radius-card)] border border-[var(--line)] bg-[var(--surface)] p-5 shadow-[var(--shadow-soft)]">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h2 className="text-sm font-semibold text-[var(--ink)]">Kişisel İki Aşamalı Doğrulama</h2>
            <p className="mt-1 text-xs text-[var(--muted)]">Şirket genelinde zorunlu olmasa bile hesabınızı bir doğrulama uygulamasıyla koruyabilirsiniz.</p>
          </div>
          {!mfaStatus?.enrolled && !mfaSetup ? (
            <button type="button" disabled={savingMfa} onClick={() => void startMfaSetup()} className="rounded-lg bg-[var(--accent)] px-4 py-2 text-xs font-semibold text-white disabled:opacity-50">İki Aşamalı Doğrulamayı Kur</button>
          ) : null}
        </div>
        {mfaStatus?.enrolled ? (
          <div className="mt-4 rounded-xl border border-[var(--line)] bg-[var(--surface-2)] p-4 text-sm font-medium text-[var(--ink)]">İki aşamalı doğrulama etkin. Şirket ayarlarında zorunluysa sonraki girişlerde ek doğrulama kodu istenir.</div>
        ) : null}
        {mfaSetup ? (
          <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_220px]">
            <div className="rounded-xl border border-[var(--line)] bg-[var(--surface-2)] p-4">
              <p className="text-xs leading-5 text-[var(--muted)]">Doğrulama uygulamanıza aşağıdaki kurulum anahtarını ekleyin. Bu anahtar yalnızca kurulum sırasında gösterilir.</p>
              ...
              <a href={mfaSetup.otpauthUri} className="mt-3 inline-flex text-xs font-semibold text-[var(--accent)]">Doğrulama uygulamasında aç</a>
            </div>
            <div>
      ...
      <section className="rounded-[var(--radius-card)] border border-[var(--line)] bg-[var(--surface)] p-5 shadow-[var(--shadow-soft)]">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-[var(--muted)]">İki aşamalı doğrulama zorunluluğu giriş sırasında uygulanır; diğer ayarlar şirket güvenlik politikasının bir parçasıdır.</p>
          </div>
        ...
        {policy ? (
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <label className="rounded-xl border border-[var(--line)] p-4 text-xs text-[var(--muted)]"><span className="flex items-center justify-between gap-3"><span><strong className="block text-sm text-[var(--ink)]">İki Aşamalı Doğrulama Zorunlu</strong>Şirket hesabına girişte doğrulama uygulamasından ek kod ister.</span><input type="checkbox" checked={policy.requireMfa} onChange={(event) => setPolicy({ ...policy, requireMfa: event.target.checked })} /></span></label>
      ...
      <section className="overflow-hidden rounded-[var(--radius-card)] border border-[var(--line)] bg-[var(--surface)] shadow-[var(--shadow-soft)]">
        <div className="flex items-center justify-between border-b border-[var(--line)] px-5 py-4">
          <div><h2 className="text-sm font-semibold text-[var(--ink)]">Aktif Oturumlar</h2><p className="mt-1 text-xs text-[var(--muted)]">Şirket, şube ve rol kapsamına göre açık olan oturumlar.</p></div>
        ...
        {loading ? <div className="px-5 py-10 text-sm text-[var(--muted)]">Oturumlar yükleniyor…</div> : sessions.length === 0 ? <div className="px-5 py-10 text-sm text-[var(--muted)]">Açık oturum bulunamadı.</div> : (
          <div className="divide-y divide-[var(--line)]">{sessions.map((session) => (
            <article key={session.id} className="flex flex-col gap-4 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="min-w-0 space-y-1"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-[var(--accent-soft)] px-2.5 py-1 text-[11px] font-semibold text-[var(--accent)]">{session.roleScope ? userLabel(session.roleScope) : "Kapsam bilinmiyor"}</span><span className="font-mono text-[11px] text-[var(--muted)]">Oturum: {session.id.slice(0, 8)}…</span></div><p className="text-sm font-medium text-[var(--ink)]">{session.branchId ? "Şube bağlamlı oturum" : "Şirket / merkezi oturum"}</p><p className="text-xs text-[var(--muted)]">Son yenileme: {new Date(session.rotatedAt).toLocaleString("tr-TR")} · Kalan süre: {formatExpiry(session.expiresInSeconds)}</p></div>
      ...
      <section className="overflow-hidden rounded-[var(--radius-card)] border border-[var(--line)] bg-[var(--surface)] shadow-[var(--shadow-soft)]">
        <div className="border-b border-[var(--line)] px-5 py-4"><h2 className="text-sm font-semibold text-[var(--ink)]">Son Güvenlik Olayları</h2><p className="mt-1 text-xs text-[var(--muted)]">İki aşamalı doğrulama ve oturum kapatma işlemleri değiştirilemeyen güvenlik kayıtlarından gösterilir.</p></div>
        <div className="divide-y divide-[var(--line)]">
          {securityEvents.length ? securityEvents.map((event) => (
            <div key={event.id} className="flex items-center justify-between gap-4 px-5 py-3 text-xs"><div><span className="font-semibold text-[var(--ink)]">{userLabel(event.resource)}</span><span className="ml-2 rounded-full bg-[var(--surface-2)] px-2 py-1 text-[var(--muted)]">{userLabel(event.action)}</span></div><span className="whitespace-nowrap text-[var(--muted)]">{new Date(event.createdAt).toLocaleString("tr-TR")}</span></div>
          )) : <div className="px-5 py-8 text-sm text-[var(--muted)]">Henüz güvenlik kaydı bulunmuyor.</div>}
        </div>
      </section>
    </main>
  );
