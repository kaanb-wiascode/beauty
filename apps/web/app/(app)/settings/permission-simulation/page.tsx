  ...
  return (
    <main className="mx-auto w-full max-w-[1240px] space-y-6 pb-10">
      <header className="border-b border-[var(--line)] pb-5">
        ...
        <h1 className="text-[28px] font-semibold tracking-[-0.04em] text-[var(--ink)]">Yetki Simülasyonu</h1>
        <p className="mt-1 max-w-3xl text-sm text-[var(--muted)]">Bir kullanıcının hangi şirkete, şubelere, modüllere ve yüksek riskli aksiyonlara erişebildiğini impersonation yapmadan önizleyin.</p>
      </header>
      ...
      {!simulating && effective ? <>
        ...
        {riskPermissions.length ? <section className="rounded-2xl border border-[#f0d8d8] bg-[#fffafa] p-5"><h2 className="text-sm font-semibold text-[#9a4545]">Yüksek Riskli Etkin Yetkiler</h2><div className="mt-3 flex flex-wrap gap-2">{riskPermissions.map((permission) => <span key={`${permission.source}-${permission.id}`} className="rounded-full border border-[#f0d8d8] bg-white px-2.5 py-1 text-xs font-semibold text-[#9a4545]">{permission.resource}.{permission.action} · {permission.source}</span>)}</div></section> : null}

        <section className="overflow-hidden rounded-2xl border border-[var(--line)] bg-[var(--surface)]">
          <div className="flex flex-col gap-3 border-b border-[var(--line)] px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-sm font-semibold">Etkin Yetkiler</h2><p className="mt-1 text-xs text-[var(--muted)]">Kaynak rol ve aktif temporary grant birleşimi.</p></div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Yetki ara…" className="rounded-lg border border-[var(--line)] bg-white px-3 py-2 text-xs" /></div>
          <div className="divide-y divide-[var(--line)]">{rows.map((permission) => <article key={`${permission.source}-${permission.id}-${permission.temporaryGrant?.id ?? "role"}`} className="grid gap-2 px-5 py-4 md:grid-cols-[1fr_160px_2fr]"><div><div className="font-mono text-xs font-semibold text-[var(--ink)]">{permission.resource}.{permission.action}</div><div className="mt-1 text-xs text-[var(--muted)]">{permission.description ?? "Açıklama yok"}</div></div><div><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${permission.source === "TEMPORARY" ? "bg-[#fff4e6] text-[#a66518]" : "bg-[var(--accent-soft)] text-[var(--accent)]"}`}>{permission.source === "TEMPORARY" ? "Geçici" : "Rol"}</span></div><div className="text-xs text-[var(--muted)]">{permission.source === "TEMPORARY" && permission.temporaryGrant ? <>{permission.temporaryGrant.branchId ? `Şube: ${permission.temporaryGrant.branchId}` : "Şirket geneli"}<br />Bitiş: {new Date(permission.temporaryGrant.endsAt).toLocaleString("tr-TR")}<br />Gerekçe: {permission.temporaryGrant.reason}</> : `Kaynak rol: ${permission.sourceRole?.name ?? effective.role.name}`}</div></article>)}</div>
        </section>
      </> : null}
    </main>
  );
