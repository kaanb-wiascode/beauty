  ...
  return (
    <main className="mx-auto w-full max-w-[1320px] space-y-6 pb-10">
      ...
      <form onSubmit={submit} className="rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-5 shadow-[var(--shadow-soft)]">
        <div className="mb-4"><h2 className="text-sm font-semibold text-[var(--ink)]">Yeni Geçici Yetki</h2><p className="mt-1 text-xs text-[var(--muted)]">Şube seçilmezse yetki şirket genelinde geçerli olur. Aynı yetki ve kapsam için çakışan tarih aralıkları oluşturulamaz.</p></div>
      ...
      <section className="overflow-hidden rounded-2xl border border-[var(--line)] bg-[var(--surface)]">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-sm"><thead className="border-b border-[var(--line)] bg-[var(--surface-2)] text-xs text-[var(--muted)]"><tr><th className="px-4 py-3 font-medium">Kullanıcı</th><th className="px-4 py-3 font-medium">Yetki</th><th className="px-4 py-3 font-medium">Kapsam</th><th className="px-4 py-3 font-medium">Geçerlilik</th><th className="px-4 py-3 font-medium">Gerekçe</th><th className="px-4 py-3 font-medium">Durum</th><th className="px-4 py-3 text-right font-medium">İşlem</th></tr></thead><tbody>
          {!loading && filtered.map((grant) => <tr key={grant.id} className="border-b border-[var(--line)] last:border-0"><td className="px-4 py-4 text-xs text-[var(--ink)]">{grant.userEmail}</td><td className="px-4 py-4 text-xs">{userPermissionLabel(grant.permissionResource, grant.permissionAction)}</td><td className="px-4 py-4 text-xs text-[var(--muted)]">{grant.branchName ?? "Şirket geneli"}</td><td className="px-4 py-4 text-xs text-[var(--muted)]">{formatDate(grant.startsAt)}<br />{formatDate(grant.endsAt)}</td><td className="max-w-[260px] px-4 py-4 text-xs text-[var(--muted)]">{grant.reason}</td><td className="px-4 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${badgeClass[grant.status]}`}>{userLabel(grant.status)}</span></td><td className="px-4 py-4 text-right">{grant.status === "ACTIVE" || grant.status === "SCHEDULED" ? <button type="button" disabled={revoking === grant.id} onClick={() => void revoke(grant.id)} className="rounded-lg border border-[#f0d8d8] px-3 py-2 text-xs font-medium text-[#9a4545] disabled:opacity-50">{revoking === grant.id ? "İptal ediliyor…" : "İptal Et"}</button> : <span className="text-xs text-[var(--muted)]">—</span>}</td></tr>)}
