...
function Icon({
  ...
  name: "calendar" | "clock" | "check" | "x" | "users" | "chevron" | "note";
  className?: string;
}) {
  const common = {
    className,
...
export default function AppointmentsPage() {
  ...
  async function loadAppointments() {
    ...
    try {
      const result = await api<Paginated<Appointment>>(
        withQuery("/appointments", { page: 1, limit: 100, from: dateFrom, to: dateTo }),
      );
  ...
  const staffMap = useMemo(
    () => new Map(staff.map((item) => [item.id, fullName(item.firstName, item.lastName)])),
    [staff],
  );
  ...
  }, [appointments, customerMap, search, serviceFilter, serviceMap, staffFilter, staffMap, statusFilter]);
  ...
  const hasFilters = Boolean(search.trim() || staffFilter || serviceFilter || statusFilter);
  ...
  async function cancelAppointment() {
    if (!pendingCancel || !canCancelAppointment) return;
    setSaving(true);
    setError("");
    try {
      ...
      <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <DataView className="min-w-0">
          <DataViewToolbar
            ...
            filters={
              <>
                <FilterChip active={!statusFilter} count={counts.total} onClick={() => setStatusFilter("")}>Tümü</FilterChip>
                <FilterChip active={statusFilter === "SCHEDULED"} count={counts.waiting} onClick={() => setStatusFilter("SCHEDULED")}>Planlandı</FilterChip>
                <FilterChip active={statusFilter === "CONFIRMED"} count={counts.confirmed} onClick={() => setStatusFilter("CONFIRMED")}>Onaylandı</FilterChip>
                <FilterChip active={statusFilter === "COMPLETED"} count={counts.completed} onClick={() => setStatusFilter("COMPLETED")}>Tamamlandı</FilterChip>
                <FilterChip active={statusFilter === "CANCELLED"} count={counts.cancelled} onClick={() => setStatusFilter("CANCELLED")}>İptal</FilterChip>
                <FilterChip active={statusFilter === "NO_SHOW"} count={counts.noShow} onClick={() => setStatusFilter("NO_SHOW")}>Gelmedi</FilterChip>
              </>
            }
          />
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <div className="min-w-[820px]">
                <div
                  className="grid grid-cols-[58px_repeat(var(--staff-count),minmax(150px,1fr))]"
                  style={{ "--staff-count": Math.max(staffColumns.length, 1) } as CSSProperties}
                >
                  <div className="border-r border-[var(--line)] bg-[var(--surface)]" />
                  {staffColumns.map((member) => {
                    const name = fullName(member.firstName, member.lastName);
                    return (
                      <div key={member.id} className="border-b border-r border-[var(--line)] bg-[var(--surface)] px-3 py-3">
                        <div className="flex min-w-0 items-center gap-2">
                          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[var(--accent-soft)] text-[10px] font-semibold text-[var(--accent)]">{initials(name)}</div>
                          <div className="min-w-0">
                            <p className="truncate text-[11px] font-semibold text-[var(--ink)]">{name}</p>
                            <p className="truncate text-[10px] text-[var(--muted)]">Personel</p>
                          </div>
                        </div>
                      </div>
                    );
                ...
                <div className="relative grid grid-cols-[58px_1fr]">
                  <div className="border-r border-[var(--line)] bg-[var(--surface)]">
                    {slots.slice(0, -1).map((minutes) => (
                      <div key={minutes} className="flex h-[58px] items-start justify-end border-b border-[var(--line)] pr-2 pt-2 text-[10px] font-medium text-[var(--muted)]">
                  ...
                  <div
                    ...
                    style={{
                      gridTemplateColumns: `repeat(${Math.max(staffColumns.length, 1)}, minmax(150px, 1fr))`,
                      height: `${(slots.length - 1) * SLOT_HEIGHT}px`,
                    }}
                  >
                    {staffColumns.map((member) => (
                      <div key={member.id} className="relative border-r border-[var(--line)] bg-[linear-gradient(to_bottom,transparent_57px,var(--line)_58px)] [background-size:100%_58px]">
                        {filteredAppointments
                          .filter((item) => item.staffId === member.id)
                          .map((item) => {
                            const customer = customerMap.get(item.customerId) ?? "Müşteri";
                            const service = serviceMap.get(item.serviceId) ?? "Hizmet";
                            ...
                            return (
                              <button
                                ...
                                className={`absolute left-1.5 right-1.5 z-10 overflow-hidden rounded-[11px] border p-2 text-left shadow-[0_2px_7px_rgba(17,70,104,0.04)] transition hover:-translate-y-px hover:shadow-md ${statusStyles[item.status]} ${isSelected ? "ring-2 ring-[rgba(22,116,189,0.25)] ring-offset-1" : ""}`}
                                style={{ top: appointmentTop(item.startAt), height: appointmentHeight(item.startAt, item.endAt) }}
                              >
                                <div className="flex items-center justify-between gap-1 text-[9px] font-medium opacity-75">
                                  <span>{formatTime(item.startAt)} – {formatTime(item.endAt)}</span>
                                  <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${dotStyles[item.status]}`} />
                                </div>
                                <p className="mt-1 truncate text-[11px] font-semibold">{customer}</p>
                                <p className="truncate text-[10px] opacity-80">{service}</p>
                                <span className="mt-1 inline-flex max-w-full items-center rounded-full bg-white/60 px-1.5 py-0.5 text-[8px] font-medium">{appointmentStatusLabel(item.status)}</span>
                              </button>
                            );
                          })}
                      </div>
        ...
        <aside className="space-y-4">
            ...
            {selected ? (
              <div className="p-5">
                ...
                <div className="mt-5 flex items-center gap-3">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[var(--accent-soft)] text-[12px] font-semibold text-[var(--accent)]">{initials(customerName)}</div>
                  <div className="min-w-0">
                    <p className="truncate text-[15px] font-semibold text-[var(--ink)]">{customerName}</p>
                    <p className="text-[11px] text-[var(--muted)]">Müşteri</p>
                  </div>
                </div>

                <div className="mt-5 divide-y divide-[var(--line)] rounded-[15px] border border-[var(--line)] bg-[var(--surface-soft)]">
                  <DetailRow icon="note" label="Hizmet" value={selectedService} meta={selectedServicePrice ? `₺${Number(selectedServicePrice).toLocaleString("tr-TR")}` : "—"} />
                  <DetailRow icon="users" label="Personel" value={selectedStaff} />
                  {selected.notes ? <DetailRow icon="note" label="Not" value={selected.notes} multiline /> : null}
                </div>
          ...
          <section className="surface overflow-hidden rounded-[22px]">
            ...
            <div className="divide-y divide-[var(--line)] px-5">
              {appointments.slice(0, 5).map((item) => {
                const name = customerMap.get(item.customerId) ?? "Müşteri";
                return (
                  <button type="button" key={item.id} onClick={() => setSelectedId(item.id)} className="flex w-full items-center gap-2.5 py-3 text-left">
                    <span className="w-10 shrink-0 text-[10px] font-medium text-[var(--muted)]">{formatTime(item.startAt)}</span>
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--accent-soft)] text-[8px] font-semibold text-[var(--accent)]">{initials(name)}</span>
                    <span className="min-w-0 flex-1"><span className="block truncate text-[10px] font-semibold text-[var(--ink)]">{name}</span><span className="block truncate text-[9px] text-[var(--muted)]">{serviceMap.get(item.serviceId) ?? "Hizmet"}</span></span>
                    <i className={`h-2 w-2 shrink-0 rounded-full ${dotStyles[item.status]}`} />
                  </button>
                );
              })}
            </div>
          ...
          <section className="surface rounded-[22px] p-5">
            ...
            <div className="mt-4 space-y-3">
              ...
              <SummaryRow label="İptal / Gelmedi" value={counts.cancelled + counts.noShow} dot="bg-[#ef5b70]" />
            </div>
          </section>
        </aside>
      </div>
...
function DetailRow({
  ...
  return (
    <div className="flex items-center gap-3 p-3">
      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[10px] bg-[var(--surface)] text-[var(--accent)]"><Icon name={icon} className="h-4 w-4" /></div>
      <div className="min-w-0 flex-1"><p className="text-[9px] uppercase tracking-[0.08em] text-[var(--muted)]">{label}</p><p className={`${multiline ? "whitespace-pre-wrap leading-5" : "truncate"} text-[12px] font-semibold text-[var(--ink)]`}>{value}</p></div>
      {meta ? <span className="text-[11px] font-medium text-[var(--muted)]">{meta}</span> : null}
    </div>
  );
}

function SummaryRow({ label, value, dot }: { label: string; value: number; dot: string }) {
  return <div className="flex items-center justify-between text-[11px]"><span className="flex items-center gap-2 text-[var(--muted)]"><i className={`h-2 w-2 rounded-full ${dot}`} />{label}</span><strong className="text-[var(--ink)]">{value}</strong></div>;
}
