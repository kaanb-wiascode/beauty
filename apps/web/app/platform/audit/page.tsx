  ...
  return (
    <div className="mx-auto max-w-[1380px] space-y-7 pb-12">
      ...
      <section className="grid gap-3 rounded-[24px] border border-white/10 bg-white/[.035] p-4 md:grid-cols-2 xl:grid-cols-5">
        <Filter label="Kaynak Modül" value={resource} onChange={(value) => { setOffset(0); setResource(value); }} placeholder="platform_iam" />
        <Filter label="İşlem Türü" value={action} onChange={(value) => { setOffset(0); setAction(value); }} placeholder="role.assign" />
...
function AuditRow({ event }: { event: PlatformAuditList["items"][number] }) {
  ...
  return <details className="group px-5 py-4"><summary className="grid cursor-pointer list-none gap-3 md:grid-cols-[1fr_.8fr_1fr_.8fr] md:items-center"><div><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-semibold text-white">{userPermissionLabel(event.resource, event.action)}</p>{event.riskLevel ? <span className="rounded-full border border-amber-400/20 bg-amber-400/[.07] px-2 py-0.5 text-[8px] font-semibold text-amber-200">{userLabel(event.riskLevel)}</span> : null}</div><p className="mt-1 text-[10px] text-white/30">{event.id}</p></div><div><p className="text-[9px] uppercase tracking-[.11em] text-white/25">Aktör</p><p className="mt-1 text-[11px] text-white/60">{actor}</p></div><div><p className="text-[9px] uppercase tracking-[.11em] text-white/25">Hedef</p><p className="mt-1 text-[11px] text-white/60">{[event.targetEntityType, event.targetEntityId].filter(Boolean).join(": ") || event.targetTenantId || "—"}</p></div><div><p className="text-[9px] uppercase tracking-[.11em] text-white/25">Zaman</p><p className="mt-1 text-[11px] text-white/60">{date.format(new Date(event.createdAt))}</p></div></summary><div className="mt-4 grid gap-3 rounded-2xl border border-white/[.06] bg-black/15 p-4 xl:grid-cols-2"><Data label="Gerekçe" value={event.reason} /><Data label="İstek Kayıt No" value={event.requestId} /><Data label="Onay Talebi Kayıt No" value={event.approvalRequestId} /><Data label="Kaynak IP Adresi" value={event.sourceIp} /><Data label="Tarayıcı / Uygulama Bilgisi" value={event.userAgent} /><Data label="İşlem İzleme No" value={event.correlationId} /><JsonBlock label="Önceki Değerler" value={event.beforeState} /><JsonBlock label="Sonraki Değerler" value={event.afterState} /><div className="xl:col-span-2"><JsonBlock label="Ek Teknik Bilgiler" value={event.metadata} /></div></div></details>;
}
