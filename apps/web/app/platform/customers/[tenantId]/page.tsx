...
export default function PlatformCustomer360Page() {
  ...
  if (!data) {
    return <div className="grid min-h-[55vh] place-items-center"><div className="h-9 w-9 animate-spin rounded-full border-2 border-white/15 border-t-violet-400" /></div>;
  }
      ...
      <div className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
        <Panel title="Organizasyon yapısı" eyebrow="Şirket ve şubeler">
          <div className="divide-y divide-white/[.07]">
            ...
            {!data.companies.length ? <p className="py-8 text-center text-sm text-white/35">Şirket kaydı bulunmuyor.</p> : null}
          </div>
        </Panel>
        ...
        <Panel title="Üyelik dağılımı" eyebrow="Kullanıcı rolleri">
          <div className="space-y-3">
            ...
            {!data.membershipBreakdown.length ? <p className="py-8 text-center text-sm text-white/35">Üyelik kaydı bulunmuyor.</p> : null}
          </div>
        </Panel>
      </div>
...
function Metric({ label, value, detail, suffix = "" }: { label: string; value: number; detail?: string; suffix?: string }) {
  return <div className="rounded-[22px] border border-white/10 bg-white/[.035] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,.04)] backdrop-blur-xl"><p className="text-[9px] font-semibold uppercase tracking-[.15em] text-white/35">{label}</p><p className="mt-3 text-2xl font-semibold tracking-[-.04em] text-white">{number.format(value)}{suffix}</p>{detail ? <p className="mt-1 text-[10px] text-emerald-300/70">{detail}</p> : null}</div>;
}

function Panel({ title, eyebrow, children }: { title: string; eyebrow: string; children: React.ReactNode }) {
  return <section className="rounded-[26px] border border-white/10 bg-white/[.035] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,.04)] backdrop-blur-xl sm:p-6"><p className="text-[9px] font-semibold uppercase tracking-[.15em] text-white/30">{eyebrow}</p><h2 className="mt-1 text-base font-semibold text-white">{title}</h2><div className="mt-5">{children}</div></section>;
}

function Cell({ label, value }: { label: string; value: string }) {
  return <div className="min-w-0"><p className="text-[9px] font-semibold uppercase tracking-[.12em] text-white/25">{label}</p><p className="mt-1 truncate text-xs font-medium text-white/65" title={value}>{value}</p></div>;
}
