...
export default function PlatformIamPage() {
  ...
  useEffect(() => {
    ...
    getPlatformIamOverview()
      ...
      .catch((reasonValue: unknown) => {
        if (!active) return;
        setError(reasonValue instanceof ApiError ? reasonValue.message : "Platform IAM verileri yüklenemedi.");
      });
    return () => { active = false; };
  }, []);
  ...
  const run = async (key: string, action: () => Promise<unknown>, success: string) => {
    if (!reasonValid) {
      setError("Privileged işlem için 8-500 karakter arasında operasyon nedeni girilmelidir.");
      return;
    }
    setBusy(key);
    setError("");
    ...
    try {
      ...
      setError(reasonValue instanceof ApiError ? reasonValue.message : "Platform IAM işlemi tamamlanamadı.");
    } finally {
      setBusy("");
    }
  };
    ...
    <div className="mx-auto max-w-[1380px] space-y-7 pb-12">
      <header className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-white/45">Platform yöneticilerini, sistem rollerini ve permission matrisini tenant rollerinden bağımsız olarak yönet.</p>
        </div>
        <span className={`w-fit rounded-full border px-3 py-2 text-[10px] font-semibold uppercase tracking-[.12em] ${canManage ? "border-emerald-400/20 bg-emerald-400/[.08] text-emerald-200" : "border-white/10 bg-white/[.04] text-white/40"}`}>
          {canManage ? "Privileged management enabled" : "Read-only IAM view"}
        </span>
      </header>
      ...
      {canManage ? (
        <Panel title="Privileged operation context" eyebrow="Reason required">
          <div className="grid gap-4 xl:grid-cols-[1fr_auto] xl:items-end">
            <label className="block">
              <span className="text-[10px] font-semibold uppercase tracking-[.12em] text-white/35">Operasyon nedeni</span>
      ...
      {canManage ? (
        <Panel title="Platform admin provision" eyebrow="Explicit assignment only">
          <div className="grid gap-3 lg:grid-cols-[1.5fr_1fr_auto] lg:items-end">
            <Field label="User ID"><input value={newUserId} onChange={(event) => setNewUserId(event.target.value)} placeholder="users.id" className="input" /></Field>
            ...
            <button type="button" disabled={!reasonValid || !newUserId.trim() || busy === "provision"} onClick={() => run("provision", () => provisionPlatformAdmin({ userId: newUserId.trim(), roleSlug: newRole, reason: reason.trim() }), "Platform admin erişimi provision edildi.")} className="action-button">{busy === "provision" ? "İşleniyor…" : "Admin provision et"}</button>
          </div>
        </Panel>
      ) : null}
