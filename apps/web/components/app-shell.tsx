                ...
                return (
                  <Link
                    ...
                    className={cx(
                      ...
                      active ? "bg-[#f2edff] text-[#7657e8] shadow-[inset_0_0_0_1px_rgba(118,87,232,0.06)]" : "text-[#626276] hover:bg-[#f8f7fb] hover:text-[#242332]",
                    )}
                  >
  ...
  return (
    <div className="relative">
      ...
      <nav aria-label="Mobil ana navigasyon" className="grid grid-cols-5 gap-1 px-1 py-1">
        ...
        <button type="button" onClick={() => setMoreOpen((current) => !current)} aria-expanded={moreOpen} aria-label="Diğer menü seçenekleri" className={cx("flex min-w-0 flex-col items-center justify-center gap-1 rounded-[16px] px-1 py-2.5 text-[11px] transition-colors", moreOpen || moreActive ? "bg-[#f2edff] font-medium text-[#7657e8]" : "text-[#777688] hover:bg-[#f7f6fa] hover:text-[#242332]")}> <span className="text-[15px] leading-none tracking-[0.12em]">•••</span><span>Daha</span></button>
      </nav>
    </div>
  );
}
    ...
    <div className="relative min-h-screen lg:grid lg:gap-4 lg:p-4" style={{ gridTemplateColumns: collapsed ? "76px minmax(0, 1fr)" : "260px minmax(0, 1fr)" } as CSSProperties}>
      <aside className={cx("glass sticky top-4 hidden h-[calc(100vh-32px)] flex-col overflow-hidden rounded-[28px] border border-white/80 bg-white/90 shadow-[0_14px_40px_rgba(43,35,72,0.07)] backdrop-blur-2xl lg:flex", collapsed ? "w-[76px]" : "w-[260px]")}>
        <div className={cx("flex items-center border-b border-[#efedf4] py-5", collapsed ? "justify-center px-3" : "justify-between px-5")}>
          ...
          {!collapsed ? <button type="button" onClick={toggleSidebar} aria-label="Menüyü daralt" title="Menüyü daralt" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[10px] text-[#888697] transition hover:bg-[#f6f4fa] hover:text-[#5d5b6b]"><ChevronLeftIcon /></button> : null}
        </div>

        {collapsed ? <button type="button" onClick={toggleSidebar} aria-label="Menüyü genişlet" title="Menüyü genişlet" className="mx-auto mt-3 flex h-9 w-9 items-center justify-center rounded-[12px] text-[#777688] transition hover:bg-[#f6f4fa] hover:text-[#7657e8]"><ChevronRightIcon /></button> : null}

        {!collapsed ? (
          <div className="px-5 pt-4">{user ? <div className="flex items-center gap-3 rounded-[16px] bg-[#faf9fd] px-3 py-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#eee8ff] text-[11px] font-semibold text-[#7657e8]">{getInitials(user.firstName, user.lastName)}</div><div className="min-w-0 flex-1"><p className="truncate text-[12px] font-semibold text-[#292837]">{fullName(user.firstName, user.lastName)}</p><p className="mt-0.5 truncate text-[10px] text-[#9694a1]">{tenant?.name ?? "Salon"}</p></div><ChevronDownIcon /></div> : null}</div>
        ) : <div className="mx-auto mt-4 flex h-9 w-9 items-center justify-center rounded-full bg-[#eee8ff] text-[11px] font-semibold text-[#7657e8]" title={user ? fullName(user.firstName, user.lastName) : "Kullanıcı"}>{user ? getInitials(user.firstName, user.lastName) : "B"}</div>}

        <NavLinks pathname={pathname} collapsed={collapsed} />

        <div className={cx("mt-auto border-t border-[#efedf4]", collapsed ? "flex justify-center p-3" : "p-4")}>
          <button type="button" onClick={() => void logout()} disabled={loggingOut} aria-busy={loggingOut} title={collapsed ? "Çıkış yap" : undefined} className={cx("flex items-center rounded-[13px] text-[12px] text-[#8a8895] transition hover:bg-[#f8f7fb] hover:text-[#2a2937] disabled:cursor-not-allowed disabled:opacity-40", collapsed ? "h-10 w-10 justify-center" : "w-full gap-3 px-3 py-2.5")}> <LogoutIcon /> {!collapsed ? (loggingOut ? "Çıkış yapılıyor..." : "Çıkış yap") : null}</button>
        </div>
      </aside>

      <div className="relative flex min-w-0 flex-col">
        <header className="glass sticky top-0 z-30 flex items-center justify-between rounded-b-[22px] px-4 py-3 lg:hidden">
          <div className="flex min-w-0 items-center gap-2.5"><BrandMark /><div className="min-w-0"><p className="truncate text-[14px] font-semibold tracking-[-0.02em] text-[var(--ink)]">Beauty ERP</p>{tenant?.name ? <p className="truncate text-[10px] text-[var(--muted)]">{tenant.name}</p> : null}</div></div>
          <IconButton type="button" aria-label="Çıkış yap" title="Çıkış yap" disabled={loggingOut} aria-busy={loggingOut} onClick={() => void logout()}><LogoutIcon /></IconButton>
        </header>
