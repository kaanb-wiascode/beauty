...
const navigation = [
  ...
  { href: "/platform/approvals", label: "Approval Queue", eyebrow: "Governance" },
  ...
  return (
    <div className="min-h-screen bg-[#070912] text-white">
      ...
      <div className="relative mx-auto grid min-h-screen max-w-[1680px] lg:grid-cols-[280px_1fr]">
            ...
            {navigation.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`rounded-2xl border px-4 py-3.5 transition ${active ? "border-violet-400/30 bg-violet-400/12 shadow-[inset_0_1px_0_rgba(255,255,255,.06)]" : "border-transparent bg-white/[.025] hover:border-white/10 hover:bg-white/[.05]"}`}
                >
                  <span className="block text-[9px] font-semibold uppercase tracking-[.16em] text-white/35">{item.eyebrow}</span>
                  <span className={`mt-1 block text-sm font-medium ${active ? "text-white" : "text-white/70"}`}>{item.label}</span>
                </Link>
              );
            })}
          </nav>
        ...
        <main className="min-w-0 px-4 py-5 sm:px-6 lg:px-10 lg:py-8 xl:px-12">{children}</main>
      </div>
    </div>
  );
}
