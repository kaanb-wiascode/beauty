...
export function CustomerSearchPicker({
  ...
  useEffect(() => {
    ...
    if (selected && value === `${selected.firstName} ${selected.lastName}`.trim()) {
      setRows([]);
      return;
    }
    if (value.length < 2) {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const result = await api<Paginated<Customer>>(
          withQuery("/customers", { page: 1, limit: 20, search: value }),
        );
        setRows(result.data);
      } catch (requestError) {
        setError(requestError instanceof ApiError ? requestError.message : "Müşteri aranamadı.");
      } finally {
        setLoading(false);
      }
    }, 250);
        ...
        <div className="absolute z-30 max-h-72 w-full overflow-auto rounded-[14px] border border-[var(--line)] bg-white p-1 shadow-xl">
          {rows.map((row) => {
            ...
            return (
              ...
              >
                <span className="block text-[12px] font-semibold">{row.firstName} {row.lastName}</span>
