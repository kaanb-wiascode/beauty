...
COOLIFY_APPLICATION_NAME="${COOLIFY_APPLICATION_NAME:-valoo-production}"
GIT_REPOSITORY="${GIT_REPOSITORY:-https://github.com/kaanb-wiascode/beauty}"
GIT_BRANCH="${GIT_BRANCH:-main}"
COMPOSE_LOCATION="${COMPOSE_LOCATION:-/infrastructure/docker-compose.coolify.production.yml}"
...
)" || fail "Coolify production application creation failed"
...
echo "COOLIFY_PRODUCTION_APPLICATION_UUID=$application_uuid"

if [[ -n "${GITHUB_OUTPUT:-}" ]]; then
  echo "application_uuid=$application_uuid" >> "$GITHUB_OUTPUT"
fi

echo "Coolify production application created: $application_uuid"
