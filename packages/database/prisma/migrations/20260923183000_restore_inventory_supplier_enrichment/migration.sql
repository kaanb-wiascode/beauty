...
BEGIN
  ...
  LOOP
    ...
    SELECT
      NEW."tenantId", b."companyId", material.product_id, warehouse,
      'SERVICE_CONSUMPTION', material.quantity, 'APPOINTMENT', NEW.id,
    ...
    IF stock_row.quantity - material.quantity <= stock_row.minimum_quantity THEN
      ...
      SELECT
        ...
        COALESCE(ps.lead_time_days, 0)
          ...
          + COALESCE(ps.shipping_days, 0) AS total_days
      INTO supplier_row
      ...
      IF NOT EXISTS (
        ...
        INSERT INTO inventory_purchase_requests(
          tenant_id, company_id, warehouse_id, product_id, supplier_id,
          current_quantity, requested_quantity, estimated_unit_cost,
          suggested_delivery_at, reason
        )
        SELECT
          NEW."tenantId",
          b."companyId",
        ...
        INSERT INTO inventory_notifications(
          tenant_id, company_id, branch_id, role_target, type, title,
